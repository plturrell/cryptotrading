...
class StrandsGleanMCPClient:
    ...
    async def connect(self, server_command: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not server_command:
                server_command = [
                    sys.executable,
                    str(project_root / "src" / "cryptotrading" / "core" / "protocols" / "mcp" / "strands_glean_server.py"),
                    "stdio"
                ]
            ...
            await self.client.initialize({
                ...
                "capabilities": {
                    "tools": {},
                    "roots": [{"uri": f"file://{project_root}"}]
                },
                "clientInfo": {
                    "name": "strands-glean-cli",
                    "version": "1.0.0"
                }
            })
    ...
    async def index_project(self, unit_name: str = "main", force: bool = False) -> Dict[str, Any]:
        """Index the project"""
        return await self.call_tool("glean_index_project", {
            "unit_name": unit_name,
            "force_reindex": force
        })
    ...
    async def search_symbols(self, pattern: str, limit: int = 10) -> Dict[str, Any]:
        """Search for symbols"""
        return await self.call_tool("glean_symbol_search", {
            "pattern": pattern,
            "limit": limit
        })
    ...
    async def analyze_dependencies(self, symbol: str, depth: int = 3) -> Dict[str, Any]:
        ...
        return await self.call_tool("glean_dependency_analysis", {
            "symbol": symbol,
            "depth": depth
        })
    ...
    async def review_architecture(self, component: str, rules: list = None) -> Dict[str, Any]:
        """Review architecture"""
        return await self.call_tool("glean_architecture_review", {
            "component": component,
            "rules": rules or ["layer_separation", "dependency_direction", "circular_deps"]
        })
    
    async def enhance_with_ai(self, analysis_data: Dict[str, Any], enhancement_type: str = "summary") -> Dict[str, Any]:
        """Enhance analysis with AI"""
        return await self.call_tool("ai_enhance_analysis", {
            "analysis_data": analysis_data,
            "enhancement_type": enhancement_type,
            "ai_provider": "grok"
        })
    
    async def ai_code_review(self, files: list, review_type: str = "comprehensive") -> Dict[str, Any]:
        """AI-powered code review"""
        return await self.call_tool("ai_code_review", {
            "files": files,
