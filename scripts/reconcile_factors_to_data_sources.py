...
from cryptotrading.core.factors import ALL_FACTORS, FactorCategory, DataSource
...
def analyze_factors():
    ...
    for category in FactorCategory:
        ...
        calculable_in_cat = [f for f in factors_in_cat if get_calculation_method(f) != "NOT AVAILABLE"]
        
        category_stats[category.value] = {
            'total': len(factors_in_cat),
            'calculable': len(calculable_in_cat),
            'percentage': (len(calculable_in_cat) / len(factors_in_cat) * 100) if factors_in_cat else 0
        }
        
        print(f"{category.value.upper():<20}: {len(calculable_in_cat):2d}/{len(factors_in_cat):2d} "
    ...
    for factor in results['calculable']:
        print(f"{factor['number']:<3} {factor['name']:<25} {factor['category']:<15} {factor['calculation_method']:<20}")
    ...
    for factor in results['not_calculable']:
        ...
        print(f"{factor['number']:<3} {factor['name']:<25} {factor['category']:<15} {sources_str:<30}")
