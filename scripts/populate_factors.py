...
async def store_factors_in_db(symbol: str, df: pd.DataFrame):
    ...
    factors_to_store = [
        'sma_20', 'ema_50', 'volume_sma_20', 'obv',
        'rsi_14', 'macd', 'macd_signal', 'bb_position',
        'stoch_k', 'atr', 'volatility'
    ]
    ...
    if records:
        ...
        cursor.executemany("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, [(r['symbol'], r['factor_name'], r['timestamp'], r['value'], 
               r['quality_score'], r['confidence'], r['calculation_method'],
               r['input_data_points'], r['passed_validation'], r['calculation_version']) 
              for r in records])
