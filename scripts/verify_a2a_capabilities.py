...
def get_cli_commands(script_name):
    ...
    try:
        result = subprocess.run(
            ["python3", script_name, "--help"], capture_output=True, text=True, timeout=10
        )
...
def main():
    ...
    for result in sorted_results:
        if result["cli_available"]:
            ...
            print(
                f"{status} {result['agent_id']}: {score:.1%} ({result['covered_capabilities']}/{result['total_capabilities']})"
            )
        else:
