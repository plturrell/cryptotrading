...
class RealtimeMCPServer:
    ...
    async def create_enhanced_server(self):
        ...
        try:
            ...
            self.file_watcher = await create_file_watcher(
                watch_directories=watch_dirs, project_root=str(project_root), enable_ai=True
            )
            ...
            class EnhancedMCPServer:
                ...
                async def call_tool(
                    ...
                    try:
                        ...
                        # Real-time tools
                            ...
                            return await self.get_recent_changes(arguments.get("limit", 20))

                        elif tool_name == "realtime_get_analyses":
                            return await self.get_recent_analyses(arguments.get("limit", 20))
                ...
                async def get_statistics(self) -> Dict[str, Any]:
                    ...
                    try:
                        ...
                        realtime_stats = (
                            self.file_watcher.get_statistics() if self.file_watcher else {}
                        )
                    ...
                    try:
                        if self.file_watcher:
                            ...
                            response_data = {
                                ...
                                "files_being_watched": self.file_watcher.stats.get(
                                    "files_watched", 0
                                ),
                            }
                        else:
                            response_data = {"status": "file_watcher_unavailable"}
                        ...
                        return {
                            "content": [
                                {"type": "text", "text": f"Start watching error: {str(e)}"}
                            ],
                            "isError": True,
                        }
                            ...
                            response_data = {
                                ...
                                "recent_activity": {
                                    "changes_last_hour": len(
                                        [
                                            c
                                            for c in self.realtime_server.recent_changes
                                            if (
                                                datetime.now()
                                    ...
                                    "analyses_last_hour": len(
                                        [
                                            a
                                            for a in self.realtime_server.recent_analyses
                                            if (
                                                datetime.now()
    ...
    def _on_analysis_complete(self, result):
        ...
        if result.ai_insights:
            logger.info(
                f"AI analysis: {Path(result.file_path).name} - {len(result.symbols_found)} symbols, {result.processing_time:.2f}s"
            )
        else:
            logger.info(
                f"Analysis: {Path(result.file_path).name} - {len(result.symbols_found)} symbols, {result.processing_time:.2f}s"
            )
...
async def main():
    ...
    parser.add_argument(
        "--start-watching", action="store_true", help="Start file watching immediately"
    )
