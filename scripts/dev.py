...
from src.cryptotrading.core.protocols.mcp.quick_start import MCPTemplates, MCPQuickStart
...
class MCPDevServer:
    ...
    def _load_or_create_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "watch_dirs": [
                "src/cryptotrading/core/protocols/mcp",
                "api"
            ]
        }
    ...
    async def start(self):
        ...
        print(f"📚 Docs: http://{self.config['server']['host']}:{self.config['server']['port']}/api/mcp (GET)")
        
        if not self.config["security"]["require_auth"]:
            print("🔓 Authentication: DISABLED (development mode)")
        
        print("\n📋 Available endpoints:")
        print(f"  POST /api/mcp - Main MCP endpoint")
        print(f"  GET  /api/mcp - Status and documentation")
    ...
    async def _create_server(self):
        ...
        if server_config.get("template") == "dev":
            ...
            self.server = (MCPQuickStart(server_config["name"])
                          .with_transport(
    ...
    async def _add_configured_tools(self):
        ...
        for tool_config in self.config.get("tools", []):
            ...
            if tool_name == "echo":
                ...
                self.server.add_simple_tool("calculator", "Calculate expressions", calculator,
        ...
        class ReloadHandler(FileSystemEventHandler):
            ...
            def on_modified(self, event):
                ...
                if event.src_path.endswith('.py'):
                    ...
                    print("⏳ Hot reload triggered... (restart manually for now)")
...
class MCPDevCLI:
    ...
    def _setup_parser(self):
        ...
        init_parser.add_argument('--template', choices=['basic', 'crypto', 'files'], default='basic')
    ...
    async def _init_command(self, args):
        ...
        print(f"🚀 Start with: python -m scripts.dev start")
...
Add custom tools in the `tools/` directory. Each tool should export functions that match the MCP tool interface.
