...
from config.secret_manager import Environment, SecretManager, SecretType
...
def migrate_env_files():
    ...
    migration_results = {"files_processed": [], "secrets_migrated": {}, "errors": []}
                ...
                for key, value in secrets_in_file.items():
                    ...
                    if success:
                        ...
                        migration_results["errors"].append(
                            f"Failed to migrate {key} from {env_file}"
                        )
...
def main():
    ...
    report_path = (
        Path.cwd() / f"SECRET_MIGRATION_REPORT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    )
