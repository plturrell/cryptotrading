    ...
    try:
        ...
        try:
            ...
            await agent.log_performance_metric(
                ...
                duration_ms=125.5,
                additional_metrics={"cpu_usage": 45.2, "memory_mb": 128.7},
            )
        ...
        try:
            ...
            await agent.store_calculation_result(
                ...
                input_parameters={"symbol": "ETH-USD", "indicators": ["RSI", "MACD", "Bollinger"]},
                result={
                    "rsi": 65.2,
        ...
        try:
            await agent.store_market_analysis(
                ...
                signals={"buy_signals": 3, "sell_signals": 1, "overall": "bullish"},
                recommendation="buy",
                timeframe="4h",
                confidence=0.71,
            )
        ...
        try:
            agent_state = {
                ...
                "risk_parameters": {"max_position_size": 0.25, "stop_loss_pct": 0.05},
                "strategy_state": {
            ...
            backup_result = await agent.backup_agent_state(
                ...
                configuration={"model_provider": "grok4", "risk_tolerance": "moderate"},
                version="1.2",
            )
        ...
        try:
            from src.cryptotrading.infrastructure.analysis.all_segregated_mcp_tools import (
                ...
                get_all_mcp_tools_with_schemas,
            )
            ...
            s3_tool_count = sum(
                1 for name in all_tools.keys() if "s3" in name.lower() or "storage" in name.lower()
            )
            s3_schema_count = sum(
                1
                for tool in tool_schemas
                if "s3" in tool.get("name", "").lower() or "storage" in tool.get("name", "").lower()
            )
