...
try:
    from cryptotrading.infrastructure.registry.a2a_registry_v2 import EnhancedA2ARegistry, AgentHealthStatus
    ...
    class FallbackA2ARegistry:
        ...
        async def discover_agents(self, filter_type: str = None, filter_value: str = None) -> List[Dict[str, Any]]:
...
@click.option('--agent-type', '-t', required=True, help='Agent type (data_loader, ml_trainer, etc.)')
...
@click.option('--filter-type', '-f', type=click.Choice(['capability', 'agent_type', 'status']), 
    ...
    try:
        ...
        if verbose:
            ...
            if agent_id:
                ...
                print(f"Minutes Since Heartbeat: {health_data.get('minutes_since_heartbeat', 'N/A')}")
                ...
                for aid, health in health_data.items():
                    if isinstance(health, dict) and "error" not in health:
                        ...
                        health_emoji = "🟢" if health_score > 80 else "🟡" if health_score > 50 else "🔴"
                        print(f"  {health_emoji} {aid}: {health_score:.1f}/100 ({health.get('status', 'unknown')})")
...
@cli.command('list')
@click.option('--status-filter', type=click.Choice(['active', 'inactive', 'error']), 
...
async def list_agents(ctx, status_filter):
    ...
    try:
        agents = await registry.discover_agents("status", status_filter) if status_filter else await registry.discover_agents()
