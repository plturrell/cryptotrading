...
def test_crypto_data_operations(s3_service):
    ...
    try:
        ...
        ohlcv_success = data_manager.save_ohlcv_data(
            symbol="BTC-USD", timeframe="1h", ohlcv_data=sample_ohlcv, timestamp=datetime.utcnow()
        )
        ...
        portfolio_success = data_manager.save_user_portfolio(
            user_id="test_user_123", portfolio=sample_portfolio, timestamp=datetime.utcnow()
        )
...
def generate_sample_analytics():
    ...
    return {
        ...
        "indicators": {
            "rsi": 65.5,
            "macd": {"macd": 1250.5, "signal": 1100.2, "histogram": 150.3},
