    ...
    class FallbackStrandsGleanAgent:
        ...
        async def search_symbols(self, query, scope="global"):
            ...
            symbols = [
                ...
                {"name": f"process_{query.lower()}", "type": "function", "file": "processors.py", "line": 123},
                {"name": f"{query.upper()}_CONFIG", "type": "constant", "file": "config.py", "line": 12}
            ]
            return {
                "query": query,
                "scope": scope,
...
async def dependencies(ctx, root_path):
    ...
    try:
        ...
        click.echo(f"Graph: {graph['nodes']} nodes, {graph['edges']} edges, {graph['clusters']} clusters")
