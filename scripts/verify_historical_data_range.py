...
def test_actual_historical_data_range():
    ...
    try:
        ...
        test_ranges = [
            ("5 years", (datetime.now() - timedelta(days=5*365)).strftime('%Y-%m-%d'), datetime.now().strftime('%Y-%m-%d')),
            ("3 years", (datetime.now() - timedelta(days=3*365)).strftime('%Y-%m-%d'), datetime.now().strftime('%Y-%m-%d')),
            ("2 years", (datetime.now() - timedelta(days=2*365)).strftime('%Y-%m-%d'), datetime.now().strftime('%Y-%m-%d')),
            ("1 year", (datetime.now() - timedelta(days=365)).strftime('%Y-%m-%d'), datetime.now().strftime('%Y-%m-%d')),
            ("6 months", (datetime.now() - timedelta(days=180)).strftime('%Y-%m-%d'), datetime.now().strftime('%Y-%m-%d')),
        ]
            ...
            for period_name, start_date, end_date in test_ranges:
                try:
                    ...
                    if data is not None and len(data) > 0:
                        ...
                        print(f"✅ {period_name:<10}: {record_count:4d} records ({actual_start} to {actual_end})")
                        
                        # Show sample of actual data
                        if period_name == "2 years":
                            print(f"   Sample prices: ${data['close'].iloc[0]:.2f} → ${data['close'].iloc[-1]:.2f}")
        ...
        if btc_2yr is not None and len(btc_2yr) > 0:
            ...
            print(f"   Actual range: {actual_start.strftime('%Y-%m-%d')} to {actual_end.strftime('%Y-%m-%d')}")
            ...
            for period in periods:
                try:
                    ...
                    if len(data) > 0:
                        ...
                        years = days / 365.25
                        print(f"   {period:<4}: {len(data):4d} records, {days:4d} days ({years:.2f} years)")
                    else:
...
def main():
    ...
    print(f"Our Client Test: {'✅ VERIFIED' if client_result else '❌ FAILED'} ({client_years:.2f} years)")
    print(f"Yahoo Direct Test: {'✅ VERIFIED' if yahoo_result else '❌ FAILED'} ({yahoo_years:.2f} years)")
