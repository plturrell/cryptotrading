...
@click.option('--types', help='Comma-separated feature types (price_features, volume_features, momentum_features)')
@click.pass_context
@async_command
...
async def importance(ctx, model):
    ...
    try:
        ...
        # Sort by importance
        sorted_features = sorted(result['importance_scores'].items(), 
