...
async def verify_agents():
    ...
    success = A2AAgentRegistry.register_agent(
        ...
        capabilities=A2A_CAPABILITIES.get("data_analysis_agent", [])
    )
    ...
    success = A2AAgentRegistry.register_agent(
        ...
        capabilities=A2A_CAPABILITIES.get("feature_store_agent", [])
    )
    ...
    for agent_id, info in all_agents.items():
        print(f"  ✓ {agent_id}")
        print(f"    Status: {info['status'].value if hasattr(info['status'], 'value') else info['status']}")
