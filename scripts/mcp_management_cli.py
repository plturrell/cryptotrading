...
async def index(ctx, unit, force):
    ...
    result = await client.call_tool(
        "glean_index_project", {"unit_name": unit, "force_reindex": force}
    )
...
async def search(ctx, pattern, ai_enhance, limit):
    ...
    result = await client.call_tool("glean_symbol_search", {"pattern": pattern, "limit": limit})
    ...
    if "content" in response:
        try:
            ...
            if ai_enhance and symbols:
                print(f"\n🤖 Enhancing with AI...")
                ai_result = await client.call_tool(
                    "ai_enhance_analysis",
                    {"analysis_data": data, "enhancement_type": "summary", "ai_provider": "grok"},
                )
                ...
                if "error" not in ai_result:
                    ...
                    if "content" in ai_response:
                        ...
                        if grok_response.get("status") == "success":
                            ...
                            print(
                                f"   ⚠️ AI analysis failed: {grok_response.get('error', 'Unknown error')}"
                            )
...
async def review(ctx, files, type):
    ...
    result = await client.call_tool("ai_code_review", {"files": list(files), "review_type": type})
...
async def explain(ctx, symbol, context):
    ...
    result = await client.call_tool("ai_explain_code", {"symbol": symbol, "context_level": context})
        ...
        if "content" in response:
            try:
                ...
                if grok_explanation.get("status") == "success":
                    ...
                    print(
                        f"❌ AI explanation failed: {grok_explanation.get('error', 'Unknown error')}"
                    )
            except Exception as e:
        ...
        try:
            ...
            if command == "quit":
                ...
                search_result = await client.call_tool(
                    "glean_symbol_search", {"pattern": pattern, "limit": 3}
                )
                if "error" not in search_result:
                    ...
                    ai_result = await client.call_tool(
                        "ai_enhance_analysis",
                        {"analysis_data": {"symbols": []}, "enhancement_type": "summary"},
                    )
...
async def demo(ctx):
    ...
    review_result = await client.call_tool(
        "ai_code_review",
        {"files": ["src/cryptotrading/core/agents/base.py"], "review_type": "comprehensive"},
    )
    print("   ✅ AI review completed")
