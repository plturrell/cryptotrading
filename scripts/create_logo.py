...
def create_logo(size=200):
    ...
    margin = 10
    draw.ellipse(
        [margin, margin, size - margin, size - margin], fill="#FF6B00", outline="#E55A00", width=3
    )
    ...
    draw.rectangle(
        [
            center - line_width // 2 - size // 6,
            center - size // 3,
            center - line_width // 2 - size // 6 + line_width,
            center + size // 3,
        ],
        fill="white",
    )
    ...
    # Top curve
    draw.arc(
        [center - size // 4, center - size // 4, center + size // 4, center],
        start=180,
        end=0,
        fill="white",
        width=line_width,
    ...
    # Bottom curve
    draw.arc(
        [center - size // 4, center, center + size // 4, center + size // 4],
        start=0,
        end=180,
        fill="white",
        width=line_width,
