    ...
    class FallbackDatabaseAgent:
        ...
        async def bulk_insert(self, table, records, batch_size=1000):
            ...
            return {
                ...
                "batches_processed": max(1, (len(records) if isinstance(records, list) else records) // batch_size),
                "success_rate": 0.98,
                "timestamp": datetime.now().isoformat()
            }
...
async def health(ctx):
    ...
    try:
        ...
        click.echo(f"Active Connections: {result['connections']['active']}/{result['connections']['max']}")
        click.echo(f"Storage: {result['storage']['used']} used, {result['storage']['available']} available")
