...
def setup_real_s3_test():
    ...
    try:
        ...
        upload_success = s3_service.upload_data(
            ...
            s3_key=test_key,
            metadata={"test": "real_integration", "timestamp": datetime.now().isoformat()},
...
def test_crypto_data_manager(s3_service):
    ...
    try:
        ...
        crypto_success = data_manager.save_ohlcv_data(
            symbol="BTC-USD", timeframe="1h", ohlcv_data=sample_ohlcv, timestamp=datetime.now()
        )
        ...
        portfolio = {
            ...
            "assets": [{"symbol": "BTC", "quantity": 2.5, "value_usd": 125000.00}],
        }

        portfolio_success = data_manager.save_user_portfolio(
            user_id="test_user", portfolio=portfolio, timestamp=datetime.now()
        )
        ...
        try:
            ...
            if stats:
                print(
                    f"  ✅ Storage stats: {stats.get('total_objects', 0)} objects, {stats.get('total_size_mb', 0)} MB"
                )
            else:
