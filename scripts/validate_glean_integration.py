...
from src.cryptotrading.infrastructure.analysis.glean_continuous_monitor import (
    glean_continuous_monitor_tool,
)
from src.cryptotrading.infrastructure.analysis.glean_zero_blindspots_mcp_tool import (
...
async def validate_glean_integration():
    ...
    try:
        ...
        monitor_result = await glean_continuous_monitor_tool(
            {"command": "status", "project_path": str(project_root)}
        )
        ...
        if monitor_result["success"]:
            ...
            # Test list sessions
            sessions_result = await glean_continuous_monitor_tool({"command": "list_sessions"})
