...
def load_2year_crypto_data():
    ...
    for i, symbol in enumerate(crypto_pairs, 1):
        ...
        try:
            ...
            if not hist_data.empty:
                ...
                print(
                    f"     💰 Price: ${first_price:.4f} → ${last_price:.4f} ({price_change:+.1f}%)"
                )
                print(f"     📊 Range: ${min_price:.4f} - ${max_price:.4f}")
                print(f"     📈 Avg Volume: {avg_volume:,.0f}")

            else:
                results[symbol] = {"success": False, "error": "No data returned", "records": 0}
    ...
    if successful_pairs:
        ...
        sorted_pairs = sorted(
            successful_pairs.items(), key=lambda x: x[1]["price_change_pct"], reverse=True
        )

        for i, (symbol, data) in enumerate(sorted_pairs[:3], 1):
            print(
                f"  {i}. {symbol}: {data['price_change_pct']:+.1f}% "
    ...
    for symbol, data in results.items():
        if data["success"]:
            ...
            print(
                f"  {symbol}: Completeness {quality['completeness']*100:.1f}%, "
