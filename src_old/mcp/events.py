...
class EventSubscription:
    ...
    def __init__(self, subscription_id: str, filter: EventFilter,
...
class EventBuffer:
    ...
    async def get_events(self, filter: EventFilter = None,
                        since: float = None, limit: int = 100) -> List[MCPEvent]:
...
class EventStreamer:
    ...
    async def publish_event(self, event: MCPEvent):
        ...
        delivery_tasks = [
            sub.deliver_event(event) for sub in subscriptions if sub.active
        ]
        ...
        mcp_metrics.collector.counter(
            "mcp.events.published",
            tags={"event_type": event.event_type.value}
        )
    
    async def subscribe(self, filter: EventFilter,
                       callback: Callable[[MCPEvent], None]) -> str:
        """Subscribe to events"""
        async with self._lock:
...
class MCPEventPublisher:
    ...
    async def publish_tool_execution(self, tool_name: str, arguments: Dict[str, Any],
    ...
    async def publish_trade_signal(self, signal_type: str, symbol: str,
    ...
    async def publish_error(self, error_type: str, message: str,
...
async def subscribe_to_events(event_types: List[EventType],
                             callback: Callable[[MCPEvent], None],
                             user_id: str = None, source: str = None) -> str:
    """Subscribe to events"""
...
async def get_recent_events(event_types: List[EventType] = None,
                           since: float = None, limit: int = 100) -> List[Dict[str, Any]]:
    """Get recent events"""
