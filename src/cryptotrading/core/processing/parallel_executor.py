...
from ..config.environment import get_feature_flags, is_vercel, is_serverless
...
class ParallelExecutor:
    ...
    def execute_sync_batch(self,
                          func: Callable,
                          items: List[Any],
                          use_threads: bool = True) -> List[ProcessingResult]:
    ...
    def _execute_with_threads(self, func: Callable, items: List[Any]) -> List[ProcessingResult]:
        ...
        for i in range(0, len(items), batch_size):
            ...
            try:
                future_to_item = {
                    self._thread_pool.submit(func, item): (j, item)
...
class OptimizedFactorComputer:
    ...
    async def compute_factors_parallel(self,
                                     symbols: List[str],
        ...
        for i, result in enumerate(results):
            if i < len(symbol_factor_map):
                ...
                if result.success:
                    ...
                    logger.error(f"Factor computation failed for {symbol}.{factor_name}: {result.error}")
