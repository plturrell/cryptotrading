...
class GrokClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError("Grok API key required. Set GROK_API_KEY environment variable or pass api_key parameter.")
    ...
    async def analyze_code_structure(self, glean_data: Dict[str, Any], focus: str = "architecture") -> Dict[str, Any]:
...
Keep the analysis concise but insightful."""
    ...
    async def generate_code_review(self, files_data: List[Dict[str, Any]], review_type: str = "comprehensive") -> Dict[str, Any]:
...
Format as structured feedback that can guide development decisions."""
    ...
    async def explain_code_component(self, symbol_data: Dict[str, Any], context_level: str = "detailed") -> Dict[str, Any]:
...
Please explain this component clearly and helpfully."""
    ...
    async def generate_refactoring_suggestions(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "choices" in response and response["choices"]:
                ...
                return {
                    "status": "success",
                    "suggestions": content,
                    "issues_analyzed": len(complexity_issues) + len(coupling_issues) + len(code_smells),
                    "model": response.get("model", "grok-4-latest"),
                    "usage": response.get("usage", {}),
                    "timestamp": datetime.now().isoformat()
                }
...
async def test_grok_integration():
    ...
    try:
        async with GrokClient() as grok:
            # Test simple chat
            test_messages = [
                {"role": "user", "content": "Explain what makes good software architecture in one sentence."}
            ]
