...
class CircuitBreaker:
    ...
    async def _record_failure(self, error: Exception, execution_time: float):
        ...
        with self._lock:
            ...
            # Log failure details
            logger.error(f"Circuit breaker '{self.name}' failure: {error} (took {execution_time:.2f}s)")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'next_attempt_time': self._next_attempt_time if self.state == CircuitBreakerState.OPEN else None
            }
    ...
    async def retry(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(self.max_attempts):
            try:
                ...
                logger.warning(f"Attempt {attempt + 1}/{self.max_attempts} failed: {e}. Retrying in {delay:.2f}s")
...
class FallbackManager:
    ...
    async def execute_with_fallback(self, service_name: str, primary_func: Callable, 
        ...
        try:
            ...
            raise FallbackExhaustedError(f"No working fallback for '{service_name}'") from primary_error
...
class ErrorRecoverySystem:
    ...
    def get_circuit_breaker(self, name: str, config: Optional[RecoveryConfig] = None) -> CircuitBreaker:
    ...
    async def execute_with_recovery(self, service_name: str, func: Callable, 
        ...
        try:
            if use_circuit_breaker:
                ...
                if use_retry:
                    ...
                    return await self.retry_manager.retry(circuit_breaker.call, func, *args, **kwargs)
                else:
            ...
            return await self.fallback_manager.execute_with_fallback(service_name, func, *args, **kwargs)
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        health['overall_health'] = (
            'healthy' if health['failed_services'] == 0 else
            'degraded' if health['healthy_services'] > 0 else
            'critical'
        )
