...
from ..interfaces import (
    ...
    IServiceRegistry, ICache, ITaskScheduler, IResourceManager,
    ILockManager, IFileStorage, ISecurityManager, IAuthenticator,
    IPermissionChecker, IRateLimiter, IInputValidator, ICryptoProvider,
    ISecurityAuditor, ICommunicationManager, IServiceDiscovery,
    ILoadBalancer, ICircuitBreaker, IEventBus,
    SecurityLevel, ServiceStatus, MessageType, MessagePriority
)
    ...
    async def list_services(self, service_type: str = None) -> List[Dict[str, Any]]:
        ...
        async with self._lock:
            ...
            if service_type:
                services = [
                    s for s in services 
                    if s.get("metadata", {}).get("service_type") == service_type
                ]
