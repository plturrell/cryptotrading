...
class UnifiedServiceDiscovery:
    ...
    async def start(self):
        ...
        tasks = [self._periodic_discovery(), self._health_monitor(), self._cache_manager()]
    ...
    async def register_service(self, endpoint: ServiceEndpoint) -> bool:
        ...
        try:
            ...
            if overall_success:
                self.logger.info(
                    f"Registered service {endpoint.service_id} ({endpoint.service_type.value})"
                )
                await self._save_service_cache()
    ...
    async def discover_services(
        ...
        for method in self.active_methods:
            try:
                ...
                for service in method_services:
                    ...
                    if (
                        service.service_id not in all_services
                        or service.last_seen > all_services[service.service_id].last_seen
                    ):
        ...
        self.logger.info(
            f"Discovered {len(filtered_services)} services (type={service_type}, capability={capability})"
        )
    ...
    async def _register_with_method(
        self, endpoint: ServiceEndpoint, method: DiscoveryMethod
    ) -> bool:
    ...
    async def _discover_with_method(
        self,
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{kv_url}/set/{service_key}", headers=headers, json=service_data
                ) as response:
    ...
    async def _discover_vercel_kv(
        self, service_type: Optional[ServiceType], capability: Optional[str]
    ) -> List[ServiceEndpoint]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{kv_url}/scan/0", headers=headers, params={"match": pattern, "count": 100}
                ) as response:
                    ...
                    for key in keys:
                        # Get service data
                        async with session.get(
                            f"{kv_url}/get/{key}", headers=headers
                        ) as get_response:
                            if get_response.status == 200:
                                service_data = await get_response.json()
                                service_data["last_seen"] = datetime.fromisoformat(
                                    service_data["last_seen"]
                                )
    ...
    async def _discover_health_probe(
        self, service_type: Optional[ServiceType], capability: Optional[str]
    ) -> List[ServiceEndpoint]:
                                ...
                                try:
                                    ...
                                    service = ServiceEndpoint(
                                        ...
                                        description=data.get(
                                            "description", "Health probe discovered"
                                        ),
                                        host="localhost",
                                        port=port,
                                        protocol="http",
    ...
    async def _save_service_cache(self):
        ...
        try:
            ...
            cache_data = {"services": [], "last_updated": datetime.utcnow().isoformat()}
            ...
            while time.time() - start_time < 3.0:
                try:
                    ...
                    if response.get("type") == "service_response":
                        ...
                        service = ServiceEndpoint(
                            service_id=response.get(
                                "service_id", f"multicast-{addr[0]}:{response.get('port', 8080)}"
                            ),
    ...
    async def _register_static_config(self, endpoint: ServiceEndpoint) -> bool:
        ...
        try:
            ...
            config["services"] = [
                s for s in config["services"] if s.get("service_id") != endpoint.service_id
            ]
        ...
        if hasattr(self, "blockchain_client") and self.blockchain_client:
            try:
                ...
                tx = {
                    ...
                    "nonce": self.blockchain_client.w3.eth.get_transaction_count(
                        self.blockchain_client.address
                    ),
                }

                signed_tx = self.blockchain_client.account.sign_transaction(tx)
                tx_hash = self.blockchain_client.w3.eth.send_raw_transaction(
                    signed_tx.rawTransaction
                )
                receipt = self.blockchain_client.w3.eth.wait_for_transaction_receipt(tx_hash)

                self.logger.info(
                    f"Unregistered {endpoint.agent_id} from blockchain, tx: {tx_hash.hex()}"
                )
        ...
        try:
            ...
            try:
                ...
                v1.create_namespaced_service(
                    namespace=os.getenv("K8S_NAMESPACE", "default"), body=service_manifest
                )
                return True
    ...
    async def _register_docker_dns(self, endpoint: ServiceEndpoint) -> bool:
        ...
        try:
            ...
            if compose_services_dir.exists():
                ...
                self.logger.info(
                    f"Registered {endpoint.service_id} in Docker Compose shared volume"
                )
                return True
            else:
                            ...
                            async with aiofiles.open(service_file, "r") as f:
                                ...
                                if self._is_service_valid(service):
                                    if self._matches_filters(
                                        service, service_type, capability, None
                                    ):
                                        services.append(service)
                        except Exception as e:
                            self.logger.debug(f"Failed to load service from {service_file}: {e}")
        ...
        for port in mcp_ports:
            for path in mcp_paths:
                try:
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            f"{url}/info", timeout=aiohttp.ClientTimeout(total=2)
                        ) as response:
                            if response.status == 200:
                                server_info = await response.json()
                                servers.append(
                                    {"url": url, "port": port, "path": path, "info": server_info}
                                )
    ...
    async def _query_mcp_server_skills(
        self, endpoint_info: Dict[str, Any], capability: Optional[str]
    ) -> List[ServiceEndpoint]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                # Get tools list
                async with session.get(
                    f"{base_url}/tools", timeout=aiohttp.ClientTimeout(total=5)
                ) as response:
                    if response.status == 200:
                        ...
                        for tool in tools_data.get("tools", []):
                            ...
                            service_capability = ServiceCapability(
                                ...
                                input_types=tool.get("inputSchema", {})
                                .get("properties", {})
                                .keys(),
                    ...
                    for skill_config in config.get("skills", []):
                        ...
                        service = ServiceEndpoint(
                            ...
                            description=skill_config.get(
                                "description", f"Static MCP skill: {skill_name}"
                            ),
                                ...
                                try:
                                    ...
                                    async with session.get(
                                        ...
                                        if get_response.status == 200:
                                            ...
                                            if (
                                                capability
                                                and capability.lower() not in skill_name.lower()
                                            ):
                                                continue
                                            ...
                                            # Create service endpoint
                                            service = ServiceEndpoint(
                                                service_id=skill_data.get(
                                                    "service_id", f"cloud-mcp-{skill_name}"
                                                ),
                                                service_type=ServiceType.MCP_SKILL,
                                                agent_id=skill_data.get("agent_id", "cloud"),
                                                name=skill_data.get(
                                                    "name", f"Cloud MCP {skill_name}"
                                                ),
                                                description=skill_data.get(
                                                    "description", f"Cloud MCP skill: {skill_name}"
                                                ),
                                    ...
                                    self.logger.debug(
                                        f"Failed to process cloud MCP skill {key}: {e}"
                                    )
