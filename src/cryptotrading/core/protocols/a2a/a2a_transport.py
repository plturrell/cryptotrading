...
from .a2a_protocol import A2AMessage, MessageType, AgentStatus
try:
...
class A2ATransportManager:
    ...
    def __init__(self, agent_id: str, host: str = "localhost", port: int = None, 
                 use_blockchain: bool = True, anvil_url: str = "http://localhost:8545"):
        ...
        try:
            ...
            if self.blockchain_client:
                ...
                if blockchain_success:
                    ...
                    self.local_logger.warning("Blockchain initialization failed, falling back to TCP only")
    ...
    async def _handle_handshake(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> Optional[str]:
    ...
    async def _process_connection_messages(self, agent_id: str, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        while not self._shutdown:
            try:
                ...
                if message_dict.get("type") == "keepalive":
                    await self._send_raw_message(writer, json.dumps({"type": "keepalive_ack"}).encode('utf-8'))
                    continue
    ...
    async def _process_message(self, message: A2AMessage, response_writer: asyncio.StreamWriter = None):
    ...
    async def send_message(self, receiver_id: str, message_type: MessageType, 
                          ...
                          correlation_id: str = None, workflow_context: Dict[str, Any] = None) -> str:
        ...
        try:
            ...
            if service_endpoint:
                if service_endpoint.protocol == "blockchain":
                    ...
                    if self.blockchain_client:
                        tx_hash = await self.blockchain_client.send_message(
                            receiver_id, message.message_type.value, 
                            message.payload, message.message_id
                        )
    ...
    async def connect_to_agent(self, host: str, port: int) -> Optional[str]:
        ...
        try:
            ...
            if response.get("type") == "a2a_handshake_ack":
                ...
                asyncio.create_task(self._process_connection_messages(remote_agent_id, reader, writer))
    ...
    async def _handle_workflow_status(self, message: A2AMessage) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": self.status.value,
            "workflow_engine_available": hasattr(self, 'workflow_engine') and self.workflow_engine is not None
        }
            ...
            try:
                if self.service_registry:
                    ...
                    for service in services:
                        ...
                        if agent_id and agent_id != self.agent_id and agent_id not in self.agent_registry:
                            ...
                            if host and port:
                                ...
                                if connected_agent:
                                    self.local_logger.info(f"Discovered and connected to agent: {connected_agent}")
    ...
    async def _handle_blockchain_workflow_request(self, message):
        ...
        if isinstance(message, BlockchainMessage):
            ...
            if MessageType.WORKFLOW_REQUEST in self.message_handlers:
                try:
                    response = await self.message_handlers[MessageType.WORKFLOW_REQUEST](a2a_message)
    ...
    def get_blockchain_status(self) -> Dict[str, Any]:
        ...
        return {
            "enabled": True,
            "status": self.blockchain_client.get_status()
        }