...
from ...infrastructure.mcp.technical_analysis_skills_mcp_agent import TechnicalAnalysisSkillsMCPAgent
...
class A2AAgentRegistryIntegration:
    ...
    def _register_agent_with_a2a(self, agent_id: str, agent_instance: Any) -> bool:
        ...
        try:
            ...
            logger.info(f"Agent {agent_id} registered with A2A protocol with capabilities: {capabilities}")
            return True
    ...
    async def route_message_to_agents(self, message_type: MessageType, message: Dict[str, Any]) -> List[Dict[str, Any]]:
