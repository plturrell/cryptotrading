...
class ServiceDiscovery:
    ...
    async def start(self):
        ...
        tasks = [self._periodic_discovery(), self._health_checker(), self._cache_cleaner()]
        ...
        try:
            ...
            if self.blockchain_client:
                success = await self.blockchain_client.register_agent(
                    service_id, endpoint.capabilities, f"{endpoint.host}:{endpoint.port}"
                )
                if not success:
    ...
    async def discover_services(self, capability: Optional[str] = None) -> List[ServiceEndpoint]:
        ...
        self.logger.info(
            f"Discovered {len(discovered_services)} services for capability '{capability}'"
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_services": len(
                [s for s in self.service_cache.values() if s.status == ServiceStatus.ACTIVE]
            ),
        }
