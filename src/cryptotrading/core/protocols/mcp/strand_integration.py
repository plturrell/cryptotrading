...
class MCPStrandBridge:
    ...
    def _create_strand_tool_wrapper(self, mcp_tool: MCPTool, agent: StrandsAgent):
        ...
        def mcp_tool_wrapper(**kwargs) -> str:
            ...
            try:
                ...
                # Execute MCP tool
                result = asyncio.run(
                    self._execute_mcp_tool_async(mcp_tool.name, kwargs, auth_context)
                )
                ...
                if self.config.event_streaming:
                    asyncio.run(
                        self.event_publisher.publish_tool_execution(
    ...
    async def _execute_mcp_tool_async(
        self, tool_name: str, arguments: Dict[str, Any], auth_context: AuthContext = None
    ) -> ToolResult:
    ...
    async def orchestrate_workflow(
        self, workflow_config: Dict[str, Any], auth_context: AuthContext = None
    ) -> Dict[str, Any]:
        ...
        results = {"workflow_id": workflow_id, "steps": [], "success": True, "error": None}
        ...
        try:
            for i, step in enumerate(steps):
                ...
                if step_type == "mcp_tool":
                    ...
                    step_result = await self._execute_strand_workflow_step(
                        step_config, auth_context
                    )
                ...
                results["steps"].append(
                    {"step_number": i + 1, "step_type": step_type, "result": step_result}
                )

                # Stop on first failure if not configured to continue
                if not step_result.get("success", False) and not step_config.get(
    ...
    async def _execute_mcp_workflow_step(
        self, config: Dict[str, Any], auth_context: AuthContext = None
    ) -> Dict[str, Any]:
    ...
    async def _execute_strand_workflow_step(
        self, config: Dict[str, Any], auth_context: AuthContext = None
    ) -> Dict[str, Any]:
    ...
    async def _execute_data_pipeline_step(
        self, config: Dict[str, Any], auth_context: AuthContext = None
    ) -> Dict[str, Any]:
    ...
    async def _execute_historical_data_pipeline(
        self, config: Dict[str, Any], auth_context: AuthContext = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            data_result = await self._execute_mcp_tool_async(
                "load_historical_data", {"symbols": symbols, "start_date": start_date}, auth_context
            )
    ...
    async def _execute_market_analysis_pipeline(
        self, config: Dict[str, Any], auth_context: AuthContext = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            market_data_result = await self._execute_mcp_tool_async(
                "get_market_data", {"symbol": symbol}, auth_context
            )

            if not market_data_result.is_success:
                return {
                    "success": False,
                    "error": f"Market data failed: {market_data_result.content}",
                }

            # Step 2: Technical analysis via MCP plugin
            tech_analysis_result = await self._execute_mcp_tool_async(
                "technical_analysis", {"symbol": symbol, "timeframe": "1h"}, auth_context
            )
            ...
            return {
                ...
                "data": {
                    ...
                    "technical_analysis": tech_analysis_result.to_dict()
                    if tech_analysis_result.is_success
                    else None,
    ...
    async def _execute_portfolio_sync_pipeline(
        self, config: Dict[str, Any], auth_context: AuthContext = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not portfolio_result.is_success:
                return {
                    "success": False,
                    "error": f"Portfolio sync failed: {portfolio_result.content}",
                }
...
def setup_mcp_strand_integration(
    mcp_server: MCPServer = None, mcp_client: MCPClient = None, config: MCPStrandConfig = None
) -> MCPStrandBridge:
...
async def execute_mcp_strand_workflow(
    workflow_config: Dict[str, Any], auth_context: AuthContext = None
) -> Dict[str, Any]:
