    ...
    parameters = {
        ...
        "input_data": {
            ...
            "properties": {
                ...
                "stream_id": {"type": "string", "description": "Stream ID for continuous inference"}
            }
        },
        "inference_config": {
            "type": "object",
...
class TrainingJobsResource(Resource):
    ...
    async def read(self) -> str:
        ...
        import json
        status = {
            "active_jobs": len([j for j in self.jobs.values() if j["status"] == TrainingStatus.TRAINING.value]),
            "completed_jobs": len([j for j in self.jobs.values() if j["status"] == TrainingStatus.COMPLETED.value]),
            "jobs": self.jobs
        }
