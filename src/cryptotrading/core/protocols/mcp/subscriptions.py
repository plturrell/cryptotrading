...
class ResourceUpdate:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "params": {"uri": self.uri, "timestamp": self.timestamp.isoformat()},
        }
            ...
            try:
                if os.path.exists(self.file_path):
                    ...
                    if self._last_modified is None:
                        ...
                        update = ResourceUpdate(uri=self.uri, content=content)
                ...
                update = ResourceUpdate(uri=self.uri, error=str(e))
                self._notify_update(update)
                ...
                async with session.get(self.api_url, headers=self.headers) as response:
                    if response.status == 200:
                        if response.headers.get("Content-Type", "").startswith("application/json"):
                            ...
                            return {"type": "json", "data": data, "headers": dict(response.headers)}
                        else:
                            text = await response.text()
                            return {"type": "text", "text": text, "headers": dict(response.headers)}
        except Exception as e:
...
class SubscriptionManager:
    ...
    async def subscribe(self, uri: str, subscriber_id: str) -> ResourceSubscription:
        ...
        subscription = ResourceSubscription(
            uri=uri, subscriber_id=subscriber_id, created_at=datetime.utcnow()
        )
