...
class TenantConfig:
    ...
    def __post_init__(self):
        ...
        if self.api_rate_limits is None:
            self.api_rate_limits = {"requests_per_minute": 60, "requests_per_hour": 1000}
...
class TenantManager:
    ...
    def _load_default_tenants(self):
        ...
        self.tenants["demo"] = TenantConfig(
            ...
            api_rate_limits={"requests_per_minute": 30, "requests_per_hour": 500},
        )
        ...
        self.tenants["premium"] = TenantConfig(
            ...
            api_rate_limits={"requests_per_minute": 120, "requests_per_hour": 5000},
        )
...
class TenantIsolation:
    ...
    def set_tenant_data(
        self, tenant_id: str, data_type: str, key: str, value: Any, ttl: int = 300
    ) -> None:
    ...
    def check_trading_permission(
        self, auth_context: AuthContext, operation: str, symbol: str = None
    ) -> Dict[str, Any]:
        ...
        if not tenant.trading_enabled:
            return {"allowed": False, "reason": "Trading is disabled for this tenant"}

        # Check symbol restrictions
        if symbol and symbol not in tenant.allowed_symbols:
            return {"allowed": False, "reason": f"Symbol {symbol} not allowed for this tenant"}
        ...
        return {"allowed": True, "remaining_trades": tenant.max_daily_trades - daily_trades}

    def record_trading_operation(
        self, auth_context: AuthContext, operation: str, symbol: str, amount: float
    ):
        ...
        self.set_tenant_data(
            auth_context.tenant_id, "counters", trades_key, current_trades + 1, 86400
        )
        ...
        mcp_metrics.collector.counter(
            ...
            tags={"tenant": auth_context.tenant_id, "operation": operation, "symbol": symbol},
        )

    def check_position_limits(
        self, auth_context: AuthContext, symbol: str, amount: float
    ) -> Dict[str, Any]:
    ...
    def update_tenant_portfolio(self, auth_context: AuthContext, portfolio_data: Dict[str, Any]):
...
class TenantAwareTools:
    ...
    async def get_portfolio(
        self, auth_context: AuthContext, include_history: bool = False
    ) -> Dict[str, Any]:
    ...
    async def get_risk_metrics(
        self, auth_context: AuthContext, scope: str = "portfolio"
    ) -> Dict[str, Any]:
...
def check_tenant_permission(
    auth_context: AuthContext, operation: str, symbol: str = None
) -> Dict[str, Any]:
