...
from .tools import MCPTool, ToolContent, ToolResult
...
class StrandsGleanMCPServer(MCPServer):
    ...
    async def _index_project(
        self, unit_name: str = "main", force_reindex: bool = False
    ) -> ToolResult:
        ...
        try:
            ...
            if unit_name in self.indexed_units and not force_reindex:
                return ToolResult.text_result(
                    f"Unit '{unit_name}' already indexed. Use force_reindex=true to reindex."
                )
            ...
            if result.get("status") == "success":
                ...
                return ToolResult.data_result(json.dumps(response, indent=2), "application/json")
            else:
                return ToolResult.text_result(
                    f"Indexing failed: {result.get('error', 'Unknown error')}", is_error=True
                )
    ...
    async def _enhance_with_ai(
        ...
        try:
            ...
            return ToolResult.data_result(json.dumps(enhanced_result, indent=2), "application/json")
    ...
    async def _ai_code_review(
        self, files: List[str], review_type: str = "comprehensive"
    ) -> ToolResult:
        ...
        try:
            ...
            return ToolResult.data_result(json.dumps(review_result, indent=2), "application/json")
    ...
    async def _ai_explain_code(self, symbol: str, context_level: str = "moderate") -> ToolResult:
        ...
        try:
            ...
            return ToolResult.data_result(json.dumps(explanation, indent=2), "application/json")
    ...
    def _generate_ai_insights(
        self, analysis_data: Dict[str, Any], enhancement_type: str
    ) -> Dict[str, Any]:
...
async def create_strands_glean_mcp_server(
    project_root: str = None, transport_type: str = "stdio"
