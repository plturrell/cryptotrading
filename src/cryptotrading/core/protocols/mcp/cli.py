...
class MCPDiagnosticCLI:
    ...
    async def _add_diagnostic_tools(self):
        ...
        async def performance_metrics():
            return {
                "response_times": {"avg_ms": 125, "p95_ms": 250, "p99_ms": 500},
                "throughput": {"requests_per_sec": 150, "peak_rps": 300},
                "resource_usage": {"cpu_cores": 4, "memory_gb": 8, "disk_io_mb_per_sec": 50},
            }
    ...
    def _add_benchmark_tools(self, server: MCPServer):
        ...
        compute_tool = MCPTool(
            "compute", "Computation test tool", {"iterations": {"type": "integer"}}, compute_test
        )
    ...
    async def _run_benchmarks(self, server: MCPServer, client: MCPClient, args) -> Dict[str, Any]:
        ...
        results["tool_calls"] = {"duration": duration, "ops_per_sec": iterations / duration}
...
def main():
    ...
    server_parser.add_argument(
        "--transport", choices=["stdio", "websocket", "sse"], default="stdio", help="Transport type"
    )
    ...
    diag_parser.add_argument(
        "diagnostic_type",
        choices=["server", "client", "transport", "tools", "all"],
