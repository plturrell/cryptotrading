...
class MCPClient:
                ...
                if parsed.id in self.pending_requests:
                    ...
                    if parsed.error:
                        future.set_exception(
                            Exception(f"MCP Error {parsed.error.code}: {parsed.error.message}")
                        )
                    else:
    ...
    async def _send_request(
        self, method: str, params: Optional[Dict[str, Any]] = None, request_id: Optional[str] = None
    ) -> Any:
    ...
    async def initialize(self):
        ...
        logger.info(
            f"MCP client initialized with server: {self.server_info.get('name', 'Unknown')}"
        )
