...
from .security.middleware import SecurityConfig, create_secure_middleware
...
class MCPQuickStart:
    ...
    def _create_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "transport": {"type": "stdio", "host": "localhost", "port": 8080},  # Safe default
            "security": {
                "enabled": True,
            ...
            "development": {"auto_reload": True, "debug_logging": True, "cors_enabled": True},
        }
    ...
    def add_directory_root(self, path: str, name: str = None) -> "MCPQuickStart":
        ...
        self.config["roots"].append({"path": abs_path, "name": name or os.path.basename(abs_path)})
        return self
    ...
    def _setup_roots(self):
        ...
        for root_config in self.config.get("roots", []):
            self.server.add_root_directory(root_config["path"], root_config["name"])
...
class MCPTemplates:
    ...
    def file_server(name: str = "file-mcp", root_dir: str = ".") -> MCPQuickStart:
        ...
        return (
            ...
            .with_features(subscriptions=True)
        )  # For file watching
...
async def run_mcp_server(name: str = "quick-mcp", tools: List[Dict[str, Any]] = None, **kwargs):
...
async def demo_simple_server():
    ...
    server = (
        MCPQuickStart("demo-server")
        .add_simple_tool("echo", "Echo a message", echo_tool, {"message": {"type": "string"}})
        .add_simple_tool(
            "calc", "Calculate expression", calc_tool, {"expression": {"type": "string"}}
        )
        .with_transport("http", port=8080)
        .with_security(enabled=True, require_auth=False)
    )
...
async def demo_file_server():
    ...
    server = (
        MCPTemplates.file_server("demo-files")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--template", choices=["dev", "prod", "api", "file"], help="Use predefined template"
    )
