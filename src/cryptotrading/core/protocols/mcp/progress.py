    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if self.sub_operations:
            status["subOperations"] = {
                ...
                for name, tracker in self.sub_operations.items()
            }
        
        percentage = ProgressUpdate(
            self.token, self.current_progress, self.total
        ).percentage
...
class ProgressManager:
    ...
    def list_trackers(
        ...
        include_sub_operations: bool = False
    ) -> List[Dict[str, Any]]:
        """List all trackers"""
        ...
        while True:
            try:
                ...
                for token, tracker in self.trackers.items():
                    if (tracker.state in completed_states and
...
class progress_tracker:
    ...
    def __enter__(self) -> ProgressTracker:
        self.tracker = progress_manager.create_tracker(
            self.total,
            self.description,
            self.token
        )
