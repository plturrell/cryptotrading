...
class SecurityValidator:
    ...
    def validate_string_security(
        self, value: str, context: str = "general"
    ) -> Tuple[bool, List[str]]:
        """
...
class SchemaValidator:
    ...
    def _validate_number(
        self, data: Union[int, float], schema: Dict[str, Any], path: str
    ) -> List[str]:
        """Validate number"""
        errors = []
        ...
        if isinstance(additional_schema, dict):
            for prop, value in data.items():
                if prop not in properties:
                    prop_path = f"{path}.{prop}" if path else prop
                    is_valid, prop_errors = self.validate_schema(
                        value, additional_schema, prop_path
                    )
                    errors.extend(prop_errors)
    ...
    def _load_mcp_schemas(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "resources/read": {
                ...
                "required": ["uri"],
                "properties": {
                    "uri": {"type": "string", "maxLength": 500, "x-security-context": "path"}
                },
            },
        }
        ...
        if isinstance(params, dict):
            ...
            for key, value in params.items():
                ...
                if isinstance(key, str):
                    ...
                    sanitized[clean_key] = self._sanitize_params(
    ...
    def validate_file_upload(
        ...
        if filename.lower().endswith((".txt", ".json", ".xml", ".html", ".css", ".js")):
            try:
                ...
                is_safe, threats = self.security_validator.validate_string_security(
