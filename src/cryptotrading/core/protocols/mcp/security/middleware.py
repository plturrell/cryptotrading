...
from .permissions import MethodSecurityLevel, get_permission_validator, validate_method_permission
from .rate_limit_headers import RateLimitInfo, get_multi_tier_manager, get_rate_limit_header_manager
from .rate_limiter import RateLimitExceeded, RateLimitMiddleware, create_rate_limit_middleware
from .secure_defaults import (
    get_secure_config_manager,
    get_security_defaults,
    validate_security_config,
)
from .validation import ValidationError, ValidationMiddleware, create_validation_middleware
...
class SecurityMetrics:
    ...
    def record_request(self, method: str, auth_method: Optional[str] = None):
        ...
        if auth_method:
            ...
            self.metrics["auth_methods"][auth_method] = (
                self.metrics["auth_methods"].get(auth_method, 0) + 1
            )
...
class SecurityMiddleware:
    ...
    def __init__(self, config: SecurityConfig):
        ...
        self.rate_limit_middleware = (
            create_rate_limit_middleware() if config.rate_limiting_enabled else None
        )
        self.validation_middleware = (
            create_validation_middleware(config.strict_validation)
    ...
    ) -> Tuple[Dict[str, Any], SecurityContext]:
        """
            ...
            if self.config.rate_limiting_enabled and self.rate_limit_middleware:
                ...
                try:
                    ...
                    if hasattr(self.rate_limit_middleware, "get_rate_limit_status"):
                        ...
                        rate_limit_info = header_manager.create_from_rate_limiter_status(
                            rate_status
                        )
    ...
    def _validate_method_permission(
    ...
    def _log_request_structured(
        ...
        security_context = extract_security_context_from_headers(
            headers, method=context.method, user_id=context.user_id
        )
        ...
        if status == "success":
            ...
            audit_logger.log_rate_limit_exceeded(security_context, "method_rate_limit")
        elif status == "validation_failed":
            audit_logger.log_input_validation_failure(security_context, context.validation_errors)
        elif status == "security_violation":
            audit_logger.log_security_threat(
                security_context, "security_policy_violation", error or "Unknown security violation"
            )
        elif context.security_threats:
            audit_logger.log_security_threat(
                security_context, "threat_detected", "; ".join(context.security_threats)
            )
        else:
            audit_logger.log_method_execution(
                security_context,
    ...
    async def get_security_status(self) -> Dict[str, Any]:
        ...
        if self.rate_limit_middleware:
            try:
                status[
                    "rate_limits"
                ] = await self.rate_limit_middleware.rate_limiter.get_rate_limit_status(
                    "global", "global"
                )
            except Exception as e:
...
class VercelSecurityMiddleware(SecurityMiddleware):
    ...
    def _load_vercel_config(self) -> SecurityConfig:
        ...
        logger.info(
            "Loaded Vercel security configuration",
            extra={
...
class SecureMiddleware(SecurityMiddleware):
    ...
    def __init__(self, config: SecurityConfig, jwt_secret: str, enable_strict_mode: bool = True):
        ...
        self.secure_authenticator = SecureAuthenticator(
            secret_key=jwt_secret, enable_strict_mode=enable_strict_mode
        )
        ...
        logger.info(
            "Secure middleware initialized with no authentication bypass",
            extra={"strict_mode": enable_strict_mode, "authenticated_only": True},
        )
    ...
    async def process_request(
        ...
        try:
            ...
            auth_context = await self.secure_authenticator.authenticate_request(
                headers, ip_address=headers.get("x-forwarded-for") or headers.get("x-real-ip")
            )
            ...
            if not permission_result["allowed"]:
                context.add_security_threat(
                    f"permission_denied_for_{method}_{permission_result['reason']}"
                )
                self.metrics.record_security_threat()
                raise AuthorizationError(
                    f"Permission denied for {method}: {permission_result['reason']}"
                )
    ...
    async def _secure_validate_params(
        self, method: str, params: Dict[str, Any], context: SecurityContext
    ) -> Dict[str, Any]:
        """
