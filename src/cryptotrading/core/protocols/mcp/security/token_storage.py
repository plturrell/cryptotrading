...
class RedisTokenStorage(TokenStorage):
        ...
        if self.redis_client is None:
            try:
                ...
                self._connection_pool = redis.ConnectionPool.from_url(
                    self.redis_url, max_connections=10, retry_on_timeout=True, decode_responses=True
                )
    ...
    async def revoke_token(self, token_hash: str, expires_at: Optional[datetime] = None) -> bool:
        ...
        try:
            ...
            await redis_client.setex(key, ttl_seconds, json.dumps(revocation_data))
...
class DatabaseTokenStorage(TokenStorage):
    ...
    async def revoke_token(self, token_hash: str, expires_at: Optional[datetime] = None) -> bool:
        ...
        try:
            metadata = {"revoked_by": "mcp_security_system", "reason": "manual_revocation"}
            ...
            with self.db.get_connection() as conn:
                ...
                cursor.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                """,
                    (token_hash, datetime.utcnow(), expires_at, json.dumps(metadata)),
                )
                conn.commit()
    ...
    async def is_token_revoked(self, token_hash: str) -> bool:
        ...
        try:
            ...
            result = await self.db_connection.fetch_one(query_sql, (token_hash, datetime.utcnow()))
    ...
    async def cleanup_expired_tokens(self) -> int:
        ...
        try:
            ...
            result = await self.db_connection.execute(delete_sql, (datetime.utcnow(),))
    ...
    async def get_revoked_count(self) -> int:
        ...
        try:
            ...
            result = await self.db_connection.fetch_one(query_sql, (datetime.utcnow(),))
...
class InMemoryTokenStorage(TokenStorage):
    ...
    async def cleanup_expired_tokens(self) -> int:
        ...
        async with self._lock:
            ...
            expired_tokens = [
                token_hash
                for token_hash, data in self._revoked_tokens.items()
