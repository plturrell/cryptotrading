...
from .authentication import Permission, AuthenticationContext
...
class SecureMethodRegistry:
    ...
    def _load_secure_defaults(self):
        ...
        self.register_method(MethodPermissionConfig(
            method="initialize",
...
class PermissionValidator:
    ...
    def validate_method_access(self, method: str, auth_context: Optional[AuthenticationContext]) -> Dict[str, any]:
        """
        ...
        if config.required_permissions:
            ...
            if missing_permissions:
                ...
                logger.warning(f"Access denied - insufficient permissions for {method}: {missing_permissions}")
                return result
    ...
    def get_user_accessible_methods(self, auth_context: Optional[AuthenticationContext]) -> List[str]:
...
def validate_method_permission(method: str, auth_context: Optional[AuthenticationContext]) -> Dict[str, any]:
...
def register_custom_method(method: str, security_level: MethodSecurityLevel, 
                          required_permissions: List[Permission], **kwargs):
...
def require_permissions(*permissions: Permission):
    ...
    def decorator(func):
        async def wrapper(self, auth_context: Optional[AuthenticationContext], *args, **kwargs):
            ...
            if missing_perms:
                raise PermissionError(f"Missing required permissions: {[p.value for p in missing_perms]}")
