        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.patch(
                    f"{self.base_url}/items", headers=headers, json=data
                ) as response:
...
class RateLimiter:
                ...
                for config_data in configs:
                    config = RateLimitConfig(
                        ...
                        algorithm=RateLimitAlgorithm(
                            config_data.get("algorithm", "sliding_window")
                        ),
    ...
    def add_config(self, config: RateLimitConfig):
        ...
        logger.info(
            f"Added rate limit config: {config.name} - {config.requests_per_window}/{config.window_seconds}s"
        )
    ...
    async def _get_state(self, cache_key: str) -> RateLimitState:
        ...
        try:
            ...
            if data:
                ...
                # Create new state
                state = RateLimitState(
                    requests=[], tokens=0, last_refill=time.time(), window_start=time.time()
                )
        except Exception as e:
    ...
    async def check_rate_limit(
        self, identifier: str, config_name: str = "global"
    ) -> Tuple[bool, int]:
        """
    ...
    async def get_rate_limit_status(
        ...
        if config.algorithm == RateLimitAlgorithm.SLIDING_WINDOW:
            ...
            reset_time = now + (config.burst_limit - current_tokens) / (
...
class RateLimitMiddleware:
    ...
    async def check_request_limits(
        self, identifier: str, method: str, api_key_id: Optional[str] = None
    ) -> None:
        """
        ...
        allowed, retry_after = await self.rate_limiter.check_rate_limit("global", "global")
        if not allowed:
            raise RateLimitExceeded("Global rate limit exceeded", retry_after)

        # Check per-user rate limit
        allowed, retry_after = await self.rate_limiter.check_rate_limit(identifier, "per_user")
        if not allowed:
