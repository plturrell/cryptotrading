...
class SanitizedError:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {"message": self.public_message, "code": self.error_code}
...
class ErrorMessageSanitizer:
    ...
    def sanitize_error_message(
        self, error: Exception, error_type: str = "internal", include_details: bool = False
    ) -> SanitizedError:
        """
    ...
    def sanitize_exception_details(self, exc_info: tuple) -> Dict[str, Any]:
        ...
        if self.sanitization_level == SanitizationLevel.MAXIMUM:
            return {"error": "An error occurred", "details": "Contact support for assistance"}
        ...
        if self.environment == "development" or self.sanitization_level == SanitizationLevel.BASIC:
...
def sanitize_mcp_error(
    error: Exception, sanitizer: Optional[ErrorMessageSanitizer] = None
) -> SanitizedError:
...
def create_safe_error_response(
    error: Exception, sanitization_level: SanitizationLevel = SanitizationLevel.STRICT
) -> Dict[str, Any]:
