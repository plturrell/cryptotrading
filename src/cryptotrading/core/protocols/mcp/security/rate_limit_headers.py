...
class RateLimitHeaderManager:
    ...
    def create_rate_limit_info(self, limit: int, used: int, window_start: float, 
                              window_duration: int = 60, policy: str = "sliding") -> RateLimitInfo:
        """
    ...
    def add_headers_to_response(self, response_dict: Dict[str, Any], 
                               rate_limit_info: RateLimitInfo) -> Dict[str, Any]:
        """
    ...
    def create_rate_limit_exceeded_response(self, rate_limit_info: RateLimitInfo, 
                                          error_message: str = "Rate limit exceeded") -> Dict[str, Any]:
        """
    ...
    def get_rate_limit_guidance(self, rate_limit_info: RateLimitInfo) -> Dict[str, Any]:
        ...
        guidance = {
            "current_usage": {
                ...
                "percentage_used": ((rate_limit_info.limit - rate_limit_info.remaining) / rate_limit_info.limit) * 100
            },
            "timing": {
        ...
        if usage_percentage >= 90:
            guidance["recommendations"].append("Critical: Very close to rate limit. Slow down requests immediately.")
        elif usage_percentage >= 75:
            guidance["recommendations"].append("Warning: Approaching rate limit. Consider reducing request frequency.")
        elif usage_percentage >= 50:
            guidance["recommendations"].append("Info: Half of rate limit used. Monitor request frequency.")
        
        if rate_limit_info.retry_after:
            guidance["recommendations"].append(f"Rate limited: Wait {rate_limit_info.retry_after} seconds before retry.")
...
class MultiTierRateLimitManager:
    ...
    def get_most_restrictive_limit(self, rate_limit_statuses: Dict[str, Dict[str, Any]]) -> RateLimitInfo:
        """
    ...
    def create_composite_headers(self, rate_limit_statuses: Dict[str, Dict[str, Any]]) -> Dict[str, str]:
        """
