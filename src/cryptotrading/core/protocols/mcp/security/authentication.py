...
class SecureTokenManager:
    ...
    async def revoke_token(self, token: str):
        ...
        try:
            ...
            expires_at = (
                datetime.fromtimestamp(payload.get("exp", 0)) if payload.get("exp") else None
            )
...
class APIKeyManager:
    ...
    def create_api_key(
        self, user_id: str, permissions: List[Permission], name: str = "", expires_days: int = 365
    ) -> tuple[str, str]:
        """Create new API key"""
        # Generate secure key
...
class SecureAuthenticator:
    ...
    async def authenticate_request(
        self, headers: Dict[str, str], ip_address: Optional[str] = None
    ...
    def create_user_api_key(
        self, user_id: str, permissions: List[Permission], name: str = ""
    ) -> tuple[str, str]:
    ...
    def decorator(func):
        async def wrapper(self, context: AuthenticationContext, *args, **kwargs):
            ...
            if not any(context.has_permission(perm) for perm in required_permissions):
                logger.warning(
                    f"Permission denied for user {context.user_id}: requires {required_permissions}"
                )
                raise PermissionError(
                    f"Insufficient permissions: requires one of {required_permissions}"
                )
