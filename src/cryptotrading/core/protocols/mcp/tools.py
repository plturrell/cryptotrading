...
class ToolResult:
    ...
    def text_result(cls, text: str, is_error: bool = False) -> "ToolResult":
        ...
        return cls(content=[ToolContent(type="text", text=text)], isError=is_error)

    @classmethod
    def data_result(cls, data: str, mime_type: str, is_error: bool = False) -> "ToolResult":
        """Create a data result"""
        return cls(
            content=[ToolContent(type="resource", data=data, mimeType=mime_type)], isError=is_error
        )
    ...
    def json_result(cls, data: Dict[str, Any], is_error: bool = False) -> "ToolResult":
        ...
        return cls(
            content=[
                ToolContent(
                    type="resource", data=json.dumps(data, indent=2), mimeType="application/json"
                )
            ],
            isError=is_error,
        )
    ...
    def error_result(cls, message: str) -> "ToolResult":
        ...
        return cls(content=[ToolContent(type="text", text=f"Error: {message}")], isError=True)
...
class CryptoTradingTools:
    ...
    async def _get_fred_economic_data(
        series_id: str, start_date: str = None, end_date: str = None, frequency: str = None
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    async def _get_crypto_relevant_indicators(
        start_date: str = None, end_date: str = None, include_liquidity_metrics: bool = True
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            data_dict = fred_client.get_crypto_relevant_data(
                start_date=start_date, end_date=end_date, save=True
            )
            ...
            if include_liquidity_metrics:
                ...
                if liquidity_df is not None and not liquidity_df.empty:
                    result["liquidity_metrics"] = {
                        ...
                        "latest_values": liquidity_df.iloc[-1].to_dict()
    ...
    @staticmethod
    async def _get_comprehensive_trading_dataset(
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    df = yahoo_client.download_data(
                        symbol=symbol, start_date=start_date, end_date=end_date, save=True
                    )
                    if df is not None and not df.empty:
            ...
            for series_id in fred_series:
                try:
                    df = fred_client.get_series_data(
                        series_id=series_id, start_date=start_date, end_date=end_date, save=True
                    )
                    if df is not None and not df.empty:
                        results["fred_data"][series_id] = {
                            "data_points": len(df),
                            "latest_value": float(df[series_id].iloc[-1]),
                            "description": fred_client.crypto_relevant_series.get(
                                series_id, series_id
                            ),
                        }
                    else:
            ...
            return {"status": "error", "error": str(e), "symbols": symbols or []}
