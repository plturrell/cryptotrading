...
class CryptoAnalysisPlugin(MCPPlugin):
    ...
    def _create_volatility_analysis_tool(self) -> MCPTool:
        ...
        return MCPTool(
            ...
            parameters={
                "symbol": {"type": "string", "description": "Crypto symbol to analyze"},
                "lookback_days": {
                    "type": "integer",
    ...
    async def _volatility_analysis(self, symbol: str, lookback_days: int = 30) -> Dict[str, Any]:
...
class PluginManager:
    ...
    def disable_plugin(self, plugin_name: str) -> bool:
        ...
        # Remove plugin tools
        tools_to_remove = [
            tool_name for tool_name, pname in self.plugin_tools.items() if pname == plugin_name
        ]
...
async def execute_plugin_tool(
    tool_name: str, arguments: Dict[str, Any], auth_context: AuthContext = None
) -> ToolResult:
    """Execute plugin tool"""
