...
class RiskConfig:
    ...
    stress_test_scenarios: List[str] = field(default_factory=lambda: [
        "market_crash", "flash_crash", "vol_spike", "liquidity_crisis"
    ])
...
class ProductionConfig:
    ...
    def _load_configurations(self):
        ...
        self.database.password = self._decrypt_credential(
            os.getenv("DB_PASSWORD_ENCRYPTED", ""), 
            os.getenv("DB_PASSWORD", self.database.password)
        )
        
        # Exchange configuration removed - only Yahoo Finance and FRED data sources
        
        # Risk configuration
        self.risk.max_portfolio_risk = float(os.getenv("MAX_PORTFOLIO_RISK", self.risk.max_portfolio_risk))
        ...
        self.strands.max_concurrent_workflows = int(os.getenv("MAX_CONCURRENT_WORKFLOWS", self.strands.max_concurrent_workflows))
        self.strands.workflow_timeout_seconds = int(os.getenv("WORKFLOW_TIMEOUT", self.strands.workflow_timeout_seconds))
        self.strands.worker_pool_size = int(os.getenv("STRANDS_WORKER_POOL_SIZE", self.strands.worker_pool_size))
        self.strands.enable_distributed = os.getenv("STRANDS_ENABLE_DISTRIBUTED", "false").lower() == "true"
        ...
        self.code_management.scan_interval = int(os.getenv("CODE_SCAN_INTERVAL", self.code_management.scan_interval))
        self.code_management.auto_fix_enabled = os.getenv("CODE_AUTO_FIX", "true").lower() == "true"
        self.code_management.dashboard_port = int(os.getenv("CODE_DASHBOARD_PORT", self.code_management.dashboard_port))
