...
try:
    from ...infrastructure.analysis.tree_library import (
        TreeOperations, PathOperations, StructuralAnalysis, TreeDiffMerge,
        NestedStructure, TreePath
    )
    TREE_AVAILABLE = True
except ImportError:
    ...
    class TreeOperations:
        @staticmethod
        def flatten(structure): return []
        @staticmethod
        def map_structure(fn, structure): return structure
    class PathOperations:
        @staticmethod
        def get_path(structure, path): return None
        @staticmethod
        def set_path(structure, path, value): return structure
    class StructuralAnalysis:
        @staticmethod
        def get_depth(structure): return 0
        @staticmethod
        def find_substructures(structure, predicate): return []
            ...
            try:
                ...
                try:
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)
                    """, (
                        self.agent_id, key, value_str, memory_type, importance,
                        context, meta_str, datetime.utcnow(), datetime.utcnow(), expires_at
                    ))
                ...
                self._cache[key] = Memory(
                    key=key, value=value, agent_id=self.agent_id,
                    ...
                    access_count=1, expires_at=expires_at
                )
        ...
        async with self._lock:
            ...
            try:
                ...
                if row:
                    ...
                    self._cache[key] = Memory(
                        key=key, value=value, agent_id=self.agent_id,
                        memory_type=row[1], importance=row[2],
                        context=row[3], metadata=json.loads(row[4]) if row[4] else None,
                        created_at=row[5], accessed_at=datetime.utcnow(),
                        access_count=row[6] + 1, expires_at=row[7]
                    )
        ...
        async with self._lock:
            try:
                ...
                for row in cursor.fetchall():
                    ...
                    memory = Memory(
                        key=row[0], value=value, agent_id=self.agent_id,
                        memory_type=row[2], importance=row[3],
                        context=row[4], metadata=json.loads(row[5]) if row[5] else None,
                        created_at=row[6], accessed_at=row[7],
