...
class DIContainer:
    ...
    def _register(self, service_type: Type[T],
                 implementation: Type[T] = None,
                 factory: Callable[[], T] = None,
                 instance: T = None,
                 singleton: bool = True) -> 'DIContainer':
    ...
    async def _resolve_internal(self, service_type: Type[T]) -> T:
        ...
        if registration.singleton and registration.instance is not None and registration.initialized:
    ...
    async def initialize_all(self) -> Dict[str, bool]:
        ...
        for service_type, registration in self._services.items():
            if registration.singleton and registration.instance and hasattr(registration.instance, 'initialize'):
                try:
...
def register_singleton(service_type: Type[T], implementation: Type[T] = None, **kwargs) -> DIContainer:
    """Register singleton service"""
    return get_container().register_singleton(service_type, implementation, **kwargs)


def register_transient(service_type: Type[T], implementation: Type[T] = None, **kwargs) -> DIContainer:
