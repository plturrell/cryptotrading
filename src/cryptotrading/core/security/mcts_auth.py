...
class SecurityManager:
    ...
    async def create_user_session(self, username: str, password: str, 
                                 permissions: List[Permission],
                                 ip_address: str = None,
                                 user_agent: str = None) -> Optional[Tuple[str, UserSession]]:
    ...
    async def authorize_operation(self, session_or_key: Union[UserSession, APIKey], 
        ...
        if isinstance(session_or_key, UserSession):
            ...
            if operation not in session_or_key.permissions and Permission.ADMIN not in session_or_key.permissions:
                logger.warning(f"API key {session_or_key.key_id} lacks {operation.value} permission")
                return False
    ...
    async def create_api_key(self, description: str, permissions: List[Permission],
                           created_by: str, expires_in_days: int = None) -> Tuple[str, str]:
    ...
    async def get_security_stats(self) -> Dict[str, Any]:
        ...
        active_sessions_count = len([s for s in self.active_sessions.values() if not s.is_expired()])
...
class SecureMCTSAgent:
    ...
    async def authenticate_request(self, auth_header: str, ip_address: str = None) -> Dict[str, Any]:
    ...
    async def get_agent_status(self) -> Dict[str, Any]:
        ...
        return await super().get_agent_status() if hasattr(super(), 'get_agent_status') else {'status': 'active'}
