...
class UnifiedBootstrap:
    ...
    def __init__(
        self, service_name: str = "cryptotrading", config: Optional[Dict[str, Any]] = None
    ):
...
def setup_flask_app(app, service_name: str = "cryptotrading"):
    ...
    def before_request():
        ...
        monitor.increment_counter(
            "http.requests",
            tags={"method": request.method, "endpoint": request.endpoint or "unknown"},
        )

    @app.after_request
    def after_request(response):
        monitor.increment_counter("http.responses", tags={"status_code": str(response.status_code)})
        return response
