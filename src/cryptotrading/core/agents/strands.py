...
import json
import uuid
from typing import Dict, Any, Optional, List, Callable, Type, Union, AsyncIterator
    ...
    async def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(self.retry_count):
            try:
                ...
                last_exception = e
                logger.warning(f"Attempt {attempt + 1}/{self.retry_count} failed for {func.__name__}: {e}")
...
class StrandsAgent(MemoryAgent):
    ...
    def __init__(self, agent_id: str, agent_type: str, 
        ...
        self.metrics_exporter = MetricsExporter(self.config) if self.config.enable_telemetry else None
        self._setup_strands()
    ...
    async def execute_tool(self, tool_name: str, parameters: Dict[str, Any]) -> ToolExecutionResult:
        ...
        try:
            ...
            self._update_avg_metric("avg_tool_time_ms", execution_time, self._metrics["tools_executed"])
    ...
    async def process_workflow(self, workflow_id: str, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if context.started_at and context.completed_at:
                ...
                self._update_avg_metric("avg_workflow_time_ms", execution_time, self._metrics["workflows_executed"])
                
            logger.info(f"Workflow {workflow_id} completed with status {context.status} in {execution_time:.2f}ms")
    ...
    async def _execute_simple_tools(self, context: WorkflowContext, tools: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _execute_defined_workflow(self, context: WorkflowContext, workflow_def: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_sequential_steps(self, context: WorkflowContext, steps: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _execute_parallel_steps(self, context: WorkflowContext, steps: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _execute_workflow_step(self, step: Dict[str, Any], previous_results: Dict[str, Any]) -> Any:
    ...
    async def _execute_dag_workflow(self, context: WorkflowContext, dag: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_dag_node(self, node: Dict[str, Any], previous_results: Dict[str, Any]) -> Any:
    ...
    async def _execute_pipeline(self, context: WorkflowContext, pipeline: List[Dict[str, Any]]) -> Any:
    ...
    async def _apply_reduce_transform(self, data: Any, func_name: str, initial: Any) -> Any:
        ...
        if isinstance(data, list):
            accumulator = initial
            for item in data:
                result = await self.execute_tool(func_name, {"accumulator": accumulator, "item": item})
    ...
    async def _cleanup_old_contexts(self):
        ...
        while True:
            try:
                ...
                old_workflows = [
                    wf_id for wf_id, context in self.workflow_engine._workflows.items()
    ...
    async def _emit_telemetry(self, event_type: str, data: Any):
        ...
        if event_type == "workflow_submitted":
            ...
            telemetry_event["data"] = {
                "workflow_id": data.workflow_id,
                "status": data.status,
                "duration_ms": (data.completed_at - data.started_at).total_seconds() * 1000 if data.completed_at and data.started_at else 0,
        ...
        try:
            ...
            if result['success']:
                ...
                if result.get('security_violations'):
                    error_msg += f" (Security violations: {', '.join(result['security_violations'])})"
