...
class ModularStrandsAgent(MemoryAgent):
    ...
    def _init_components(self):
        ...
        # Components will be resolved from DI container during initialization
        # Core components
        self.tool_manager: Optional[ToolManager] = None
        self.workflow_engine: Optional[WorkflowEngine] = None
        self.context_manager: Optional[ContextManager] = None
        self.code_executor: Optional[SecureCodeExecutor] = None
    ...
    async def initialize(self, **kwargs) -> bool:
        ...
        try:
            ...
            if self.communication_manager and hasattr(self.communication_manager, "initialize"):
                init_tasks.append(
                    self.communication_manager.initialize({"agent_id": self.config.agent_id})
                )
    ...
    async def _resolve_dependencies(self):
        ...
        di_config = get_strands_di_config()
        await di_config.configure_all().initialize_container()

        # Resolve core components
        try:
            self.tool_manager = await resolve(ToolManager)
            # Configure with agent-specific settings
            if hasattr(self.tool_manager, "configure"):
                self.tool_manager.configure(
                    max_concurrent_executions=self.config.max_concurrent_tools
                )
        except Exception:
            logger.warning("ToolManager not available in DI container")

        try:
            self.workflow_engine = await resolve(WorkflowEngine)
            # Configure with agent-specific settings
            if hasattr(self.workflow_engine, "configure"):
                self.workflow_engine.configure(
                    tool_manager=self.tool_manager,
                    max_concurrent_workflows=self.config.max_concurrent_workflows,
                )
        except Exception:
            logger.warning("WorkflowEngine not available in DI container")

        try:
            self.context_manager = await resolve(ContextManager)
            # Configure with agent-specific settings
            if hasattr(self.context_manager, "configure"):
                self.context_manager.configure(
                    agent_id=self.config.agent_id, max_contexts=self.config.max_contexts
                )
        except Exception:
            logger.warning("ContextManager not available in DI container")

        # Secure code executor
        if self.config.enable_code_execution:
            try:
                self.code_executor = await resolve(SecureCodeExecutor)
                if hasattr(self.code_executor, "configure"):
                    self.code_executor.configure(security_level=self.config.security_level)
            except Exception:
                logger.warning("SecureCodeExecutor not available in DI container")
        ...
        try:
            ...
            if session_id:
                await self.context_manager.add_tool_execution(
                    session_id, tool_name, parameters, execution.result, execution.error
                )
    ...
    async def execute_workflow(
        self, workflow_id: str, initial_context: Dict[str, Any] = None, session_id: str = None
    ) -> Dict[str, Any]:
        """Execute a workflow"""
        try:
            # Execute workflow
            ...
            if self.metrics_collector:
                await self.metrics_collector.record_timing(
                    ...
                    execution.total_execution_time * 1000,
    ...
    async def store_memory(
        self, key: str, value: Any, session_id: str = None, memory_type: str = "short_term"
    ) -> bool:
    ...
    async def _tool_get_context(
        self, session_id: str, include_stats: bool = False
    ) -> Dict[str, Any]:
    ...
    async def _tool_execute_code(
        self, code: str, language: str = "python", session_id: str = None
    ) -> Dict[str, Any]:
