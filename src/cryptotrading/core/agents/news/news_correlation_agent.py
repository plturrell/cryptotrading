...
from ..strands import StrandConfig, StrandsAgent
...
class NewsCorrelationAgent(StrandsAgent):
    ...
    def __init__(self, config: Optional[StrandConfig] = None):
        ...
        # Analysis parameters
        self.analysis_timeframes = [timedelta(hours=1), timedelta(hours=4), timedelta(hours=24)]
    ...
    async def _analyze_single_news_impact(
        self, news_item: NewsItem, market_data: List[Dict], symbol: str
    ) -> Optional[NewsEventImpact]:
        ...
        try:
            ...
            volume_before = self._find_closest_price(
                df, news_timestamp - timedelta(hours=24), volume_col
            )
            volume_after = self._find_closest_price(
                df, news_timestamp + timedelta(hours=24), volume_col
            )

            # Calculate impacts
            immediate_impact = self._calculate_price_change(prices_before["1h"], prices_after["1h"])
            short_term_impact = self._calculate_price_change(
                prices_before["4h"], prices_after["4h"]
            )
            long_term_impact = self._calculate_price_change(
                prices_before["24h"], prices_after["24h"]
            )
    ...
    def _find_closest_price(
        self, df: pd.DataFrame, target_time: datetime, price_col: str
    ) -> Optional[float]:
    ...
    def _calculate_price_change(
        self, price_before: Optional[float], price_after: Optional[float]
    ) -> float:
    ...
    def _calculate_correlation_statistics(
        self, correlations: List[NewsEventImpact]
    ) -> Dict[str, Any]:
        """Calculate overall correlation statistics"""
        if not correlations:
            return {}
        ...
        stats = {
            ...
            "significant_correlations_count": len(
                [c for c in correlations if c.significance_level > 0.7]
            ),
            "strong_correlations_count": len(
                [c for c in correlations if abs(c.correlation_strength) > 0.5]
            ),
            "positive_correlations_count": len(
                [c for c in correlations if c.correlation_strength > 0]
            ),
            "negative_correlations_count": len(
                [c for c in correlations if c.correlation_strength < 0]
            ),
        }
    ...
    async def _identify_correlation_patterns(
        self, correlations: List[NewsEventImpact], symbol: str
        ...
        try:
            ...
            for correlation in correlations:
                ...
                impact_magnitude = (
                    "high"
                    if abs(correlation.immediate_impact) > 3
                    else "medium"
                    if abs(correlation.immediate_impact) > 1
                    else "low"
                )
    ...
    async def _store_correlation_results(
        self, correlations: List[NewsEventImpact], patterns: List[CorrelationPattern]
    ):
            ...
            for correlation in correlations:
                ...
                """,
                    (
                        correlation.news_id,
                        correlation.symbol,
                        correlation.news_timestamp,
                        correlation.price_before_1h,
                        correlation.price_before_4h,
                        correlation.price_before_24h,
                        correlation.price_after_1h,
                        correlation.price_after_4h,
                        correlation.price_after_24h,
                        correlation.volume_before_24h,
                        correlation.volume_after_24h,
                        correlation.immediate_impact,
                        correlation.short_term_impact,
                        correlation.long_term_impact,
                        correlation.volume_impact,
            ...
            for pattern in patterns:
                ...
                """,
                    (
                        ...
                        pattern.typical_impact,
                        pattern.impact_timeframe,
    ...
    async def predict_news_impact(self, news_item: NewsItem) -> Dict[str, Any]:
        ...
        try:
            ...
            for pattern in self.pattern_cache.values():
                ...
                if any(
                    keyword in news_item.title.lower() or keyword in news_item.content.lower()
            ...
            weighted_impact = (
                sum(p.typical_impact * p.confidence_score for p in matching_patterns) / total_weight
            )
            ...
            return {"predicted_impact": 0.0, "confidence": 0.0, "error": str(e)}
