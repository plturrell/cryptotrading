...
class NewsMarketCorrelation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {**asdict(self), "news_timestamp": self.news_timestamp.isoformat()}
...
class NewsCollectionAgent(StrandsAgent):
                ...
                try:
                    ...
                    news_item = NewsItem(
                        ...
                        published_at=self._parse_datetime(
                            article.get("published_at", article.get("publishedAt"))
                        ),
    ...
    async def collect_news_for_all_symbols(self) -> Dict[str, List[NewsItem]]:
        ...
        tasks = [self.collect_news_for_symbol(symbol) for symbol in self.monitored_symbols]
    ...
    async def get_news_summary(self, symbol: str, hours_back: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            if not news_items:
                return {"symbol": symbol, "news_count": 0, "message": "No news found"}

            # Calculate summary statistics
            sentiment_scores = [
                item.sentiment_score for item in news_items if item.sentiment_score is not None
            ]
            relevance_scores = [
                item.relevance_score for item in news_items if item.relevance_score is not None
            ]
            ...
            summary = {
                ...
                "average_relevance": round(sum(relevance_scores) / len(relevance_scores), 3)
                if relevance_scores
                else 0.5,
