...
class ToolManager:
    ...
    def __init__(self, max_concurrent_executions: int = 10):
        ...
        logger.info(
            f"ToolManager initialized with max {max_concurrent_executions} concurrent executions"
        )
    ...
    async def execute_tool(
        ...
        execution = ToolExecution(
            ...
            metadata={"auth_context": auth_context, "execution_context": execution_context},
        )
            ...
            async with self._execution_semaphore:
                ...
                if asyncio.iscoroutinefunction(tool.handler):
                    result = await asyncio.wait_for(
                        tool.handler(**parameters), timeout=tool.timeout
                    )
                else:
                ...
                logger.info(
                    f"Tool {tool_name} completed successfully in {execution.execution_time:.2f}s"
                )
    ...
    def _create_failed_execution(
        self, execution_id: str, tool_name: str, parameters: Dict[str, Any], error: str
    ) -> ToolExecution:
    ...
    def get_tool_metrics(self, tool_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if tool_name:
            ...
            return {
                ...
                "circuit_breaker_state": self._circuit_breakers.get(tool_name, {}).state
    ...
    async def shutdown(self):
        ...
        if self._active_executions:
            ...
            if self._active_executions:
                logger.warning(
                    f"Forcibly terminating {len(self._active_executions)} active executions"
                )
