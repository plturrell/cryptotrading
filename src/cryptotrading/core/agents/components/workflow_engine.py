...
class WorkflowExecutor:
    ...
    async def execute_workflow(self, workflow: WorkflowDefinition, 
        ...
        try:
                ...
                if not ready_steps:
                    ...
                    if parallel_ready:
                        ...
                        await self._execute_parallel_steps(
                            workflow, execution, parallel_ready
                        )
            ...
            logger.info(f"Workflow {workflow.name} completed with status {execution.status} "
                       f"in {execution.total_execution_time:.2f}s")
        
        return execution
    
    async def _execute_step(self, workflow: WorkflowDefinition, 
                          execution: WorkflowExecution, step: WorkflowStep) -> bool:
    ...
    async def _execute_parallel_steps(self, workflow: WorkflowDefinition,
                                    execution: WorkflowExecution, 
                                    steps: List[WorkflowStep]) -> Dict[str, bool]:
    ...
    def _find_ready_steps(self, steps: List[WorkflowStep], completed: Set[str], 
                         failed: Set[str], dependency_graph: Dict[str, Set[str]]) -> List[WorkflowStep]:
    ...
    def _find_parallel_ready_steps(self, workflow: WorkflowDefinition,
                                 completed: Set[str], failed: Set[str]) -> List[WorkflowStep]:
...
class WorkflowEngine:
    ...
    def __init__(self, tool_manager, max_concurrent_workflows: int = 5):
        ...
        # Executors
        self._executors = [
            WorkflowExecutor(tool_manager) 
            for _ in range(max_concurrent_workflows)
        ]
    ...
    async def shutdown(self):
        ...
        if self._active_executions:
            ...
            if self._active_executions:
                logger.warning(f"Forcibly terminating {len(self._active_executions)} active workflows")
