...
class AgentManagerCLI:
    ...
    async def register_agent(
        ...
        compliance_result = await self.execute_mcp_tool(
            "validate_compliance",
            {"agent_id": agent_id, "capabilities": capabilities, "mcp_tools": mcp_tools},
        )

        if not compliance_result.get("compliant", False):
            logger.error(
                f"Agent {agent_id} failed compliance: {compliance_result.get('violations')}"
            )
        ...
        skill_card_result = await self.execute_mcp_tool(
            "generate_skill_card", {"agent_id": agent_id, "capabilities": capabilities}
        )
        ...
        if blockchain and registration_result.get("status") == "success":
            blockchain_result = await self.execute_mcp_tool(
                "blockchain_register", {"agent_id": agent_id, "skill_card": skill_card}
            )
    ...
    async def manage_lifecycle(self, agent_id: str, action: str) -> Dict[str, Any]:
        ...
        result = await self.execute_mcp_tool(
            "manage_lifecycle", {"agent_id": agent_id, "action": action}
        )
    ...
    async def query_agents(self, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        result = await self.execute_mcp_tool("query_registry", {"filter": filters or {}})
    ...
    async def audit_compliance(self, detailed: bool = False) -> Dict[str, Any]:
        ...
        result = await self.execute_mcp_tool(
            "audit_compliance", {"include_blockchain": True, "detailed_report": detailed}
        )
    ...
    async def monitor_health(self) -> Dict[str, Any]:
        ...
        result = await self.execute_mcp_tool("monitor_health", {"include_metrics": True})
