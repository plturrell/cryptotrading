...
class MCTSNode:
    ...
    def puct_value(self, c_param: float = 1.0, parent_visits: int = None) -> float:
        ...
        prior = self.action_priors.get(self.action, 1.0 / len(self.parent.children)) if self.action else 0
    ...
    def rave_adjusted_value(self, c_param: float = math.sqrt(2), parent_visits: int = None,
                           rave_c: float = 1.0, equiv_param: float = 1000) -> float:
        """
    ...
    def best_child(self, c_param: float = 0.0, use_rave: bool = False) -> Optional['MCTSNode']:
        ...
        if use_rave and c_param > 0:
            ...
            return max(self.children.values(),
                      key=lambda n: n.rave_adjusted_value(c_param, self.visits))
        else:
            # Standard UCT or pure exploitation
            return max(self.children.values(),
                      key=lambda n: n.uct_value(c_param, self.visits))
...
class MCTSAlgorithm:
    ...
    def __init__(self,
                 ...
                 pw_k: float = 1.0,
                 virtual_loss: int = 1,
                 selection_method: str = 'robust'):  # robust, secure, max
        """
    ...
    def search(self, root_state: Any, 
               num_iterations: int,
               environment: Any,
               parallel: bool = False) -> Tuple[Any, Dict[str, Any]]:
        """
        ...
        root = MCTSNode(
            state=root_state,
            untried_actions=environment.get_actions(root_state)
        )
...
class ParallelMCTS(MCTSAlgorithm):
    ...
    async def search_parallel(self, root_state: Any,
                            num_iterations: int,
                            environment: Any) -> Tuple[Any, Dict[str, Any]]:
        """
