...
def assess_market_regime(data: pd.DataFrame, lookback: int = 50) -> Dict[str, Any]:
    ...
    try:
        ...
        consistent_moves = sum(1 for r in returns if (r > 0) == (trend_direction > 0)) / len(returns)
...
def calculate_risk_metrics(data: pd.DataFrame, signals: List[Dict[str, Any]], 
...
def generate_trading_recommendation(data: pd.DataFrame, all_signals: List[Dict[str, Any]],
    ...
    try:
        ...
        if not signal_strength["success"] or not market_regime["success"] or not risk_metrics["success"]:
        ...
        if risk_score > threshold["max_risk_score"]:
            ...
            if overall_signal in ["strong_buy", "buy"]:
                ...
                base_size = 0.1 if risk_tolerance == "low" else 0.2 if risk_tolerance == "medium" else 0.3
                confidence_multiplier = confidence
                risk_multiplier = max(0.5, 1 - (risk_score / 100))
                
                recommendation["position_size"] = base_size * confidence_multiplier * risk_multiplier
