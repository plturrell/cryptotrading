                ...
                triangles.append(
                    {
                        ...
                        "recent_peaks": [
                            {"index": int(i), "price": float(highs[i])} for i in recent_peaks
                        ],
                        "recent_troughs": [
                            {"index": int(i), "price": float(lows[i])} for i in recent_troughs
                        ],
                    }
                )
...
def detect_head_and_shoulders(
    data: pd.DataFrame, window: int = 5, shoulder_tolerance: float = 0.05
) -> Dict[str, Any]:
    """
        ...
        if len(peaks) >= 3:
            ...
            for i in range(len(recent_peaks) - 2):
                ...
                if head_price > left_shoulder_price and head_price > right_shoulder_price:
                    shoulder_diff = abs(left_shoulder_price - right_shoulder_price) / max(
                        left_shoulder_price, right_shoulder_price
                    )

                    if shoulder_diff <= shoulder_tolerance:
                        relevant_troughs = [
                            t for t in troughs if left_shoulder_idx < t < right_shoulder_idx
                        ]
...
def detect_double_tops_bottoms(
    data: pd.DataFrame, window: int = 5, similarity_tolerance: float = 0.02
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for pattern in patterns:
            if pattern["type"] == "double_top" and current_price < pattern["support_level"] * 0.99:
                ...
                and current_price > pattern["resistance_level"] * 1.01
            ):
                signals.append(
                    {
...
def detect_flags_pennants(
    data: pd.DataFrame, trend_window: int = 20, pattern_window: int = 10
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        trend_change = (trend_data["close"].iloc[-1] - trend_data["close"].iloc[0]) / trend_data[
            "close"
        ].iloc[0]

        if abs(trend_change) >= 0.05:  # 5% minimum trend
            trend_direction = "up" if trend_change > 0 else "down"
            pattern_range = (pattern_data["high"].max() - pattern_data["low"].min()) / pattern_data[
                "close"
            ].mean()
