    ...
    try:
        ...
        if not pd.isna(current_vwap):
            ...
            if abs(price_vs_vwap) > 2.0:
                signal_type = "strong_deviation_above" if price_vs_vwap > 0 else "strong_deviation_below"
...
def calculate_mfi(data: pd.DataFrame, period: int = 14,
                 overbought: float = 80.0, oversold: float = 20.0) -> Dict[str, Any]:
    """
    ...
    try:
        if TALIB_AVAILABLE:
            mfi = pd.Series(
                talib.MFI(data['high'].values, data['low'].values, 
                         data['close'].values, data['volume'].values, 
                         timeperiod=period),
                index=data.index
            )
...
def detect_volume_anomalies(data: pd.DataFrame, lookback: int = 20,
...
def analyze_volume_indicators(data: pd.DataFrame, mfi_period: int = 14,
                            volume_lookback: int = 20) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        volume_strength = "strong" if strong_signals > total_signals * 0.5 else "medium" if strong_signals > 0 else "weak"
