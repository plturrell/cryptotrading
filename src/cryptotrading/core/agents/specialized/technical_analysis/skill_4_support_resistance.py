...
def detect_pivot_points(data: pd.DataFrame, window: int = 5) -> Dict[str, Any]:
    ...
    try:
        ...
        for idx in high_peaks:
            pivot_highs.append({
                "index": int(idx),
                "date": data.index[idx].isoformat() if hasattr(data.index[idx], 'isoformat') else str(data.index[idx]),
...
def detect_psychological_levels(data: pd.DataFrame, round_numbers: List[int] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for interval in round_numbers:
            if interval <= price_range * 0.1:  # Only consider intervals that make sense for the price range
        ...
        return {
            "success": True,
            "levels": {
                "psychological_levels": closest_levels,
...
def calculate_level_strength(data: pd.DataFrame, levels: List[Dict], 
                           touch_tolerance: float = 0.02, min_touches: int = 2) -> Dict[str, Any]:
    """
        ...
        for level_info in levels:
            ...
            if touches >= min_touches:
                ...
                strength_score = min(100, (
                    (touches * 15) +  # More touches = stronger
                    (bounce_rate * 40) +  # Higher bounce rate = stronger
                    (min(volume_ratio, 2) * 20) +  # Higher volume = stronger (capped at 2x)
                    (25 if breaks == 0 else max(0, 25 - breaks * 5))  # Fewer breaks = stronger
                ))
                ...
                strengthened_levels.append({
                    **level_info,
                    ...
                    "strength_rating": (
                        "very_strong" if strength_score >= 80 else
                        "strong" if strength_score >= 60 else
                        "medium" if strength_score >= 40 else
                        "weak"
                    )
                })
...
def detect_support_resistance_flips(data: pd.DataFrame, levels: List[Dict],
                                   flip_tolerance: float = 0.03) -> Dict[str, Any]:
    """
...
def analyze_support_resistance(data: pd.DataFrame, pivot_window: int = 5,
                             lookback: int = 50, min_touches: int = 2) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # Combine all signals
        all_signals = []
        for result in [pivot_result, psychological_result, fibonacci_result, strength_result, flip_result]:
        ...
        nearby_support = [l for l in strong_levels if l["type"] == "support" and l["level"] < current_price]
        nearby_resistance = [l for l in strong_levels if l["type"] == "resistance" and l["level"] > current_price]
