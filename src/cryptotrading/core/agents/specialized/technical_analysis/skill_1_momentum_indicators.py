...
def calculate_sma(data: pd.DataFrame, periods: List[int] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        for period in periods:
            if TALIB_AVAILABLE:
                sma = pd.Series(
                    talib.SMA(data["close"].values, timeperiod=period), index=data.index
                )
            else:
...
def calculate_ema(data: pd.DataFrame, periods: List[int] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        for period in periods:
            if TALIB_AVAILABLE:
                ema = pd.Series(
                    talib.EMA(data["close"].values, timeperiod=period), index=data.index
                )
            else:
...
def calculate_rsi(
    data: pd.DataFrame, period: int = 14, overbought: float = 70.0, oversold: float = 30.0
) -> Dict[str, Any]:
    """
    ...
    try:
        if TALIB_AVAILABLE:
            rsi = pd.Series(talib.RSI(data["close"].values, timeperiod=period), index=data.index)
        else:
