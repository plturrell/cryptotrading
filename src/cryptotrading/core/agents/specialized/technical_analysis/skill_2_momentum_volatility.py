...
def calculate_macd(data: pd.DataFrame, fast_period: int = 12, slow_period: int = 26, 
    ...
    try:
        ...
        if len(macd_line) > 1 and not pd.isna(macd_line.iloc[-1]) and not pd.isna(signal_line.iloc[-1]):
...
def analyze_momentum_volatility(data: pd.DataFrame,
                               macd_fast: int = 12, macd_slow: int = 26, macd_signal: int = 9,
                               bb_period: int = 20, bb_std: float = 2.0,
                               atr_period: int = 14,
                               stoch_k: int = 14, stoch_d: int = 3) -> Dict[str, Any]:
    """
