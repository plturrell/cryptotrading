...
def generate_ta_summary_report(data: pd.DataFrame, all_analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        signal_summary = {
            ...
            "signal_bias": "bullish" if len(buy_signals) > len(sell_signals) else "bearish" if len(sell_signals) > len(buy_signals) else "neutral"
        }
        
        # Top signals by strength
        strong_signals = [s for s in signals if s.get("strength") in ["strong", "very_strong"]]
        strong_signals.sort(key=lambda x: {"weak": 1, "medium": 2, "strong": 3, "very_strong": 4}.get(x.get("strength", "medium"), 2), reverse=True)
        ...
        if "support_resistance" in all_analysis:
            ...
            if "levels" in sr_data:
                support_levels = [l for l in sr_data["levels"].get("nearby_support", []) if l.get("level")]
                resistance_levels = [l for l in sr_data["levels"].get("nearby_resistance", []) if l.get("level")]
        ...
        if support_levels:
            ...
            risk_assessment["support_distance"] = (current_price - nearest_support["level"]) / current_price * 100
        
        if resistance_levels:
            nearest_resistance = min(resistance_levels, key=lambda x: abs(current_price - x["level"]))
            risk_assessment["resistance_distance"] = (nearest_resistance["level"] - current_price) / current_price * 100
        
        # Generate executive summary
        executive_summary = []
        
        if signal_summary["signal_bias"] != "neutral":
            executive_summary.append(f"Overall sentiment is {signal_summary['signal_bias']} with {signal_summary['total_signals']} active signals")
        
        if strong_signals:
            executive_summary.append(f"{len(strong_signals)} strong signals detected")
        
        if pattern_summary["total_patterns"] > 0:
            executive_summary.append(f"{pattern_summary['total_patterns']} chart patterns identified")
...
def create_signal_heatmap(signals: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        heatmap_data = {}
        skills = ["momentum_indicators", "momentum_volatility", "volume_analysis", 
        ...
        for skill in skills:
            row = {
                ...
                "total_signals": sum(heatmap_data[skill][st]["count"] for st in ["buy", "sell", "neutral"])
            }
...
def generate_performance_metrics(data: pd.DataFrame, signals: List[Dict[str, Any]],
    ...
    try:
        ...
        performance_metrics = {
            ...
            "period": {
                ...
                "start_date": recent_data.index[0].isoformat() if hasattr(recent_data.index[0], 'isoformat') else str(recent_data.index[0]),
                "end_date": recent_data.index[-1].isoformat() if hasattr(recent_data.index[-1], 'isoformat') else str(recent_data.index[-1])
            }
        }
...
def create_alert_system(signals: List[Dict[str, Any]], alert_config: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        strong_signals = [s for s in signals if strength_values.get(s.get("strength", "medium"), 2) >= min_strength]
        ...
        # Breakout alerts
        breakout_signals = [s for s in signals if "breakout" in s.get("type", "").lower() or "breakdown" in s.get("type", "").lower()]
...
def create_ui5_dashboard_data(data: pd.DataFrame, all_analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        ui5_data = {
            ...
            "currentPrice": summary_report.get("report", {}).get("market_overview", {}).get("current_price", 0),
            "priceChange24h": summary_report.get("report", {}).get("market_overview", {}).get("price_change_24h", 0),
            "indicators": {
                "RSI": 65.4,
                "MACD": 0.045,
                "SMA_20": summary_report.get("report", {}).get("market_overview", {}).get("current_price", 0) * 0.98,
                "EMA_20": summary_report.get("report", {}).get("market_overview", {}).get("current_price", 0) * 0.985
            },
        ...
        if isinstance(skill_patterns, list):
            for pattern in skill_patterns:
                formatted_patterns.append({
                    ...
                    "reliabilityState": _get_reliability_state(pattern.get("reliability", "Medium")),
    ...
    if "levels" in support_resistance:
        ...
        for level in levels_data.get("nearby_support", [])[:3]:
            formatted_levels.append({
                ...
                "distance": abs((level.get("level", 0) - level.get("current_price", level.get("level", 0))) / level.get("level", 1) * 100),
                "typeState": "Success"
            })
...
def export_dashboard_data(data: pd.DataFrame, all_analysis: Dict[str, Any],
    ...
    try:
        ...
        if export_format.lower() == "json":
            ...
            if summary_report.get("success"):
                ...
                csv_data.append(["Price Change 24h %", report["market_overview"]["price_change_24h"]])
