    ...
    try:
        ...
        if len(all_points) >= 9:  # Need at least 9 points for complete pattern
            for i in range(len(all_points) - 8):
                ...
                if (
                    sequence[0]["type"] == "trough"
                    and sequence[1]["type"] == "peak"
                    and sequence[2]["type"] == "trough"
                    and sequence[3]["type"] == "peak"
                    and sequence[4]["type"] == "trough"
                    and sequence[5]["type"] == "peak"
                    and sequence[6]["type"] == "trough"
                    and sequence[7]["type"] == "peak"
                    and sequence[8]["type"] == "trough"
                ):
                    ...
                    if (
                        wave2_retrace <= wave1_length
                        and wave3_length >= max(wave1_length, wave5_length)  # Rule 1
                        and wave4_low > wave1_end  # Rule 2
                    ):  # Rule 3
                        # Calculate Fibonacci ratios
                        fib_ratios = {
                            "wave2_retrace": wave2_retrace / wave1_length
                            if wave1_length > 0
                            else 0,
                            "wave3_extension": wave3_length / wave1_length
                            if wave1_length > 0
                            else 0,
                            "wave4_retrace": (wave3_end - wave4_end) / wave3_length
                            if wave3_length > 0
                            else 0,
                            "wave5_extension": wave5_length / wave1_length
                            if wave1_length > 0
                            else 0,
                        }
...
def detect_harmonic_patterns(
    data: pd.DataFrame, window: int = 5, tolerance: float = 0.05
) -> Dict[str, Any]:
    """
                        ...
                        harmonic_patterns.append(
                            {
                                ...
                                "ratios": ratios,
                                "completion_level": D,
                                "target_levels": self._calculate_harmonic_targets(
                                    X, A, B, C, D, pattern_type
                                ),
                            }
                        )
...
def _identify_harmonic_pattern(ratios: Dict[str, float], tolerance: float) -> Optional[str]:
    ...
    # Gartley Pattern
    if (
        within_tolerance(ratios["AB_XA"], 0.618, tolerance)
        and within_tolerance(ratios["BC_AB"], 0.382, tolerance)
        and within_tolerance(ratios["CD_BC"], 1.272, tolerance)
        and within_tolerance(ratios["XD_XA"], 0.786, tolerance)
    ):
        return "gartley"

    # Butterfly Pattern
    if (
        within_tolerance(ratios["AB_XA"], 0.786, tolerance)
        and within_tolerance(ratios["BC_AB"], 0.382, tolerance)
        and within_tolerance(ratios["CD_BC"], 1.618, tolerance)
        and within_tolerance(ratios["XD_XA"], 1.27, tolerance)
    ):
        return "butterfly"

    # Bat Pattern
    if (
        within_tolerance(ratios["AB_XA"], 0.382, tolerance)
        and within_tolerance(ratios["BC_AB"], 0.382, tolerance)
        and within_tolerance(ratios["CD_BC"], 1.618, tolerance)
        and within_tolerance(ratios["XD_XA"], 0.886, tolerance)
    ):
        return "bat"
    ...
    if (
        within_tolerance(ratios["AB_XA"], 0.382, tolerance)
        and within_tolerance(ratios["BC_AB"], 0.382, tolerance)
        and within_tolerance(ratios["CD_BC"], 2.24, tolerance)
        and within_tolerance(ratios["XD_XA"], 1.618, tolerance)
    ):
        return "crab"
...
def _calculate_harmonic_targets(
    ...
    if pattern_type in ["gartley", "bat"]:
        ...
        targets.extend([D + CD_range * 0.382, D + CD_range * 0.618, C])  # Return to C level
    elif pattern_type in ["butterfly", "crab"]:
        # More aggressive targets
        targets.extend([D + CD_range * 0.618, D + CD_range * 1.0, D + CD_range * 1.618])

    return sorted(targets)


def detect_wyckoff_phases(
    data: pd.DataFrame, volume_window: int = 20, price_window: int = 50
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for i in range(price_window, len(recent_data) - price_window):
            ...
            range_ratio = (
                avg_range / data["price_range"].mean() if data["price_range"].mean() > 0 else 1
            )
    ...
    return [
        {
            ...
            "parameters": {"data": "OHLCV DataFrame", "window": "Window for peak/trough detection"},
        ...
        },
        {
            ...
            "parameters": {"data": "OHLCV DataFrame", "window": "Window for pattern detection"},
            "category": "technical_analysis",
            "skill": "harmonic_patterns",
        },
    ]
