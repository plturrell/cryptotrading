...
class MCTSMonitor:
    ...
    async def record_calculation(self, metrics: PerformanceMetrics):
        ...
        from ...protocols.mcp.events import MCPEvent, EventType
        import time
        event = MCPEvent(
    ...
    async def get_health_status(self, circuit_breaker_state: str, 
                               rate_limit_remaining: int,
                               memory_ok: bool) -> HealthStatus:
        ...
        last_calc_time = self.performance_metrics[-1].timestamp if self.performance_metrics else None
        
        # Determine health status
        if circuit_breaker_state == 'open' or not memory_ok:
            status = 'unhealthy'
        elif error_rate > self.alert_thresholds['error_rate'] or avg_response_time > self.alert_thresholds['response_time']:
            status = 'degraded'
        else:
            status = 'healthy'
    ...
    async def get_performance_summary(self, hours: int = 1) -> Dict[str, Any]:
        ...
        return {
            ...
            'error_count': len([e for e in self.error_log if datetime.fromisoformat(e['timestamp']) > cutoff_time]),
            'metrics': {
                ...
                'iterations_per_second': total_iterations / sum(m.execution_time for m in recent_metrics)
            }
        }
