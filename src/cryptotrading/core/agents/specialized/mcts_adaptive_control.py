...
class DynamicExplorationParams:
    ...
    def update(self, convergence_metrics: ConvergenceMetrics, iteration: int, 
        ...
        self.current_c_param = (
            self.current_c_param * (1 - adjustment_rate) + 
            target_c_param * adjustment_rate
        )
...
class AdaptiveIterationController:
    ...
    def should_continue_search(self, 
                             ...
                             best_action: str) -> Tuple[bool, str, Dict[str, Any]]:
    ...
    def get_final_report(self) -> Dict[str, Any]:
        ...
        avg_iteration_time = statistics.mean(list(self.iteration_times)) if self.iteration_times else 0
        ...
        return {
            ...
            "efficiency_gain": max(0, (self.max_iterations - self.current_iteration) / self.max_iterations)
        }
...
class MemoryOptimizedMCTSNode:
    ...
    def prune_subtree(self, keep_best_n: int = 3):
        ...
        for action_key, child in self.children.items():
            if child.visits > 0:
                ucb_score = (child.value / child.visits) + math.sqrt(2 * math.log(self.visits) / child.visits)
            else:
                ucb_score = float('inf')
        ...
        children_to_keep = {action_key: child for _, action_key, child in child_scores[:keep_best_n]}
