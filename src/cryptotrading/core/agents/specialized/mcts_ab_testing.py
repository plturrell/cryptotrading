...
class ExperimentStrategy(ABC):
    ...
    async def run_variant(
        self, variant: VariantConfig, mcts_agent: Any, test_parameters: Dict[str, Any]
    ) -> ExperimentResult:
        """Run a specific variant and return results"""
        pass

    @abstractmethod
    def get_variant_hash(self, variant: VariantConfig, test_parameters: Dict[str, Any]) -> str:
...
class MCTSExperimentStrategy(ExperimentStrategy):
    ...
    def get_variant_hash(self, variant: VariantConfig, test_parameters: Dict[str, Any]) -> str:
        ...
        combined_data = {"variant": variant.to_dict(), "test_params": test_parameters}
...
class ABTestManager:
    ...
    def create_experiment(self, experiment_id: str, variants: List[VariantConfig]) -> str:
    ...
    def _analyze_results(
        ...
        best_by_value = max(variant_stats.items(), key=lambda x: x[1].mean_expected_value)
        best_by_time = min(variant_stats.items(), key=lambda x: x[1].mean_execution_time)
        best_by_efficiency = max(variant_stats.items(), key=lambda x: x[1].mean_efficiency_gain)
    ...
    def _calculate_variant_stats(
        self, variant_id: str, results: List[ExperimentResult]
    ) -> ExperimentStats:
        ...
        return ExperimentStats(
            ...
            percentile_95_time=statistics.quantiles(execution_times, n=20)[18]
            if len(execution_times) >= 20
            else max(execution_times),
            percentile_99_time=statistics.quantiles(execution_times, n=100)[98]
    ...
    def _run_significance_tests(self, variant_stats: Dict[str, ExperimentStats]) -> Dict[str, Any]:
        ...
        for variant_id, stats in variant_stats.items():
            ...
            pooled_std = (
                (control_stats.std_expected_value**2 + stats.std_expected_value**2) / 2
            ) ** 0.5
            effect_size = (
                (stats.mean_expected_value - control_stats.mean_expected_value) / pooled_std
                if pooled_std > 0
                else 0
            )
            ...
            value_improvement = (
                (
                    (stats.mean_expected_value - control_stats.mean_expected_value)
                    / abs(control_stats.mean_expected_value)
                )
                if control_stats.mean_expected_value != 0
                else 0
            )

            time_improvement = (
                (
                    (control_stats.mean_execution_time - stats.mean_execution_time)
                    / control_stats.mean_execution_time
                )
                if control_stats.mean_execution_time > 0
                else 0
            )
        ...
        return {"control_variant": control_variant, "comparisons": comparisons}

    def _generate_recommendations(
        self, variant_stats: Dict[str, ExperimentStats], significance_tests: Dict[str, Any]
    ) -> List[str]:
        ...
        win_win_variants = [
            variant_id
            for variant_id in comparisons.keys()
            if (comparisons[variant_id]["is_better_value"] and comparisons[variant_id]["is_faster"])
        ]
