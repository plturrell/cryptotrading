...
class TradingAgentIntegration:
    ...
    async def _handle_analysis_request(self, message: A2AMessage) -> None:
        ...
        if strategy == "grid_trading":
            result = await self.trading_agent._mcp_grid_create(
                symbols[0] if symbols else "BTC/USDT", Decimal(payload.get("investment", "1000"))
            )
        ...
        while True:
            try:
                ...
                avg_win_rate = np.mean(
                    [p.get("win_rate", 0) for p in performance.values() if p.get("win_rate")]
                )
                ...
                overall_performance = {
                    ...
                    "active_strategies": [
                        s.value
                        for s, active in self.trading_agent.active_strategies.items()
                        if active
                    ],
                }
    ...
    async def _execute_full_market_analysis(self) -> Dict[str, Any]:
        ...
        analysis["opportunities"][
            "mean_reversion"
        ] = await self.trading_agent._mcp_mean_reversion_identify(symbols)
        analysis["opportunities"]["breakout"] = await self.trading_agent._mcp_breakout_detect(
            symbols
        )
        analysis["opportunities"]["arbitrage"] = await self.trading_agent._mcp_arbitrage_scan(
            symbols
        )
    ...
    async def _execute_portfolio_rebalance(self) -> Dict[str, Any]:
        ...
        optimization = await self.trading_agent._mcp_portfolio_optimize(
            current_positions, target_return=0.15
        )
