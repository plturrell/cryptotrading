...
class TradingStrategiesExtension:
    ...
    async def _mcp_scalping_monitor(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "trades_per_hour": total_trades / max((scalping_trades[-1]["timestamp"] - scalping_trades[0]["timestamp"]).total_seconds() / 3600, 1)
        }
    ...
    async def _mcp_market_making_quote(self, symbol: str) -> Dict[str, Any]:
        ...
        if params["dynamic_spread"]:
            ...
            inventory_ratio = inventory / params["inventory_target"] if params["inventory_target"] > 0 else 0
    ...
    async def _mcp_breakout_trade(self, confirmation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if order["status"] == "filled":
            asyncio.create_task(
                self._monitor_breakout_pullback(breakout["symbol"], breakout)
            )
    ...
    async def _mcp_ml_predict(self, symbol: str, horizon_hours: int = 24) -> Dict[str, Any]:
        ...
        if hasattr(model.get("random_forest"), "feature_importances_"):
            feature_importance = dict(zip(
                params["features"],
                model["random_forest"].feature_importances_
            ))
        else:
            feature_importance = {}
    ...
    async def _mcp_ml_backtest(self, symbol: str, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        if trades:
            ...
            avg_profit = np.mean([t["profit"] for t in trades if t["profit"] > 0]) if any(t["profit"] > 0 for t in trades) else 0
            avg_loss = np.mean([t["profit"] for t in trades if t["profit"] < 0]) if any(t["profit"] < 0 for t in trades) else 0
    ...
    async def _mcp_strategy_optimize(self) -> Dict[str, Any]:
        ...
        for strategy in TradingStrategy:
            ...
            if strategy == TradingStrategy.GRID_TRADING:
                ...
                self.strategy_params[strategy]["profit_target_percentage"] = optimal_targets["profit"]
    ...
    async def _mcp_position_size(self, symbol: str, entry_price: Decimal, stop_loss: Decimal) -> Dict[str, Any]:
    ...
    async def _mcp_portfolio_optimize(self, symbols: List[str], target_return: float = 0.1) -> Dict[str, Any]:
        ...
        allocations = {symbol: weight for symbol, weight in zip(symbols, optimal_weights) if weight > 0.01}
