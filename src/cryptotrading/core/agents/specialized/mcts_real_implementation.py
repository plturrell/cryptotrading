...
class RealMCTSConfig:
    ...
    position_size_limits: Dict[str, float] = field(default_factory=lambda: {
    ...
    def _calculate_drawdown(self) -> float:
        ...
        for trade in self.trades:
            ...
            if trade['type'] == 'buy':
                ...
                running_portfolio[trade['symbol']] = running_portfolio.get(trade['symbol'], 0) + trade['amount']
            else:
...
class RealMCTSNode:
    ...
    def __init__(self, state: Dict[str, Any], action: Dict[str, Any] = None, 
    ...
    def _generate_intelligent_actions(self) -> List[Dict[str, Any]]:
        ...
        for symbol, data in market_data.items():
            ...
            if current_position > 0:
                ...
                position_pnl = (price - self._get_entry_price(symbol)) / self._get_entry_price(symbol)
...
class RealProductionMCTSAgent:
    ...
    def set_historical_patterns(self, patterns: List[Dict[str, Any]]):
        ...
        logger.info(f"Loaded {len(self.success_patterns)} success patterns and {len(self.failure_patterns)} failure patterns")
        
    async def calculate_optimal_action(self, 
    ...
    async def _evaluate_position(self, node: RealMCTSNode, env: RealTradingEnvironment) -> float:
        ...
        logger.debug(f"Position evaluation: base={base_score:.3f}, pattern_boost={pattern_boost:.3f}, final={final_score:.3f}")
        
        return final_score
    
    def _apply_historical_patterns(self, symbol: str, action: Dict[str, Any], market_data: Dict[str, Any]) -> float:
        ...
        if failure_key in self.failure_patterns:
            ...
            if patterns:
                ...
                logger.debug(f"Applied generic failure pattern penalty: {-avg_confidence * 0.15:.3f}")
