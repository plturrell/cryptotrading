...
from ..protocols.a2a.a2a_protocol import A2A_CAPABILITIES, AgentStatus, MessageType
...
class MCPAgentLifecycleManager:
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        running_agents = sum(
            1 for h in self.agent_health.values() if h.state == AgentLifecycleState.RUNNING
        )
        error_agents = sum(
            1 for h in self.agent_health.values() if h.state == AgentLifecycleState.ERROR
        )

        overall_health = sum(h.health_score for h in self.agent_health.values()) / max(
            total_agents, 1
        )
    ...
    async def _create_agent_instance(self, agent_id: str, config: AgentConfiguration):
        ...
        try:
            ...
            agent = config.agent_class(agent_id=agent_id, **config.initialization_params)
