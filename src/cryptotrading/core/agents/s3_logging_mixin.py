...
class S3LoggingMixin(ABC):
    ...
    def _init_s3_logging(self):
        ...
        if S3_AVAILABLE:
            try:
                ...
                logger.info(
                    f"S3 logging initialized for agent: {getattr(self, 'agent_id', 'unknown')}"
                )
        ...
        try:
            ...
            if force_sync:
                ...
                result = await self._s3_logger.log_agent_activity(
                    parameters=log_params, agent_context=self._agent_context
                )
            else:
        ...
        try:
            ...
            asyncio.create_task(
                self._s3_logger.store_calculation_result(
                    parameters=params, agent_context=self._agent_context
                )
            )
    ...
    async def backup_agent_state(
        self, state_data: Dict[str, Any], configuration: Dict[str, Any] = None, version: str = "1.0"
    ):
        """
    ...
    async def log_agent_shutdown(self, reason: str = "normal"):
        ...
        await self.log_to_s3(
            ...
            data={"shutdown_reason": reason, "shutdown_time": datetime.utcnow().isoformat()},
    ...
    async def log_performance_metric(
        self, operation: str, duration_ms: float, additional_metrics: Dict[str, Any] = None
    ):
    ...
    async def log_decision(
        ...
        await self.log_to_s3(
            f"Decision made: {decision}", level="info", activity_type="decision", data=decision_data
        )
...
def log_agent_method(activity_type: str = None, log_params: bool = False, log_result: bool = False):
    ...
    def decorator(func):
        async def async_wrapper(self, *args, **kwargs):
            ...
            log_data = {"method": method_name, "start_time": start_time.isoformat()}
