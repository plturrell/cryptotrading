...
class RealA2ACommunication:
        ...
        try:
            ...
            for service in services:
                ...
                if agent_id and agent_id != self.agent_id:
                    ...
                    self.logger.info(
                        f"Discovered agent: {agent_id} at {endpoint.host}:{endpoint.port}"
                    )
    ...
    async def _handle_http_message(self, request: aiohttp.web.Request):
        ...
        try:
            ...
            return aiohttp.web.json_response(
                {"success": True, "message_id": message.id, "response": response}
            )
    ...
    async def _handle_status_request(self, request: aiohttp.web.Request):
        ...
        status = {
            ...
            "transport": self.transport.value,
            "server_info": {"host": self.host, "port": self.port},
        }
                ...
                if msg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    if data.get("type") == "handshake":
                        ...
                        if agent_id:
                            ...
                            await ws.send_text(
                                json.dumps({"type": "handshake_ack", "agent_id": self.agent_id})
                            )
                    else:
                        # Handle message
                        message = A2AMessage(**data)
    ...
    async def _deliver_to_agent(self, agent_id: str, message: A2AMessage) -> A2ADeliveryResult:
        ...
        if agent_id not in self.known_agents:
            return A2ADeliveryResult(
                message_id=message.id, success=False, error=f"Agent {agent_id} not found"
            )
    ...
    async def _deliver_via_http(
        self, endpoint: A2AEndpoint, message: A2AMessage
    ) -> A2ADeliveryResult:
    ...
    async def _deliver_via_websocket(self, agent_id: str, message: A2AMessage) -> A2ADeliveryResult:
        ...
        if agent_id not in self.websocket_connections:
            return A2ADeliveryResult(
                message_id=message.id, success=False, error=f"No WebSocket connection to {agent_id}"
            )
        ...
        try:
            ...
            return A2ADeliveryResult(message_id=message.id, success=True)
        except Exception as e:
            return A2ADeliveryResult(message_id=message.id, success=False, error=str(e))
            ...
            async with self.client_session.post(url, json=discovery_data) as response:
                if response.status == 200:
                    ...
                    if discovered_agent_id:
                        ...
                        self.logger.info(
                            f"Connected to agent {discovered_agent_id} at {host}:{port}"
                        )
                        return True
    ...
    async def _handle_lock_request(self, message: A2AMessage) -> Dict[str, Any]:
        ...
        if self.lock_manager:
            try:
                ...
                return {"action": "lock_error", "resource_id": resource_id, "error": str(e)}
        else:
    ...
    async def _handle_state_sync(self, message: A2AMessage) -> Dict[str, Any]:
        ...
        return {"action": "state_sync_response", "state_version": 1, "synchronized": True}
    ...
    async def _message_cleanup_loop(self):
        ...
        while not self._shutdown:
            try:
                ...
                expired_messages = [
                    msg_id for msg_id, msg in self.pending_messages.items() if msg.ttl < now
                ]
                ...
                if len(self.delivery_results) > 1000:
                    sorted_results = sorted(
                        self.delivery_results.items(), key=lambda x: x[1].delivery_time_ms or 0
                    )
