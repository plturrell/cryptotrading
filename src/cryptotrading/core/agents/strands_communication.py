...
class StrandsA2ACommunication:
    ...
    async def disconnect_from_agent(self, agent_id: str):
        ...
        # Update routing table
        self.routing_table = {
            dest: next_hop for dest, next_hop in self.routing_table.items()
    ...
    async def _deliver_message(self, agent_id: str, message: A2AMessage):
        ...
        try:
            ...
            self.logger.debug(f"Successfully delivered message {message.id} to {agent_id} (transport_id: {result_id})")
    ...
    async def _cleanup_expired(self):
        ...
        while True:
            try:
                ...
                expired_proposals = [
                    prop_id for prop_id, prop in self.active_proposals.items()
    ...
    def get_network_status(self) -> Dict[str, Any]:
        ...
        if self.distributed_coordinator:
            status["distributed_coordination"] = self.distributed_coordinator.get_coordination_status()
    ...
    async def acquire_distributed_lock(self, resource_id: str, timeout_seconds: float = 30.0) -> bool:
        ...
        return await self.distributed_coordinator.acquire_distributed_lock(resource_id, timeout_seconds)
    ...
    async def start_distributed_transaction(self, participants: List[str], 
        ...
        return await self.distributed_coordinator.start_distributed_transaction(participants, operations)
