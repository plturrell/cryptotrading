...
from .decision_audit import get_audit_trail, PerformanceMetrics
...
class KnowledgeAccumulator:
    ...
    async def get_accumulated_knowledge(self, session_id: str) -> AccumulatedKnowledge:
        ...
        try:
            ...
            logger.info(f"Built accumulated knowledge: {total_interactions} interactions, "
            ...
            return AccumulatedKnowledge(
                session_id=session_id, total_interactions=0,
                success_patterns=[], failure_patterns=[],
                market_insights={}, agent_performance={},
                learned_strategies=[], risk_assessments={},
    ...
    async def _get_total_interactions(self) -> int:
        ...
        try:
            if self.db.config.mode.value == "local":
                cursor.execute("SELECT COUNT(*) FROM conversation_history WHERE message_type = 'query'")
            else:
                cursor.execute("SELECT COUNT(*) FROM conversation_history WHERE message_type = 'query'")
    ...
    async def _get_success_patterns(self) -> List[KnowledgePattern]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                conf_level = "high" if confidence and confidence > 0.8 else "medium" if confidence and confidence > 0.6 else "low"
            ...
            for algorithm, profits in algorithm_success.items():
                if len(profits) >= 3:  # Minimum sample size
                    patterns.append(KnowledgePattern(
                        ...
                        confidence=min(0.95, len(profits) / 10),  # Confidence based on sample size
                        success_rate=1.0,  # These are all successful
                        sample_size=len(profits),
                        conditions={"algorithm": algorithm},
                        outcomes=[{"avg_profit": statistics.mean(profits), "max_profit": max(profits)}],
        ...
        try:
            ...
            for symbol, insights in symbol_insights.items():
                if len(insights) >= 2:
                    ...
                    avg_confidence = statistics.mean([i['confidence'] for i in insights if i['confidence']])
    ...
    async def _update_algorithm_performance(self, mcts_decision: Dict[str, Any], outcome: Optional[Dict[str, Any]]):
    ...
    async def _load_existing_patterns(self):
        ...
        try:
            pattern_memories = await self.memory.search(
                query="pattern",
                memory_types=["knowledge_pattern"],
                limit=50
            )
