...
class DecisionAuditTrail:
    ...
    async def record_decision_outcome(
        self, decision_id: int, execution_result: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            await self._update_decision_status(
                decision_id, DecisionStatus.SUCCESSFUL if success else DecisionStatus.FAILED
            )
    ...
    async def _analyze_decision_lessons(
        self, decision_id: int, profit_loss: float, execution_result: Dict[str, Any]
    ) -> str:
        ...
        try:
            ...
            if profit_loss > 0:
                if confidence > 0.8:
                    ...
                    lessons.append(
                        "Low confidence decision succeeded - may have been overly cautious"
                    )
                ...
                if ai_confidence and ai_confidence > 0.8:
                    lessons.append(
                        "AI was confident but wrong - check market conditions vs AI analysis"
                    )
            ...
            if expected_value != 0:
                ...
                if value_error > 0.5:
                    lessons.append(
                        f"Value prediction was off by {value_error:.1%} - improve value estimation"
                    )
            ...
            if market_volatility > 0.05:  # High volatility
                if profit_loss > 0:
                    lessons.append(
                        "Decision succeeded despite high volatility - good risk management"
                    )
                else:
        ...
        try:
            ...
            if not results:
                return PerformanceMetrics(
                    ...
                    best_decision_profit=0,
                    worst_decision_loss=0,
                    avg_confidence_successful=0,
                    avg_confidence_failed=0,
                )
            ...
            avg_confidence_failed = (
                sum(failed_confidences) / len(failed_confidences) if failed_confidences else 0
            )
    ...
    async def get_decision_patterns(self) -> Dict[str, Any]:
        ...
        try:
            ...
            confidence_patterns = {}
            for row in cursor.fetchall():
                confidence_patterns[row[0]] = {"success_rate": row[1], "count": row[2]}
