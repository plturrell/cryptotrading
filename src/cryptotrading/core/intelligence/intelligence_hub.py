...
from ...data.database.intelligence_schema import IntelligenceType, DecisionStatus
...
class IntelligenceHub:
    ...
    async def analyze_and_decide(self, context: IntelligenceContext) -> CombinedIntelligence:
        ...
        mcts_decision = await self._make_mcts_decision(context, ai_insights, historical_intelligence)
        ...
        combined = await self._combine_intelligence(context, ai_insights, mcts_decision, ml_predictions)
    ...
    async def _store_ai_insight(self, insight: MarketInsight, context: IntelligenceContext):
        ...
        try:
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                insight.analysis_type.value, insight.symbol, insight.recommendation,
                insight.confidence, insight.score, insight.risk_level,
                insight.reasoning, "grok4", context.timestamp, context.session_id
            ))
            self.db.db_conn.commit()
    ...
    async def _get_historical_intelligence(self, context: IntelligenceContext) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_success_rate = len(successful_decisions) / len(historical_data) if historical_data else 0
    ...
    async def _make_mcts_decision(self, context: IntelligenceContext, 
                                ai_insights: List[MarketInsight],
                                historical_intelligence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if ai_insights:
                ...
                ai_sentiment_score = sum(insight.score for insight in ai_insights) / len(ai_insights)
            ...
            if not self.mcts_agent:
                from ..agents.specialized.mcts_real_implementation import RealMCTSConfig, RealProductionMCTSAgent
            ...
            decision = await self.mcts_agent.calculate_optimal_action(
                context.portfolio, 
                {context.symbol: enhanced_market_data}
            )
        ...
        try:
            ...
            if self.db.config.mode.value == "local":
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    "mcts_enhanced", decision.get('type'), context.symbol,
                    decision.get('amount', 0), decision.get('confidence', 0),
                    decision.get('mcts_value', 0), decision.get('reason', ''),
                    "mcts_with_ai", parent_insight_id, context.session_id, context.timestamp
                ))
            else:
                cursor.execute("""
                    INSERT INTO trading_decisions 
    ...
    async def _get_ml_predictions(self, context: IntelligenceContext) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            from ..ml.inference import inference_service, PredictionRequest
            ...
            if prediction:
                ...
                logger.info(f"Stored ML prediction for {context.symbol}: {prediction.price_change_percent:.2f}% change predicted")
        ...
        try:
            if self.db.config.mode.value == "local":
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    prediction.model_type, prediction.symbol, "price_prediction",
                    prediction.predicted_price, prediction.confidence,
                    prediction.horizon, str(prediction.features_used), prediction.model_version,
                    context.timestamp, context.session_id
                ))
            else:
                cursor.execute("""
                    INSERT INTO ml_predictions 
    ...
    async def _combine_intelligence(self, context: IntelligenceContext,
                                  ai_insights: List[MarketInsight],
                                  mcts_decision: Dict[str, Any],
                                  ml_predictions: Optional[Dict[str, Any]]) -> CombinedIntelligence:
        ...
        risk_assessment = {
            "overall_risk": "HIGH" if any("HIGH" in insight.risk_level for insight in ai_insights) else "MEDIUM",
    ...
    async def _store_combined_intelligence(self, context: IntelligenceContext, 
                                         combined: CombinedIntelligence):
    ...
    async def _store_conversation(self, context: IntelligenceContext, 
                                combined: CombinedIntelligence):
        ...
        try:
            ...
            if self.db.config.mode.value == "local":
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    context.session_id, self.agent_id, "query",
                    f"Analyze {context.symbol}", json.dumps(asdict(context)),
                    context.timestamp
                ))
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    context.session_id, self.agent_id, "response",
                    f"Recommendation: {combined.final_recommendation} (confidence: {combined.confidence:.2f})",
                    json.dumps(asdict(combined)), context.timestamp
                ))
            else:
                cursor.execute("""
                    INSERT INTO conversation_history 
    ...
    async def _learn_from_decision(self, context: IntelligenceContext, 
                                 combined: CombinedIntelligence):
        """Learn from this decision for future improvements"""
        # Extract key learnings
        learnings = {
            "symbol_analyzed": context.symbol,
            "ai_consensus": combined.ai_insights[0].recommendation if combined.ai_insights else None,
