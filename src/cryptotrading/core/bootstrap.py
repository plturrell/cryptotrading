...
from .interfaces import (
    ILogger, IMetricsCollector, IHealthChecker, IConfigProvider,
    IServiceRegistry, ICache, ISecurityManager, ICommunicationManager
)
from .infrastructure import (
    EnterpriseLogger, EnterpriseMetricsCollector, EnterpriseHealthChecker,
    EnterpriseConfigProvider, EnterpriseServiceRegistry, EnterpriseInMemoryCache,
    SimpleSecurityManager, SimpleCommunicationManager
)
...
class ApplicationBootstrap:
    ...
    async def _register_infrastructure_services(self):
        ...
        if config_file:
            self.container.register_singleton(
                IConfigProvider,
                factory=lambda: EnterpriseConfigProvider(config_file)
            )
        else:
    ...
    async def _register_business_services(self):
        ...
        try:
            ...
            from .agents.modular_strands_agent import ModularStrandsAgent
            ...
            if system_health['overall_status'] != 'healthy':
                ...
                for service_name, health in system_health['services'].items():
                    if not health.get('healthy', False):
                        logger.warning(f"Unhealthy service: {service_name} - {health.get('error', 'Unknown error')}")
