...
class EnhancedDataIngestionOrchestrator:
    ...
    async def ingest_historical_data_comprehensive(
        ...
        try:
            ...
            await self._run_final_quality_checks(workflow_id, config)
    ...
    async def _validate_ingestion_config(self, config: IngestionConfig):
        ...
        logger.info(f"Configuration validated: {len(config.symbols)} symbols, "
    ...
    async def _execute_job_plan(
        ...
        # Check for failures
        failed_jobs = [
            (job, result) for job, result in zip(jobs, results) 
    ...
    async def _execute_single_job(
        ...
        try:
            ...
            await self._calculate_basic_factors(job, config.factors)
    ...
    async def _fetch_data_from_source(
        ...
        if source == DataSourceEnum.BINANCE:
            ...
            return await self._fetch_generic_data(source, symbol, start_date, end_date)
    ...
    async def _fetch_generic_data(
        ...
        """Generic data fetcher for other sources"""
    ...
    async def _calculate_basic_factors(
        self, 
        job: DataIngestionJob, 
    ...
    async def _calculate_derived_factors(
        self, 
        workflow_id: str, 
        config: IngestionConfig
    ):
    ...
    async def _calculate_single_derived_factor(
        self, 
        factor: Factor, 
        config: IngestionConfig
    ):
    ...
    async def _run_final_quality_checks(
        self, 
        workflow_id: str, 
        config: IngestionConfig
    ):
        ...
        for symbol in config.symbols:
            ...
            if quality_report['overall_score'] < config.quality_threshold:
                logger.warning(f"Quality threshold not met for {symbol}: "
                             f"{quality_report['overall_score']}")
