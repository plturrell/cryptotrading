...
class MonitoringIntegration:
    ...
    def counter(self, name: str, value: int = 1, labels: Optional[Dict[str, str]] = None):
        ...
        if self._started:
            asyncio.create_task(self.persistence.record_counter(name, value, labels))

    def gauge(self, name: str, value: float, labels: Optional[Dict[str, str]] = None):
        """Record gauge metric (async in background)"""
        if self._started:
            asyncio.create_task(self.persistence.record_gauge(name, value, labels))

    def histogram(self, name: str, value: float, labels: Optional[Dict[str, str]] = None):
        """Record histogram metric (async in background)"""
        if self._started:
            asyncio.create_task(self.persistence.record_histogram(name, value, labels))
