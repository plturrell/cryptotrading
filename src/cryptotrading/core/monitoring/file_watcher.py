...
class CodeFileHandler(FileSystemEventHandler if WATCHDOG_AVAILABLE else object):
    ...
    def on_modified(self, event):
        ...
        if not event.is_directory and self.should_process_file(event.src_path):
            change = FileChange(file_path=event.src_path, change_type=ChangeType.MODIFIED)
            self._schedule_async_task(change)

    def on_created(self, event):
        """Handle file creation"""
        if not event.is_directory and self.should_process_file(event.src_path):
            change = FileChange(file_path=event.src_path, change_type=ChangeType.CREATED)
            self._schedule_async_task(change)

    def on_deleted(self, event):
        """Handle file deletion"""
        if not event.is_directory and self.should_process_file(event.src_path):
            change = FileChange(file_path=event.src_path, change_type=ChangeType.DELETED)
            self._schedule_async_task(change)
    ...
    def _schedule_async_task(self, change: FileChange):
        ...
        try:
            ...
            asyncio.run_coroutine_threadsafe(self.file_watcher.handle_file_change(change), loop)
        except RuntimeError:
            ...
            if hasattr(self.file_watcher, "pending_changes"):
                ...
                logger.warning(
                    f"No event loop available to process file change: {change.file_path}"
                )
...
class AnalysisQueue:
    ...
    def __init__(self, max_size: int = 100):
        ...
        self.stats = {"queued": 0, "processed": 0, "failed": 0, "avg_processing_time": 0.0}
...
class RealtimeFileWatcher:
    ...
    async def _perform_file_analysis(self, change: FileChange) -> AnalysisResult:
        ...
        result = AnalysisResult(file_path=change.file_path, analysis_type="incremental")
            ...
            if self.enable_ai_analysis and self.grok_client:
                try:
                    ...
                    ai_result = await self.grok_client.analyze_code_structure(
                        analysis_data, focus="real_time_change"
                    )
...
async def create_file_watcher(
    watch_directories: List[str], project_root: str = None, enable_ai: bool = True
) -> RealtimeFileWatcher:
    ...
    if await watcher.start_watching():
        ...
        try:
            while True:
                ...
                print(
                    f"📊 Stats: {stats['changes_detected']} changes, {stats['analyses_completed']} analyses"
                )
