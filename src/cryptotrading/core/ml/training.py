...
class ModelTrainingPipeline:
        ...
        with monitor.span(f"fetch_training_data_{symbol}") as span:
            try:
                ...
                if data is None or len(data) < self.training_config["min_data_points"]:
                    logger.warning(
                        f"Insufficient data for {symbol}: {len(data) if data else 0} points"
                    )
                    return None

                monitor.increment_counter(
                    "ml.training.data_fetched", 1, {"symbol": symbol, "points": str(len(data))}
                )
                ...
                monitor.increment_counter(
                    "ml.training.fetch_error", 1, {"symbol": symbol, "error": str(e)}
                )
                return None

    async def train_model(
        self, symbol: str, model_type: str, horizon_hours: int
        ...
        with monitor.span(f"train_model_{symbol}_{model_type}_{horizon_hours}h") as span:
            try:
                ...
                # Train model
                logger.info(
                    f"Training {model_type} model for {symbol} with {horizon_hours}h horizon"
                )
                ...
                monitor.record_histogram(
                    "ml.training.time_seconds", training_time, {"model_type": model_type}
                )

                logger.info(
                    f"Model trained successfully: {model_name} - R²: {metrics['r2']:.4f}, MAPE: {metrics['mape']:.2f}%"
                )
    ...
    async def train_all_models(self):
        ...
        logger.info(
            f"Training completed: {len(successful)}/{len(tasks)} models trained successfully"
        )
        ...
        try:
            ...
            metrics = {
                ...
                "direction_accuracy": float(
                    np.mean((predictions[1:] > predictions[:-1]) == (actuals[1:] > actuals[:-1]))
                ),
