...
class VercelBlobModelStorage:
    ...
    async def upload_model(self, model_id: str, version: str, model_data: bytes, 
        ...
        try:
            ...
            if self.is_production and self.blob_url:
                ...
                model_path = os.path.join(self.local_storage_path, f"{model_id}_{version}_model.pkl")
                metadata_path = os.path.join(self.local_storage_path, f"{model_id}_{version}_metadata.json")
                            ...
                            if len(parts) >= 2:
                                model_info = {
                                    ...
                                    'uploaded_at': datetime.fromtimestamp(
                                        os.path.getmtime(os.path.join(self.local_storage_path, filename))
                                    ).isoformat()
                                }
...
class ModelVersionManager:
    ...
    async def promote_version(self, model_id: str, version: str) -> bool:
        ...
        try:
            ...
            await self.storage.upload_model(
                model_id, 
                prod_version, 
                model_data, 
                metadata
