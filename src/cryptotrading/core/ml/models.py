...
class CryptoPricePredictor:
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["support_resistance_ratio"] = (df["close"] - df["support"]) / (
    ...
    def prepare_data(
        self, df: pd.DataFrame, target_hours: int = 24
    ...
    def _get_top_features(self, n: int = 5) -> List[Dict[str, float]]:
        ...
        return [
            {"name": name, "importance": float(importance)} for name, importance in sorted_features
        ]
    ...
    async def load(self):
        ...
        try:
            ...
            model_data = await registry.load_model(
                f"crypto_predictor_{self.model_type}", self.version
            )
