...
class ProductionCryptoPricePredictor:
    ...
    def _get_xgboost_config(self) -> Dict[str, Any]:
        ...
        model = xgb.XGBRegressor(random_state=42, n_jobs=-1, tree_method="hist")  # Faster training
    ...
    def _get_et_config(self) -> Dict[str, Any]:
        ...
        model = ExtraTreesRegressor(random_state=42, n_jobs=-1, bootstrap=True, oob_score=True)
    ...
    def optimize_hyperparameters(
        self, X: np.ndarray, y: np.ndarray, model_name: str, max_evals: int = 50
    ) -> Dict[str, Any]:
        """
        ...
        for i, params in enumerate(param_combinations):
            try:
                ...
                if i % 10 == 0:
                    logger.info(
                        f"Evaluated {i+1}/{len(param_combinations)} combinations. Best score: {best_score:.4f}"
                    )
    ...
    def train_advanced_ensemble(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        self.optimized_params = {
            name: result["best_params"] for name, result in model_results.items()
        }
        self.model_performance = {
            name: result["best_score"] for name, result in model_results.items()
        }
    ...
    def _calculate_ensemble_weights(self, model_results: Dict[str, Any]) -> Dict[str, float]:
        ...
        combined_weights = (
            0.4 * stacking_weights
            + 0.3 * performance_weights  # Meta-learner predictions
            + 0.2 * diversity_weights  # Raw performance
            + 0.1 * stability_weights  # Model diversity  # Cross-validation stability
        )
    ...
    def _calculate_stacking_weights(self, model_results: Dict[str, Any]) -> np.ndarray:
        ...
        try:
            ...
            return self._calculate_performance_weights(
                np.array([model_results[name]["best_score"] for name in model_names])
            )
    ...
    def predict_ensemble(self, X: np.ndarray) -> np.ndarray:
        ...
        if np.any(outlier_mask):
            logger.debug(
                f"Detected {np.sum(outlier_mask)} outlier predictions, using robust median"
            )
    ...
    def _calculate_dynamic_weights(
        ...
        combined_dynamic = (
            0.4 * consensus_weights
            + 0.4 * confidence_weights  # Consensus alignment
            + 0.2 * volatility_weights  # Model confidence  # Prediction stability
        )
