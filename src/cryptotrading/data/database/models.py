...
from sqlalchemy import Column, Integer, String, Float, DateTime, Boolean, Text, ForeignKey, Index, Enum, JSON
...
class MacroData(Base):
    ...
    __table_args__ = (
        Index('idx_symbol_timestamp_macro', 'symbol', 'timestamp'),
    )
...
class DataIngestionJob(Base):
    ...
    job_type = Column(String(50), nullable=False)  # 'historical_backfill', 'real_time', 'factor_calculation'
    symbol = Column(String(20), nullable=False, index=True)
    source = Column(Enum(DataSourceEnum), nullable=False)
