...
class ErrorMessageBuilder:
    ...
    @classmethod
    def build_message(cls, error_code: ErrorCode, context: Optional[Dict[str, Any]] = None) -> str:
...
class ErrorHandler:
    ...
    def handle_error(
        self, error: Exception, operation: str = "unknown", context: Optional[Dict[str, Any]] = None
    ) -> DatabaseError:
        ...
        db_error = db_error_class(
            message=message, error_code=error_code, details=context or {}, original_error=error
        )
    ...
    def _classify_error(self, error: Exception) -> tuple:
        ...
        if any(
            ...
            for keyword in ["validation", "constraint", "unique", "foreign key"]
        ):
...
def format_user_friendly_error(error: DatabaseError) -> str:
    ...
    return user_friendly_messages.get(
        error.error_code, "An unexpected error occurred. Please contact support if this continues."
    )
