...
class DatabaseMigrator:
        ...
        for migration_record in to_rollback:
            ...
            try:
                ...
                migration = next((m for m in self.migrations 
                                if m.version == migration_record['version']), None)
    ...
    def validate_migrations(self) -> Dict[str, Any]:
        ...
        for record in applied:
            migration = next((m for m in self.migrations 
                            if m.version == record['version']), None)
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'applied_migrations': [{'version': m['version'], 'name': m['name']} 
                                 for m in applied],
            'pending_migrations': [{'version': m.version, 'name': m.name} 
                                 for m in pending]
        }
