...
class ConstraintEnforcer:
    ...
    def check_constraints(self, table: str, data: Dict[str, Any], 
                         operation: str = 'insert') -> List[str]:
    ...
    def enforce_referential_integrity(self, table: str, foreign_keys: Dict[str, str], 
...
class DataQualityMonitor:
            ...
            with self.db_client.engine.connect() as conn:
                ...
                for field in required_fields:
                    ...
                    if null_count > 0:
                        ...
                        issues.append(
                            f"{field}: {null_count} null values ({null_percent:.1f}%)"
                        )
    ...
    def _check_orphaned_records(self) -> Dict[str, int]:
        ...
        checks = [
            ...
            ('portfolios', 'SELECT COUNT(*) FROM portfolios WHERE user_id NOT IN (SELECT id FROM users)'),
            ('conversation_messages', 'SELECT COUNT(*) FROM conversation_messages WHERE session_id NOT IN (SELECT session_id FROM conversation_sessions)'),
        ]
