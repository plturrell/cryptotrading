...
class DatabaseHealthMonitor:
    ...
    def _check_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": HealthStatus.HEALTHY, "message": "Connection successful"}
        except Exception as e:
            return {"status": HealthStatus.UNHEALTHY, "message": f"Connection failed: {str(e)}"}
    ...
    def _check_connection_pool(self) -> Dict[str, Any]:
        ...
        try:
            ...
            utilization = (
                (active_connections / total_connections * 100) if total_connections > 0 else 0
            )
            ...
            return {"status": HealthStatus.UNHEALTHY, "message": f"Pool check failed: {str(e)}"}

    def _check_disk_space(self) -> Dict[str, Any]:
        """Check disk space for SQLite databases"""
        if not self.db_client.is_sqlite:
            return {"status": HealthStatus.HEALTHY, "message": "Not applicable for PostgreSQL"}
        ...
        try:
            ...
            if not os.path.exists(db_path):
                return {"status": HealthStatus.UNHEALTHY, "message": "Database file not found"}
    ...
    def _check_table_sizes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            large_tables = [
                t for t, count in table_stats.items() if isinstance(count, int) and count > 1000000
            ]
            ...
            return {"status": status, "message": message, "metrics": {"table_stats": table_stats}}
    ...
    def _check_replication_lag(self) -> Dict[str, Any]:
        ...
        if self.db_client.is_sqlite:
            return {"status": HealthStatus.HEALTHY, "message": "Not applicable for SQLite"}
        ...
        try:
            ...
            with self.db_client.engine.connect() as conn:
                ...
                if not result:
                    return {"status": HealthStatus.HEALTHY, "message": "No replication configured"}
            ...
            return {"status": HealthStatus.HEALTHY, "message": "Replication check not available"}
    ...
    def _check_lock_contention(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": HealthStatus.HEALTHY, "message": "Lock check not available"}
    ...
    def _check_status_change(self, results: Dict[str, Any]):
        ...
        previous_status = (
            self.last_check_results.get("overall_status") if self.last_check_results else None
        )
        ...
        if previous_status and previous_status != current_status:
            if current_status == HealthStatus.UNHEALTHY:
                ...
                current_status == HealthStatus.HEALTHY and previous_status == HealthStatus.UNHEALTHY
            ):
    ...
    def resilient_connection(self):
        ...
        while retry_count < self.connection_retry_config["max_retries"]:
            try:
                ...
                logger.warning(
                    f"Connection failed (attempt {retry_count}): {e}. Retrying in {delay}s..."
                )
                time.sleep(delay)
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "healthy_checks": sum(
                1
                for c in self.last_check_results["checks"].values()
