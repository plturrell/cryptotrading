...
class QueryOptimizer:
    ...
    def analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        if re.search(r"LIKE\s+'%[^']+", query, re.IGNORECASE):
            ...
            analysis['suggestions'].append('Leading wildcards prevent index usage, consider full-text search')
        
        # Check for OR conditions
        or_count = len(re.findall(r'\sOR\s', query, re.IGNORECASE))
        if or_count > 3:
            analysis['issues'].append(f'Multiple OR conditions ({or_count})')
            analysis['suggestions'].append('Consider using IN clause or UNION for better performance')
    ...
    def _has_index(self, table: str, column: str) -> bool:
        ...
        if self.db_client.is_sqlite:
            query = "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name=? AND sql LIKE ?"
    ...
    def _log_query_performance(self, query: str, query_hash: str, 
                             execution_time_ms: int, params: Optional[Dict] = None):
    ...
    def get_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            with self.db_client.engine.connect() as conn:
                ...
                result = conn.execute(slow_query_sql, 
                                    (since, self.slow_query_threshold_ms))
        ...
        for query_hash, stats in sorted(self.query_stats.items(), 
        ...
        for slow_query in report['slow_queries']:
            ...
            if analysis['issues']:
                report['optimization_opportunities'].append({
                    ...
                    'potential_improvement': 'High' if len(analysis['issues']) > 2 else 'Medium'
                })
