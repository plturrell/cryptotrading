...
class A2AHistoricalDataLoader:
    ...
    def _create_tools(self) -> List[ToolSpec]:
        ...
        def align_temporal_data(
            data_sources: Dict[str, Any], frequency: str = "D"
        ) -> Dict[str, Any]:
    ...
    async def load_comprehensive_data(self, request: DataLoadRequest) -> Dict[str, Any]:
        ...
        if "yahoo" in request.sources:
            ...
            tool_calls.append(
                f"load_yahoo_data({symbols}, '{request.start_date}', '{request.end_date}')"
            )
        ...
        if "fred" in request.sources:
            ...
            tool_calls.append(
                f"load_fred_data({series}, '{request.start_date}', '{request.end_date}')"
            )

        prompt_parts.extend(["", "Execute the following data loading operations:", *tool_calls])
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "sources_requested": request.sources}

    def load_crypto_trading_dataset(
    ...
    def get_data_summary(self) -> Dict[str, Any]:
        ...
        summary = {"yahoo_finance": {}, "fred": {}, "total_files": 0}
