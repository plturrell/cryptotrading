...
class VercelBlobClient:
    ...
    def put(
        ...
        try:
            ...
            response = self.session.post(f"{self.base_url}/api/put", json=payload, timeout=30)
    ...
    def list(self, prefix: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.session.get(f"{self.base_url}/api/list", params=params, timeout=30)
            response.raise_for_status()
    ...
    def delete(self, urls: Union[str, List[str]]) -> bool:
        ...
        try:
            ...
            response = self.session.post(f"{self.base_url}/api/delete", json=payload, timeout=30)
            response.raise_for_status()
    ...
    def get_analysis_history(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        for blob in blobs:
            try:
                ...
                logger.warning(
                    f"Could not parse timestamp from blob {blob.get('pathname', 'unknown')}: {e}"
                )
                continue
    ...
    def purge_expired_blobs(self, prefix: str, days_to_keep: int = 30) -> int:
        ...
        for blob in blobs:
            try:
                ...
                if uploaded_at:
                    blob_time = datetime.fromisoformat(
                ...
                logger = logging.getLogger(__name__)
                logger.warning(
                    f"Could not parse upload date from blob {blob.get('pathname', 'unknown')}: {e}"
                )
                continue
...
def put_json_blob(
    pathname: str, data: Dict, access: str = "public", token: Optional[str] = None
) -> Dict[str, str]:
    """
