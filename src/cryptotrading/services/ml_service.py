...
from ..core.ml.inference import BatchPredictionRequest, PredictionRequest, inference_service
...
class MLModelRegistry:
    ...
    def get_best_model(self, model_name: str, metric: str = "accuracy") -> Optional[Dict[str, Any]]:
        ...
        matching_models = [
            (key, meta) for key, meta in self.model_metadata.items() if meta["name"] == model_name
        ]
...
class AutoMLRetrainer:
    ...
    async def schedule_retraining(self, symbol: str, model_type: str, schedule: str = "daily"):
    ...
    async def check_and_retrain(self) -> List[str]:
        ...
        for key, schedule_info in self.retraining_schedule.items():
            if datetime.utcnow() >= schedule_info["next_retrain"]:
                try:
                    await self._retrain_model(schedule_info["symbol"], schedule_info["model_type"])
...
class EnhancedMLService:
    ...
    async def get_prediction(
        self, symbol: str, horizon: str = "24h", model_type: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def get_batch_predictions(
        self, symbols: List[str], horizon: str = "24h", model_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        with trace_context("ml_batch_prediction") as span:
            try:
                ...
                if model_type is None:
                    ...
                    tasks = [self._get_ensemble_prediction(symbol, horizon) for symbol in symbols]
    ...
    async def trigger_training(
        self, symbols: Optional[List[str]] = None, model_types: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        with trace_context("ml_training") as span:
            try:
                ...
                return {
                    ...
                    or training_pipeline.training_config.get("model_types", []),
                    "duration_ms": duration_ms,
    ...
    async def get_feature_importance(
        self, symbol: str, model_type: str = "xgboost"
    ) -> Dict[str, Any]:
