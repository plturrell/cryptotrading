...
class AnalyticsService:
    ...
    async def get_market_analytics(self, symbol: str) -> Dict[str, Any]:
        ...
        with trace_context(f"analytics_{symbol}") as span:
            try:
                ...
                support_resistance = technical_analysis.get('indicators', {}).get('support_resistance', {})
    ...
    def _calculate_performance_metrics(self, price_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(prices) >= 90:  # 3 months
            ...
            performance['return_3m'] = float((current_price - quarter_ago_price) / quarter_ago_price)
        ...
        if returns:
            ...
            if np.std(returns) > 0:
                performance['sharpe_ratio'] = float(np.mean(returns) / np.std(returns) * np.sqrt(252))
        ...
        with trace_context("correlation_analysis") as span:
            try:
                ...
                for symbol, data in symbol_data.items():
                    prices = [float(item.get('Close', item.get('price', 0))) 
    ...
    def _get_top_performers(self, market_data: Dict[str, Any], 
                           metric: str, count: int) -> List[Dict[str, Any]]:
