...
class RateLimiter:
    ...
    def __init__(self):
        self.limits = {
            # API limits per minute - only real APIs
            "yahoo": {"calls": 2000, "period": 3600},  # 2000/hour (unofficial)
            "perplexity": {"calls": 20, "period": 60},  # 20 calls/min
            "grok4": {"calls": 60, "period": 60},  # Grok-4 rate limit
        }
    ...
    def check_limit(self, api_name: str) -> bool:
        ...
        with self.lock:
            ...
            self.calls[api_name] = [
                call_time for call_time in self.calls[api_name] if now - call_time < limit["period"]
            ]
        ...
        with self.lock:
            ...
            if len(self.calls[api_name]) >= limit["calls"]:
                ...
                if wait_time > 0:
                    ...
                    logger.warning(
                        f"Rate limit reached for {api_name}. Waiting {wait_time:.1f}s..."
                    )
    ...
    def get_remaining_calls(self, api_name: str) -> Dict[str, int]:
        ...
        with self.lock:
            ...
            return {
                ...
                "period": limit["period"],
                "reset_in": min([limit["period"] - (now - call) for call in recent_calls])
                if recent_calls
                else 0,
            }
