...
class PersistentAgentRegistry:
    ...
    def get_agent(self, agent_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self.db.get_session() as session:
            agent = session.query(A2AAgent).filter(
                A2AAgent.agent_id == agent_id,
    ...
    def find_agents_by_capability(self, capability: str) -> List[str]:
        ...
        with self.db.get_session() as session:
            agents = session.query(A2AAgent).filter(
                A2AAgent.is_active == True,
    ...
    def get_all_agents(self) -> Dict[str, Dict[str, Any]]:
        ...
        agents = {}
        with self.db.get_session() as session:
            db_agents = session.query(A2AAgent).filter(
                A2AAgent.is_active == True
            ).all()
    ...
    def establish_connection(self, agent1_id: str, agent2_id: str, protocol: str) -> bool:
        ...
        try:
            ...
            with self.db.get_session() as session:
                ...
                existing = session.query(A2AConnection).filter(
                    A2AConnection.connection_id == connection_id
                ).first()
    ...
    def get_agent_connections(self, agent_id: str) -> List[Dict[str, Any]]:
        ...
        with self.db.get_session() as session:
            db_connections = session.query(A2AConnection).filter(
                (A2AConnection.agent1_id == agent_id) | 
                (A2AConnection.agent2_id == agent_id),
...
class PersistentWorkflowRegistry:
    ...
    def get_workflow(self, workflow_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self.db.get_session() as session:
            workflow = session.query(A2AWorkflow).filter(
                A2AWorkflow.workflow_id == workflow_id,
                A2AWorkflow.is_active == True
            ).first()
    ...
    def register_workflow(
        ...
        try:
            with self.db.get_session() as session:
                existing = session.query(A2AWorkflow).filter(
                    A2AWorkflow.workflow_id == workflow_id
                ).first()
    ...
    def list_workflows(self) -> List[Dict[str, str]]:
        ...
        workflows = []
        with self.db.get_session() as session:
            db_workflows = session.query(A2AWorkflow).filter(
                A2AWorkflow.is_active == True
            ).all()
    ...
    def update_execution_status(
        ...
        try:
            with self.db.get_session() as session:
                execution = session.query(A2AWorkflowExecution).filter(
                    A2AWorkflowExecution.execution_id == execution_id
                ).first()
