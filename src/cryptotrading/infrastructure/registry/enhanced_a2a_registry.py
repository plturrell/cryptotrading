...
class EnhancedA2ARegistry:
    ...
    def _get_default_mcp_tools(self, agent_id: str) -> List[str]:
        ...
        tool_mappings = {
            ...
            "transform-001": [
                "preprocess_data", "validate_data", "calculate_features"
            ],
            "illuminate-001": [
                "analyze_market_sentiment", "get_price_alerts", "generate_insights"
            ],
            "execute-001": [
                "execute_trade", "manage_orders", "assess_risk"
            ]
        }
    ...
    async def get_registry_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self.db.get_session() as session:
                ...
                active_agents = session.query(A2AAgent).filter(
                    A2AAgent.is_active == True
                ).count()
                ...
                return {
                    ...
                    'healthy_agents': len([h for h in self.health_status.values() 
                                         if h.status == AgentStatus.ACTIVE]),
