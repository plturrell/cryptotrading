...
class PerplexityNewsService:
    ...
    async def translate_articles_to_russian(self, articles: List[NewsArticle]) -> List[NewsArticle]:
        ...
        for article in articles:
            try:
                ...
                translated_title = await self.translator.translate_to_russian(
                    article.title, self.session
                )
                translated_content = await self.translator.translate_to_russian(
                    article.content, self.session
                )
    ...
    async def get_news_by_category_russian(
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        "Russian category news API error %d: %s", response.status, error_text
                    )
                    return []
    ...
    async def enhance_articles_with_images(self, articles: List[NewsArticle]) -> List[NewsArticle]:
        ...
        try:
            ...
            for article in articles:
                ...
                images = await self.image_enhancer.enhance_article_with_images(
                    article, max_images=5
                )
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        "Russian symbol news API error %d: %s", response.status, error_text
                    )
                    return []
...
async def get_crypto_news_russian(
    category: str = "general", limit: int = 10, api_key: str = None
) -> List[Dict[str, Any]]:
