...
class AWSDataExchangeService:
    ...
    def __init__(self, region_name: str = "us-east-1"):
        ...
        try:
            ...
            self.temp_bucket = os.getenv('AWS_DATA_EXCHANGE_BUCKET', 'cryptotrading-dataexchange-temp')
    ...
    def discover_financial_datasets(self) -> List[DataExchangeDataset]:
        ...
        try:
            ...
            financial_keywords = [
                'financial', 'market', 'economic', 'trading', 'price', 
                'crypto', 'stock', 'forex', 'commodity', 'derivatives'
            ]
            ...
            for page in paginator.paginate():
                for dataset in page['DataSets']:
                    ...
                    is_financial = any(keyword in name or keyword in description 
                                     for keyword in financial_keywords)
                    
                    if is_financial:
                        datasets.append(DataExchangeDataset(
                            dataset_id=dataset['Id'],
                            name=dataset['Name'],
                            description=dataset.get('Description', ''),
                            provider=dataset['Origin'],
                            categories=[], # Will be populated from tags if available
                            last_updated=datetime.fromisoformat(dataset['UpdatedAt'].replace('Z', '+00:00')),
        ...
        try:
            ...
            for page in asset_paginator.paginate(DataSetId=dataset_id, RevisionId=revision_id):
                for asset in page['Assets']:
                    assets.append(DataExchangeAsset(
                        asset_id=asset['Id'],
                        name=asset['Name'],
                        dataset_id=dataset_id,
                        file_format=asset.get('AssetType', 'unknown'),
                        size_bytes=asset.get('AssetDetails', {}).get('S3SnapshotAsset', {}).get('Size', 0),
    ...
    def get_available_economic_datasets(self) -> List[Dict[str, Any]]:
        ...
        econ_keywords = ['economic', 'gdp', 'inflation', 'employment', 'interest rate', 'fed', 'monetary']
    ...
    def load_dataset_to_database(self, dataset_id: str, asset_id: str, table_name: str) -> Dict[str, Any]:
