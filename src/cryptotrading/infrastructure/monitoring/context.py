...
class TraceContext:
    ...
    def create_child_context(
        self, operation: Optional[str] = None, agent_id: Optional[str] = None
    ) -> "TraceContext":
...
class A2AContextEnhancer:
    ...
    @staticmethod
    def enhance_message(
        message: Dict[str, Any], context: Optional[TraceContext] = None
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def create_child_from_message(
        message: Dict[str, Any], operation: str, agent_id: str
    ) -> TraceContext:
        """Create child context from incoming A2A message"""
        parent_context = A2AContextEnhancer.extract_context(message)

        if parent_context:
            return parent_context.create_child_context(operation=operation, agent_id=agent_id)
        else:
            # Create new trace if no parent context
            return create_trace_context(
                operation=operation, agent_id=agent_id, metadata={"source": "a2a_message"}
            )
...
class WorkflowContext:
    ...
    def create_step_context(self, step_name: str, agent_id: Optional[str] = None) -> TraceContext:
        """Create context for workflow step"""
        step_context = self.root_context.create_child_context(
            operation=f"step:{step_name}", agent_id=agent_id
        )
