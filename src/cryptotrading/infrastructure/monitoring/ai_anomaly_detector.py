...
class AIAnomalyDetector:
    ...
    def _detect_statistical_anomaly(
        self, metric_name: str, value: float, history: List[Dict]
    ) -> Optional[AnomalyEvent]:
    ...
    def _detect_time_series_anomaly(
        self, metric_name: str, value: float, history: List[Dict]
    ) -> Optional[AnomalyEvent]:
    ...
    def _detect_ml_anomaly(
        ...
        try:
            ...
            now = datetime.now()
            current_features = [
                [value, now.hour, now.weekday(), value - history[-1]["value"] if history else 0]
            ]
    ...
    def _detect_business_anomaly(
        ...
        business_rules = {
            "api_response_time": lambda v: ("HIGH", "API response time too high")
            if v > 5000
            else None,
            "error_rate": lambda v: ("CRITICAL", "Error rate critically high")
            if v > 0.1
            else ("HIGH", "Error rate elevated")
            if v > 0.05
            else None,
            "memory_usage": lambda v: ("CRITICAL", "Memory usage critical")
            if v > 0.9
            else ("HIGH", "Memory usage high")
            if v > 0.8
            else None,
            "cpu_usage": lambda v: ("HIGH", "CPU usage high")
            if v > 0.9
            else ("MEDIUM", "CPU usage elevated")
            if v > 0.8
            else None,
            "trade_volume": lambda v: ("MEDIUM", "Unusually low trade volume")
            if v < 1000
            else None,
            "active_connections": lambda v: ("HIGH", "Too many active connections")
            if v > 1000
            else None,
            "disk_usage": lambda v: ("CRITICAL", "Disk space critical")
            if v > 0.95
            else ("HIGH", "Disk space low")
            if v > 0.85
            else None,
        }
    ...
    def _get_suggested_action(self, metric_name: str, category: str) -> str:
        ...
        suggestions = {
            (
                "api_response_time",
                "statistical",
            ): "Check API performance, database queries, and network latency",
            ...
            (
                "error_rate",
                "business_logic",
            ): "Immediate investigation required - system may be failing",
            ...
            (
                "cpu_usage",
                "business_logic",
            ): "Scale up CPU resources or optimize workload distribution",
        }

        key = (metric_name, category)
        return suggestions.get(
            key, f"Investigate {metric_name} anomaly - check logs and system resources"
        )
