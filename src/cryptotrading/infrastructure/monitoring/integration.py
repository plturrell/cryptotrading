...
from .error_tracker import ErrorCategory, ErrorSeverity, get_error_tracker, track_errors
from .logger import get_logger, log_function_call
from .metrics import get_business_metrics, get_metrics
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            if current_ctx:
                tool_ctx = current_ctx.create_child_context(
                    operation=f"tool:{tool_name}", agent_id=self.agent_id
                )
            else:
    ...
    def process_a2a_message_with_observability(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with with_trace_context(context):
            ...
            try:
                ...
                self.logger.error(
                    f"A2A message processing failed",
                    error=e,
                    extra={
                        "message_type": message.get("type"),
                        "sender_id": message.get("sender_id"),
...
class ObservableWorkflow:
    ...
    async def execute_step(
        self, step_name: str, agent_id: str, operation: Callable, *args, **kwargs
    ) -> Any:
        ...
        with with_trace_context(step_context):
            ...
            try:
                self.logger.info(
                    f"Starting workflow step: {step_name}",
                    extra={
                ...
                self.logger.error(
                    f"Workflow step failed: {step_name}",
                    error=e,
                    extra={
                        "workflow_id": self.workflow_context.workflow_id,
                        "step_name": step_name,
...
def observable_agent_method(agent_id: str, operation_name: Optional[str] = None):
    ...
    def decorator(func):
        ...
        def sync_wrapper(self, *args, **kwargs):
            ...
            current_ctx = get_current_trace()
            if current_ctx:
                ctx = current_ctx.create_child_context(operation=f"{agent_id}:{method_name}")
            else:
                from .context import create_trace_context

                ctx = create_trace_context(operation=f"{agent_id}:{method_name}", agent_id=agent_id)
...
def get_observability_health() -> Dict[str, Any]:
    ...
    return {
        "tracer": {"status": "healthy", "service_name": tracer.service_name},
        "error_tracker": {"status": "healthy", "total_errors_tracked": len(error_tracker.errors)},
        "metrics": {"status": "healthy", "total_metrics": len(metrics.metrics)},
        "timestamp": time.time(),
    }
