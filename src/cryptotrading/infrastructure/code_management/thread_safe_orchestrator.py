...
class CoordinatedTaskManager:
    ...
    async def wait_for_all(self, task_ids: List[str], timeout: float = None) -> Dict[str, Any]:
        ...
        if timeout:
            tasks = [
                asyncio.create_task(self.get_task_result(task_id, timeout))
...
class ThreadSafeEnterpriseOrchestrator:
    ...
    def quality_monitor(self) -> AutomatedQualityMonitor:
        ...
        if self._quality_monitor is None:
            self._quality_monitor = AutomatedQualityMonitor(self.project_path, self.database_adapter)
    ...
    def proactive_detector(self) -> ProactiveIssueDetector:
        ...
        if self._proactive_detector is None:
            self._proactive_detector = ProactiveIssueDetector(self.project_path, self.database_adapter)
            ...
            try:
                async with self._component_locks["health"]:
                    ...
                    if health_metrics.coverage_percentage < 70.0:
                        await self._emit_event("critical_health_issue", {
                            "type": "low_coverage",
                            "value": health_metrics.coverage_percentage
                        })
        ...
        while self.running:
            try:
                async with self._component_locks["quality"]:
                    ...
                    # Emit event
                    await self._emit_event("quality_check_completed", {
        ...
        while self.running:
            try:
                async with self._component_locks["proactive"]:
                    ...
                    # Emit event
                    await self._emit_event("proactive_scan_completed", {
                        "issues_found": len(issues),
                        "timestamp": datetime.utcnow().isoformat()
                    })
    ...
    async def _start_lifecycle_management(self):
        ...
        if self._backlog_tracker is None:
            self._backlog_tracker = IssueBacklogTracker(self.database_adapter, self._lifecycle_manager)
        ...
        while self.running:
            try:
                async with self._component_locks["lifecycle"]:
                    ...
                    # Emit event
                    await self._emit_event("lifecycle_cycle_completed", {
                        "triaged": triaged,
    ...
    async def _start_seal_workflow(self):
        ...
        if self._seal_workflow_engine is None:
            self._seal_workflow_engine = SEALWorkflowEngine(
                ...
                self._backlog_tracker, self._seal_adapter
            )
