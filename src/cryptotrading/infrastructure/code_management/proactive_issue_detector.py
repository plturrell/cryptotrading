...
class ProactiveIssueDetector:
    ...
    def _should_scan_file(self, file_path: Path) -> bool:
        ...
        exclude_patterns = [
            ...
            ".pytest_cache", "dist", "build", ".vercel"
        ]
    ...
    def _check_function_length(self, content: str, file_path: Path, rule: ProactiveRule) -> List[CodeIssue]:
                ...
                if isinstance(node, ast.FunctionDef):
                    ...
                    if func_length > 50:  # Configurable threshold
                        issue = CodeIssue(
                            ...
                            type=rule.issue_type,
                            severity=rule.severity + (func_length // 25),  # Increase severity for longer functions
                            file_path=str(file_path.relative_to(self.project_path)),
    ...
    def _analyze_ast(self, content: str, file_path: Path) -> List[CodeIssue]:
        ...
        try:
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, (ast.If, ast.For, ast.While)) and self._get_nesting_depth(node) > 4:
                    issue = CodeIssue(
    ...
    async def _check_dependencies(self) -> List[CodeIssue]:
        ...
        for py_file in python_files:
            try:
                ...
                deprecated_imports = [
                    'imp', 'optparse', 'distutils', 'platform.dist'
                ]
    ...
    def get_issue_summary(self) -> Dict[str, Any]:
        ...
        for issue in self.detected_issues:
            ...
            # Count by severity
            severity_range = "low" if issue.severity <= 3 else "medium" if issue.severity <= 6 else "high"
