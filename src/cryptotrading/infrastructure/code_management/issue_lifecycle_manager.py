...
class IssueLifecycleManager:
    ...
    async def transition_issue(self, issue_id: str, to_state: IssueState, reason: str, 
                             automated: bool = False, metadata: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            metadata = json.loads(issue_data.get('metadata', '{}')) if isinstance(issue_data.get('metadata'), str) else issue_data.get('metadata', {})
            ...
            if not self._is_valid_transition(current_state, to_state):
                logger.error("Invalid transition from %s to %s for issue %s", 
                           current_state.value, to_state.value, issue_id)
                return False
            ...
            logger.info("Issue %s transitioned from %s to %s: %s", 
                       issue_id, current_state.value, to_state.value, reason)
    ...
    async def auto_triage_issues(self) -> int:
        ...
        try:
            ...
            detected_issues = [
                ...
                if i.get('metadata') and (
                    json.loads(i.get('metadata', '{}')) if isinstance(i.get('metadata'), str) 
                    else i.get('metadata', {})
                ).get("lifecycle_state") == "detected"
            ]
    ...
    async def auto_progress_fixable_issues(self) -> int:
        ...
        try:
            ...
            auto_fixable = [
                i for i in issues 
                if (i.auto_fixable and 
                    i.metadata and 
                    i.metadata.get("lifecycle_state") == "triaged")
            ]
        ...
        try:
            ...
            for issue_data in issues_data:
                ...
                if state == "closed" and metadata:
                    detected_time = datetime.fromisoformat(issue_data.get('created_at', datetime.now().isoformat()))
                    closed_time = datetime.fromisoformat(metadata.get("last_transition", issue_data.get('created_at', datetime.now().isoformat())))
            ...
            total_fixed = by_state.get("fixed", 0) + by_state.get("verified", 0) + by_state.get("closed", 0)
            ...
            auto_fixed = len([i for i in issues_data if i.get('auto_fixable', False) and 
                            (json.loads(i.get('metadata', '{}')) if isinstance(i.get('metadata'), str) else i.get('metadata', {})).get("lifecycle_state") in ["fixed", "verified", "closed"]])
            ...
            return IssueLifecycleMetrics(
                total_issues=0, by_state={}, by_priority={}, by_type={},
                average_resolution_time=0, backlog_age_distribution={},
                fix_success_rate=0, automated_fix_rate=0
            )
    ...
    async def purge_expired_issues(self, days_threshold: int = 90) -> int:
        ...
        try:
            ...
            for issue in issues:
                if (issue.metadata and 
                    issue.metadata.get("lifecycle_state") in ["closed", "rejected"] and
                    issue.metadata.get("last_transition")):
    ...
    async def bulk_transition_issues(self, issue_ids: List[str], to_state: IssueState, 
