...
class EnterpriseCodeOrchestrator(ThreadSafeEnterpriseOrchestrator):
    ...
    async def _handle_critical_health_issues(self, health_metrics: CodeHealthMetrics) -> None:
        ...
        if health_metrics.coverage_percentage < critical_threshold:
            logger.warning(
                "🚨 CRITICAL: Code coverage below threshold (%.1f%% < %.1f%%)",
        ...
        if health_metrics.technical_debt_score < 60.0:
            logger.warning(
                "🚨 CRITICAL: Technical debt score too low (%.1f)",
    ...
    async def _generate_orchestration_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_proactive_scan": self.last_proactive_scan.isoformat()
            if self.last_proactive_scan
            else None,
        ...
        if self.database_adapter:
            try:
                await self.database_adapter.log_monitoring_event(
                    event_type="orchestration_report", details=report
                )
            except Exception as e:
    ...
    async def _check_system_health(self) -> None:
        ...
        if (
            ...
            and (now - self.last_health_check).seconds
            > self.config.continuous_monitoring_interval * 2
        ):
            logger.warning("⚠️ Health monitoring appears to be lagging")

        if (
            self.last_quality_check
            and (now - self.last_quality_check).seconds > self.config.quality_check_interval * 2
        ):
            logger.warning("⚠️ Quality monitoring appears to be lagging")

        if (
            self.last_proactive_scan
            and (now - self.last_proactive_scan).seconds > self.config.proactive_scan_interval * 2
        ):
...
async def main():
    ...
    parser.add_argument(
        "--health-interval", type=int, default=1800, help="Health check interval (seconds)"
    )
    parser.add_argument(
        "--quality-interval", type=int, default=900, help="Quality check interval (seconds)"
    )
    parser.add_argument(
        "--proactive-interval", type=int, default=3600, help="Proactive scan interval (seconds)"
    )
