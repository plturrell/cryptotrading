...
class CodeManagementDatabaseAdapter:
            ...
            if self.db.config.mode.value == "local":
                ...
                """,
                    (
                        ...
                        issue.fix_status.value,
                        json.dumps(issue_metadata, cls=EnumJSONEncoder),
                        datetime.now(),
                        datetime.now(),
                    ),
                )
    ...
    async def update_issue_status(
        self, issue_id: str, status: FixStatus, fixed_at: datetime = None
    ) -> bool:
    ...
    async def save_code_file(
        self, file_path: str, language: str, file_size: int, content_hash: str, facts_count: int = 0
    ) -> str:
            ...
            if self.db.config.mode.value == "local":
                ...
                """,
                    (
                        ...
                        severity,
                        message,
                        json.dumps(details or {}, cls=EnumJSONEncoder),
                        datetime.now(),
                    ),
                )
            else:
