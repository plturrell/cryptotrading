...
class AutomatedQualityMonitor:
    ...
    async def run_continuous_monitoring(self, interval_seconds: int = 300) -> None:
        ...
        logger.info("🔄 Starting continuous quality monitoring (interval: %d seconds)", interval_seconds)
        ...
        while True:
            try:
                ...
                # Log summary
                logger.info("Quality check complete: %d issues found, %d auto-fixed", 
    ...
    async def run_single_check(self, check: QualityCheck) -> Dict[str, Any]:
        ...
        # Run the command
        cmd = f"{check.command} {' '.join(str(f) for f in python_files[:10])}"  # Limit files for demo
            ...
            if stdout.strip():
                ...
                for result in data.get("results", []):
                    issues.append({
                        ...
                        "severity": check.severity_mapping.get(result.get("issue_severity", "LOW"), 3)
                    })
                return {"issues": issues, "total": len(issues)}
            return {"issues": [], "total": 0}
        except json.JSONDecodeError:
        ...
        for tool_name, result in results.items():
            ...
            for issue_data in tool_issues:
                issue = CodeIssue(
                    ...
                    line_number=issue_data.get("line", 0) if issue_data.get("line", 0) > 0 else None,
    ...
    def store_results(self, results: Dict[str, Any], issues: List[CodeIssue], fixed_issues: List[CodeIssue]) -> None:
        ...
        for issue in issues:
            tool = issue.description.split(']')[0].replace('[', '') if ']' in issue.description else "unknown"
