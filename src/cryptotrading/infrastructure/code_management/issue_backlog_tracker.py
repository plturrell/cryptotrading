...
from .issue_lifecycle_manager import IssueState, IssuePriority, IssueLifecycleManager
...
class IssueBacklogTracker:
    ...
    async def create_sprint(self, name: str, start_date: datetime = None, end_date: datetime = None, duration_weeks: int = 2) -> str:
        """Create a new sprint"""
        try:
    ...
    async def populate_sprint_backlog(self, sprint_id: str, 
        ...
        try:
            ...
            if auto_select:
                ...
                logger.info("Auto-populated sprint %s with %d issues", 
                          sprint.name, len(selected_issues))
    ...
    async def end_sprint(self, sprint_id: str) -> ProgressReport:
        ...
        try:
            ...
            logger.info("Completed sprint: %s (%.1f%% completion)", 
                       sprint.name, report.completion_percentage)
        ...
        try:
            ...
            if not backlog_issues:
                return BacklogMetrics(
                    total_backlog_size=0, by_priority={}, by_type={}, by_component={},
                    average_age_days=0, velocity_issues_per_week=0,
                    estimated_completion_weeks=0, critical_issues=0, auto_fixable_count=0
                )
    ...
    async def _calculate_velocity(self) -> float:
        ...
        try:
            ...
            completed_issues = [
                i for i in issues
                if (i.metadata and 
                    i.metadata.get("lifecycle_state") in ["verified", "closed"] and
                    i.metadata.get("last_transition") and
                    datetime.fromisoformat(i.metadata["last_transition"]) >= cutoff_date)
            ]
    ...
    async def get_progress_report(self, sprint_id: str) -> ProgressReport:
        ...
        try:
            ...
            sprint_issues = [
                i for i in issues
                if (i.metadata and 
                    i.metadata.get("sprint_id") == sprint_id)
            ]
            
            # Count by status
            completed = len([i for i in sprint_issues 
                           if i.metadata.get("lifecycle_state") in ["verified", "closed"]])
            in_progress = len([i for i in sprint_issues 
                             if i.metadata.get("lifecycle_state") in ["in_progress", "fixing"]])
            blocked = len([i for i in sprint_issues 
                         if i.metadata.get("blocked", False)])
        ...
        try:
            ...
            while current_date <= min(datetime.now(), end_date):
                ...
                remaining = len([
                    i for i in issues
                    if (i.metadata and 
                        i.metadata.get("sprint_id") == sprint_id and
                        i.metadata.get("lifecycle_state") not in ["verified", "closed"])
                ])
                
                burndown_data.append({
                    "date": current_date.isoformat(),
                    "remaining_issues": remaining,
                    "ideal_remaining": max(0, len(sprint.assigned_issues) * 
    ...
    def _identify_risk_factors(self, sprint: Sprint, completion_percentage: float,
                             days_remaining: int, velocity: float) -> List[str]:
        """Identify potential risk factors for sprint completion"""
        risks = []
        
        # Low completion rate
        sprint_duration = (datetime.fromisoformat(sprint.end_date) - 
    ...
    async def archive_completed_sprints(self, days_threshold: int = 30) -> int:
        ...
        for sprint in self.sprints[:]:
            if (sprint.status == SprintStatus.COMPLETED and
                datetime.fromisoformat(sprint.end_date) < cutoff_date):
