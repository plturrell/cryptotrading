...
class HistoricalDataMCPTools:
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        ...
                        "quality_checks": {
                            ...
                            "items": {
                                "type": "string",
                                "enum": ["completeness", "consistency", "outliers", "gaps", "duplicates"]
                            },
    ...
    async def _load_aligned_data(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Failed to load aligned data: {str(e)}"
            }
    ...
    async def _get_data_catalog(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Failed to get data catalog: {str(e)}"
            }
            ...
            for check in quality_checks:
                try:
                    if check == "completeness":
                        ...
                        completeness_result = await self._check_data_completeness(data_source, symbols_or_series)
                        validation_results["results"]["completeness"] = completeness_result
                    elif check == "consistency":
                        # Actually check data consistency
                        consistency_result = await self._check_data_consistency(data_source, symbols_or_series)
                        ...
                        outliers_result = await self._detect_outliers(data_source, symbols_or_series)
                        validation_results["results"]["outliers"] = outliers_result
                    elif check == "duplicates":
                        # Actually check for duplicates
                        duplicates_result = await self._check_duplicates(data_source, symbols_or_series)
        ...
        try:
            ...
            try:
                if statistics_type == "basic":
                    stats_result["statistics"] = await self._compute_basic_statistics(data_source, symbols_or_series)
                elif statistics_type == "detailed":
                    stats_result["statistics"] = await self._compute_detailed_statistics(data_source, symbols_or_series)
                elif statistics_type == "correlation":
                    stats_result["statistics"] = await self._compute_correlation_statistics(data_source, symbols_or_series)
                elif statistics_type == "volatility":
                    stats_result["statistics"] = await self._compute_volatility_statistics(data_source, symbols_or_series)
                else:
                    return {
                        "success": False,
    ...
    async def _check_data_completeness(self, data_source: str, symbols_or_series: List[str]) -> Dict[str, Any]:
        """Check actual data completeness"""
        try:
            data_dir = self.data_loader.data_dir / data_source
            if not data_dir.exists():
                return {"score": 0.0, "missing_data_percentage": 100.0, "details": f"No {data_source} data directory found"}
            
            total_files = len(list(data_dir.glob("*.csv")))
            if total_files == 0:
                return {"score": 0.0, "missing_data_percentage": 100.0, "details": "No data files found"}
            ...
            missing_percentage = (missing_count / len(symbols_or_series)) * 100 if symbols_or_series else 0
            score = max(0.0, 1.0 - (missing_percentage / 100))
            
            return {
                "score": score,
                "missing_data_percentage": missing_percentage,
                "details": f"{missing_count}/{len(symbols_or_series)} files missing" if symbols_or_series else f"{total_files} files available"
            }
        except Exception as e:
            return {"score": 0.0, "error": str(e), "details": "Completeness check failed"}
    
    async def _check_data_consistency(self, data_source: str, symbols_or_series: List[str]) -> Dict[str, Any]:
    ...
    async def _check_data_gaps(self, data_source: str, symbols_or_series: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            score = max(0.0, 1.0 - (total_gaps / max(1, files_checked * 10)))  # Allow up to 10 gaps per file
            return {
                "score": score,
    ...
    async def _detect_outliers(self, data_source: str, symbols_or_series: List[str]) -> Dict[str, Any]:
    ...
    async def _check_duplicates(self, data_source: str, symbols_or_series: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            score = 1.0 if total_duplicates == 0 else max(0.0, 1.0 - (total_duplicates / max(1, files_checked * 100)))
            return {
                "score": score,
    ...
    async def _compute_basic_statistics(self, data_source: str, symbols_or_series: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "data_types": {
                    "numeric": len([col for col in all_columns if col in ['Open', 'High', 'Low', 'Close', 'Volume']]),
                    "datetime": 1,
                    "categorical": len(all_columns) - len([col for col in all_columns if col in ['Open', 'High', 'Low', 'Close', 'Volume']]) - 1
                }
            }
        except Exception as e:
            return {"error": str(e), "details": "Basic statistics computation failed"}
    
    async def _compute_detailed_statistics(self, data_source: str, symbols_or_series: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not all_close_prices:
                return {"error": "No price data found", "details": "Cannot compute detailed statistics"}
    ...
    async def _compute_correlation_statistics(self, data_source: str, symbols_or_series: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(price_data) < 2:
                return {"error": "Insufficient data for correlation", "details": "Need at least 2 symbols"}
    ...
    async def _compute_volatility_statistics(self, data_source: str, symbols_or_series: List[str]) -> Dict[str, Any]:
