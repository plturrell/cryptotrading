...
class S3StorageMCPTools:
    ...
    async def store_agent_data(
        ...
        try:
            ...
            date_str = datetime.utcnow().strftime("%Y/%m/%d")
            s3_key = (
                f"agent-data/{agent_id}/{data_type}/{date_str}/{datetime.utcnow().isoformat()}.json"
            )
    ...
    async def store_calculation_result(
        ...
        try:
            ...
            return {
                "success": success,
                "message": "Calculation result stored"
                if success
                else "Failed to store calculation",
            }
    ...
    async def retrieve_agent_logs(
        ...
        try:
            ...
            return {"success": True, "logs": logs, "total_found": len(logs)}
