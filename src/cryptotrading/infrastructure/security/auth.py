...
class AuthManager:
    ...
    def _create_default_admin(self):
        ...
        admin_user = User(
            user_id="admin-001", username="admin", email="admin@example.com", roles=[Role.ADMIN]
        )
    ...
    def create_user(
        self, username: str, email: str, roles: List[Role], created_by: str
    ) -> tuple[User, str]:
    ...
    def create_jwt_token(self, user: User) -> tuple[str, str]:
        ...
        access_token = jwt.encode(
            access_payload, self.config.JWT_SECRET_KEY, algorithm=self.config.JWT_ALGORITHM
        )

        refresh_token = jwt.encode(
            refresh_payload, self.config.JWT_SECRET_KEY, algorithm=self.config.JWT_ALGORITHM
        )
    ...
    def check_rate_limit(
        ...
        if identifier in self.login_attempts:
            self.login_attempts[identifier] = [
                attempt for attempt in self.login_attempts[identifier] if attempt > window_start
            ]
