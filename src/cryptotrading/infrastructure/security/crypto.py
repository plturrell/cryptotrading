    ...
    def _load_keys_from_storage(self):
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                key_metadata_list = (
                    session.query(EncryptionKeyMetadata).filter_by(is_active=True).all()
                )
                ...
                for key_metadata in key_metadata_list:
                    try:
                        ...
                        if (
                            not self.current_key_id
                            or key_metadata.created_at > self.keys[self.current_key_id].created_at
                        ):
            ...
            logger.warning(
                f"Failed to load keys from storage: {e}. Will create new keys as needed."
            )
...
class MessageEncryption:
    ...
    def encrypt_payload(
        self, payload: Dict[str, Any], additional_data: bytes = None
    ) -> Dict[str, str]:
    ...
    def decrypt_payload(
        self, encrypted_payload: Dict[str, str], additional_data: bytes = None
    ) -> Dict[str, Any]:
...
def encrypt_sensitive_data(data: Dict[str, Any], sensitive_fields: list = None) -> Dict[str, Any]:
    ...
    for field in sensitive_fields:
        if field in encrypted_data and isinstance(encrypted_data[field], str):
            encrypted_data[field] = message_encryption.encrypt_field(encrypted_data[field], field)

    return encrypted_data


def decrypt_sensitive_data(
    encrypted_data: Dict[str, Any], sensitive_fields: list = None
) -> Dict[str, Any]:
