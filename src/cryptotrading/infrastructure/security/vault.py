...
class SecretManager:
            ...
            try:
                with open(secrets_file, "r") as f:
                    ...
                    for key, secret_data in data.items():
                        secret_data["created_at"] = datetime.fromisoformat(
                            secret_data["created_at"]
                        )
                        if secret_data.get("expires_at"):
                            secret_data["expires_at"] = datetime.fromisoformat(
    ...
    async def store_secret(
        self, key: str, value: str, metadata: Dict[str, Any] = None, ttl_hours: int = None
    ):
        ...
        if len(parts) >= 2:
            ...
            await self.credentials_manager.store_credential(
                service_name, credential_type, value, metadata=metadata, expires_at=expires_at
            )
...
class DatabaseCredentials:
    ...
    async def get_database_url(self, db_name: str = "main") -> str:
        ...
        if not db_url:
            ...
            db_url = os.getenv(
                f"DATABASE_URL_{db_name.upper()}",
                os.getenv("DATABASE_URL", "postgresql://postgres:password@localhost:5432/reks"),
            )
        return db_url

    async def store_database_credentials(
        self, db_name: str, host: str, port: int, database: str, username: str, password: str
    ):
...
class APICredentials:
    ...
    async def generate_service_account(self, service_name: str) -> Dict[str, str]:
        ...
        return {"api_key": api_key, "api_secret": api_secret, "service_name": service_name}
...
async def initialize_secrets():
    ...
    await db_credentials.store_database_credentials(
        "main", "localhost", 5432, "reks_production", "reks_user", secrets.token_urlsafe(16)
    )
    ...
    vault_policies = {
        "reks-a2a-policy": {
            "path": {
                ...
                "secret/metadata/reks/a2a/*": {"capabilities": ["list", "delete"]},
            }
        }
    }
