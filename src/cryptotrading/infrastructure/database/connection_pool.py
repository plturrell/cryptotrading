        ...
        try:
            if self.db_url.startswith("postgresql://"):
                if not ASYNCPG_AVAILABLE:
                    raise RuntimeError(
                        "PostgreSQL requested but asyncpg not available. Install with: pip install asyncpg"
                    )
                else:
            ...
            logger.info(
                "Database connection pool initialized with %d-%d connections",
    ...
    async def acquire(self) -> AsyncIterator[Any]:
        ...
        try:
            ...
            self._metrics["avg_query_time"] = (
                self._metrics["avg_query_time"] * (self._metrics["queries_executed"] - 1)
                + query_time
...
async def get_connection_pool(
    db_url: str, config: Optional[PoolConfig] = None
