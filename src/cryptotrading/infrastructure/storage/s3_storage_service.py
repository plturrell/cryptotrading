...
class S3StorageService:
    ...
    def __init__(
        self, secret_name: str = "cryptotrading/s3-storage", region_name: str = "us-east-1"
    ):
        """
    ...
    def upload_file(
        ...
        try:
            ...
            self.s3_client.upload_file(
                local_file_path, self.bucket_name, s3_key, ExtraArgs=upload_args
            )

            logger.info(
                f"File uploaded successfully: {local_file_path} -> s3://{self.bucket_name}/{s3_key}"
            )
            return True
    ...
    def upload_data(
        ...
        try:
            ...
            # Upload data
            self.s3_client.put_object(Bucket=self.bucket_name, Key=s3_key, Body=data, **upload_args)
    ...
    def download_file(self, s3_key: str, local_file_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"File downloaded successfully: s3://{self.bucket_name}/{s3_key} -> {local_file_path}"
            )
            return True
...
def save_market_data_to_s3(
    storage_service: S3StorageService, symbol: str, market_data: Dict, timestamp: datetime = None
) -> bool:
    """
    ...
    # Add metadata
    metadata = {"symbol": symbol, "data_type": "market_data", "timestamp": timestamp.isoformat()}
...
def save_user_data_to_s3(
    storage_service: S3StorageService, user_id: str, data_type: str, user_data: Dict
) -> bool:
    """
    ...
    metadata = {"user_id": user_id, "data_type": data_type, "timestamp": timestamp.isoformat()}
