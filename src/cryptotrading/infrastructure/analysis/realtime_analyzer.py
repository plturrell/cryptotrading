...
class CodeChangeHandler(FileSystemEventHandler):
    ...
    async def _debounced_analysis(self, file_path: str, event_type: str, schedule_time: float):
        ...
        if self.pending_changes.get(file_path) == schedule_time:
            ...
            # Trigger analysis
            await self.analyzer.analyze_file_change(FileChangeEvent(
                file_path=file_path,
                event_type=event_type,
                timestamp=schedule_time,
                file_hash=self._calculate_file_hash(file_path) if Path(file_path).exists() else None
            ))
...
class RealtimeCodeAnalyzer:
    ...
    async def _run_analysis(self, analysis_type: AnalysisType, file_path: str) -> Optional[AnalysisResult]:
    ...
    def get_results_for_file(self, file_path: str, limit: Optional[int] = None) -> List[AnalysisResult]:
    ...
    async def run_full_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if AnalysisType.COMPLEXITY_ANALYSIS in self.enabled_analyses:
                ...
                complexity = await self.code_analyzer.analyze_complexity(file_paths[:20])  # Limit for performance
            ...
            if AnalysisType.DEAD_CODE_DETECTION in self.enabled_analyses:
                ...
                src_files = list(Path(self.project_root / "src").rglob("*.py"))
                file_paths = [str(f) for f in src_files if f.name != "__init__.py"]
                dead_code = await self.code_analyzer.detect_dead_code(file_paths[:20])  # Limit for performance
