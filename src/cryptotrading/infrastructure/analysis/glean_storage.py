...
class GleanStorage:
    ...
    def store_facts(self, facts: List[Dict[str, Any]], unit: str = "default") -> int:
        ...
        with self.db.get_connection() as conn:
            ...
            cursor.execute(
                "INSERT OR IGNORE INTO glean_units (unit_id, name) VALUES (?, ?)", (unit, unit)
            )
    ...
    def export_unit(self, unit: str, output_path: Path) -> None:
        ...
        with self.db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT predicate, key_json, value_json FROM glean_facts WHERE unit = ?", (unit,)
            )

            for row in cursor.fetchall():
                facts.append(
                    {"predicate": row[0], "key": json.loads(row[1]), "value": json.loads(row[2])}
                )

        with open(output_path, "w") as f:
            json.dump(
                {"unit": unit, "facts": facts, "exported_at": datetime.utcnow().isoformat()},
                f,
                indent=2,
            )
    ...
    def get_schema(self, predicate: str) -> Optional[Dict[str, Any]]:
        ...
        with self.db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT schema_json FROM glean_schemas WHERE predicate = ?", (predicate,)
            )
            row = cursor.fetchone()
            if row:
...
PYTHON_SCHEMAS = {
    ...
    "python.Reference": {
        "key": {"target": "string", "file": "string", "span": "array:int"},
        "value": {},
    },
    "python.Import": {
        "key": {"module": "string", "file": "string"},
        "value": {"imported_names": "array:string", "is_relative": "boolean"},
    },
    "src.File": {
        "key": {"path": "string"},
        "value": {"language": "string", "size_bytes": "int", "last_modified": "timestamp"},
    },
}
