...
class AgentSegregationManager:
    ...
    def check_permission(
        ...
        if resource_type not in agent_context.permissions:
            self._log_access(
                agent_context, "permission_check", resource_type, action,
    ...
    def check_resource_quota(
        ...
        if current_usage + requested_amount > quota_limit:
            self._log_access(
                agent_context, "quota_check", ResourceType.CODE_ANALYSIS, "quota_exceeded",
                success=False, error_message=f"Quota exceeded for {resource_key}"
            )
            return False
    ...
    def is_cross_tenant_access(self, agent_context: AgentContext, target_data: Dict[str, Any]) -> bool:
        ...
        if target_tenant and target_tenant != agent_context.tenant_id:
            self._log_access(
                agent_context, "cross_tenant_check", ResourceType.CODE_ANALYSIS, "blocked",
                success=False, error_message="Cross-tenant access denied"
            )
            return True
        return False
    ...
    def get_agent_stats(self, agent_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_activity_count": len([log for log in recent_logs if log.timestamp > datetime.utcnow() - timedelta(hours=1)]),
            "success_rate": len([log for log in recent_logs if log.success]) / max(len(recent_logs), 1) * 100
        }
    ...
    async def execute_with_segregation(
        ...
        try:
            ...
            self.segregation_manager._log_access(
                agent_context, self.tool_name, self.resource_type, "execute",
