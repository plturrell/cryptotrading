...
from .glean_client import GleanClient, CodeSymbol, CodeReference
...
class ImpactAnalyzer:
    ...
    async def analyze_file_change_impact(self, file_path: str, changed_lines: Optional[List[int]] = None) -> ChangeImpactReport:
        ...
        affected_modules = list(set(self._get_module_from_path(ref.file_path) for ref in all_references))
    ...
    async def analyze_symbol_change_impact(self, symbol_name: str, change_type: str = "modification") -> Dict[str, Any]:
            ...
            for indirect_ref in indirect_refs:
                if indirect_ref.file_path not in [i.file_path for i in direct_impacts]:
                    indirect_impact = ImpactedComponent(
                        name=self._get_symbol_at_location(indirect_ref.file_path, indirect_ref.line),
    ...
    async def analyze_pr_impact(self, changed_files: List[str], pr_diff: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def _find_affected_tests(self, changed_symbols: List[str], affected_files: List[str]) -> List[str]:
    ...
    async def _calculate_risk_level(self, file_path: str, changed_symbols: List[str], 
                                  affected_files: List[str], affected_modules: List[str]) -> tuple[RiskLevel, float]:
        """Calculate risk level and impact score"""
        risk_factors = []
    ...
    async def _generate_recommendations(self, file_path: str, symbols: List[CodeSymbol], 
                                      affected_files: List[str], risk_level: RiskLevel) -> List[str]:
        ...
        if len(affected_files) > 10:
            recommendations.append(f"📊 {len(affected_files)} files affected - consider breaking into smaller changes")
        ...
        if "agents" in module:
            ...
            recommendations.append("🗄️ Database changes - run migration tests and backup procedures")
        
        return recommendations
    
    async def _detect_breaking_changes(self, symbols: List[CodeSymbol], references: List[CodeReference]) -> List[Dict[str, Any]]:
    ...
    def _calculate_overall_risk(self, impacts: List[ImpactedComponent], change_type: str) -> RiskLevel:
    ...
    def _generate_pr_recommendations(self, reports: List[ChangeImpactReport], overall_risk: RiskLevel) -> List[str]:
        """Generate PR-level recommendations"""
        recommendations = []
        ...
        if total_breaking_changes > 0:
            recommendations.append(f"💥 {total_breaking_changes} potential breaking changes detected")
        ...
        if critical_modules_affected:
            recommendations.append(f"⚠️ Critical modules affected: {', '.join(critical_modules_affected)}")
        
        return recommendations
    
    def _generate_test_strategy(self, reports: List[ChangeImpactReport], overall_risk: RiskLevel) -> Dict[str, Any]:
