...
class SAPCAPIndexer:
    ...
    ENTITY_PATTERN = re.compile(
        r"entity\s+(\w+)\s*(?::\s*\w+\s*)?{([^}]+)}", re.MULTILINE | re.DOTALL
    )
    ...
    def index_file(self, file_path: Path) -> Optional[CAPDocument]:
        ...
        try:
            ...
            for entity_match in self.ENTITY_PATTERN.finditer(content):
                ...
                entity = CAPEntity(name=entity_name, namespace=current_namespace)
            ...
            for service_match in self.SERVICE_PATTERN.finditer(content):
                ...
                service = CAPService(name=service_name, namespace=current_namespace)
    ...
    def _parse_entity_elements(self, body: str, entity: CAPEntity):
        ...
        for assoc_match in self.ASSOCIATION_PATTERN.finditer(body):
            ...
            entity.elements[element_name] = {"type": "Association", "target": target_entity}
        ...
        for comp_match in self.COMPOSITION_PATTERN.finditer(body):
            ...
            entity.elements[element_name] = {"type": "Composition", "target": target_entity}
        ...
        for elem_match in self.ELEMENT_PATTERN.finditer(body):
            ...
            if element_name not in entity.elements:
                entity.elements[element_name] = {"type": element_type}
        ...
        for doc in self.documents:
            ...
            for type_name, type_info in doc.types.items():
                facts.append(
                    {
                        "predicate": "cap.Type",
                        "key": {"name": type_name, "file": doc.relative_path},
                        "value": {"definition": type_info["definition"]},
                    }
                )
            ...
            for using_stmt in doc.using_statements:
                facts.append(
                    {
                        "predicate": "cap.Using",
                        "key": {"file": doc.relative_path, "import": using_stmt},
                        "value": {},
                    }
                )
