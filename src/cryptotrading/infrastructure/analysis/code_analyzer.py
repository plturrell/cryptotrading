...
class DependencyAnalyzer:
    ...
    async def build_dependency_graph(self, root_module: str = "cryptotrading") -> nx.DiGraph:
        ...
        logger.info("Dependency graph built: %d nodes, %d edges", len(self.dependency_graph.nodes), len(self.dependency_graph.edges))
...
class CodeAnalyzer:
    ...
    async def analyze_symbol_usage(self, symbol: str) -> Dict[str, Any]:
        ...
        async with self.glean_client:
            ...
            return {
                ...
                "modules_affected": len(set(self._get_module_from_path(ref.file_path) for ref in references))
            }
    
    def _categorize_references(self, references: List[CodeReference]) -> Dict[str, List[CodeReference]]:
    ...
    def _estimate_loc(self, module: str) -> int:
        ...
        try:
            module_path = self.project_root / "src" / "cryptotrading" / f"{module.replace('.', '/')}.py"
            ...
            for py_file in src_path.rglob("*.py"):
                ...
                for symbol in symbols:
                    if symbol.type == "function":
                        ...
                        external_refs = [ref for ref in refs 
                                       if ref.file_path != symbol.file_path or 
