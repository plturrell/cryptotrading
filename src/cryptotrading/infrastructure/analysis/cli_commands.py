...
from .realtime_analyzer import AnalysisType, RealtimeCodeAnalyzer
...
class GleanCLI:
    ...
    async def analyze_dependencies(
        self, module: str, depth: int = 2, output_format: str = "json"
    ) -> Dict[str, Any]:
    ...
    async def _generate_dependency_graph(self, deps: Dict[str, Any]) -> str:
        ...
        return (
            f"Dependency graph for {deps.get('module')} (visual graph generation not implemented)"
        )
    ...
    def _format_architecture_summary(self, report: Dict[str, Any]) -> str:
        ...
        status_icon = {"clean": "✅", "minor": "ℹ️", "warning": "⚠️", "critical": "🚨"}.get(
            status, "❓"
        )
    ...
    async def start_realtime_monitoring(
        self, analyses: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                ...
                def result_callback(result):
                    ...
                    click.echo(
                        f"{timestamp} {status} {result.analysis_type.value}: {Path(result.file_path).name}"
                    )
    ...
    def _format_full_analysis_summary(self, results: Dict[str, Any]) -> str:
        ...
        if summary.get("success"):
            lines.append(
                f"✅ Analysis completed in {summary.get('duration_seconds', 0):.1f} seconds"
            )
        else:
...
async def clrs_analyze_code(file_path: str, algorithm: str = "all", output_format: str = "json"):
    ...
    try:
        ...
        result = await tool.execute({"file_path": file_path, "algorithm": algorithm})
...
async def tree_analyze_structure(
    file_path: str, operation: str = "analyze", output_format: str = "json"
):
    ...
    try:
        ...
        result = await tool.execute({"file_path": file_path, "operation": operation})
...
async def dependency_graph_analysis(
    project_path: str = ".", algorithm: str = "dfs", output_format: str = "json"
):
    ...
    try:
        ...
        result = await tool.execute({"project_path": project_path, "algorithm": algorithm})
...
async def code_similarity_analysis(
    file1: str, file2: str, algorithm: str = "lcs", output_format: str = "json"
):
    ...
    try:
        ...
        result = await tool.execute({"file1": file1, "file2": file2, "algorithm": algorithm})
...
async def optimization_recommendations(
    project_path: str = ".", focus: str = "performance", output_format: str = "json"
):
    ...
    try:
        ...
        result = await tool.execute({"project_path": project_path, "focus": focus})
