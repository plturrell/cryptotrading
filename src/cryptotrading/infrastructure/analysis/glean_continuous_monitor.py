...
from .glean_zero_blindspots_mcp_tool import GleanZeroBlindSpotsMCPTool, GleanValidationResult
...
class GleanContinuousMonitor:
    ...
    def __init__(self):
        ...
        self.description = "Continuously monitors code changes and validates Glean knowledge completeness"
        ...
        try:
            ...
            if command == "start":
                ...
                return {
                    ...
                    "available_commands": ["start", "stop", "status", "validate_now", "list_sessions"]
                }
    ...
    async def _start_monitoring(self, project_path: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _stop_monitoring(self, project_path: str) -> Dict[str, Any]:
        ...
        if session_id not in self.active_sessions:
            return {
                "success": False,
                "error": "No active monitoring session for this project"
            }
    ...
    async def _get_monitoring_status(self, project_path: str) -> Dict[str, Any]:
        ...
        recent_changes = [
            asdict(event) for event in session.change_events[-10:]
        ]
        ...
        return {
            ...
            "last_validation": session.last_validation.isoformat() if session.last_validation else None,
            "total_change_events": len(session.change_events),
                ...
                if self._should_validate(session):
                    ...
                    if validation_result["success"]:
                        ...
                        if abs(score_change) > 5.0:
                            logger.warning(f"📊 Significant score change detected: {score_change:+.1f} points")
                            
                            # Could trigger alerts here
                            await self._handle_score_change(session, score_change, validation_result)
    ...
    async def _handle_score_change(self, session: MonitoringSession, score_change: float, 
...
async def glean_continuous_monitor_tool(parameters: Dict[str, Any], agent_context=None) -> Dict[str, Any]:
