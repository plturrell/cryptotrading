...
from .angle_parser import PYTHON_QUERIES, AngleQueryEngine
...
class EnhancedAngleQueryEngine(AngleQueryEngine):
    ...
    def find_cross_language_relationships(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        python_classes = [
            f
            for f in self.facts
            if f.get("predicate") == "python.Declaration"
        ...
        for controller in ui5_controllers:
            ...
            for py_class in python_classes:
                ...
                if (
                    controller_name.lower() in class_name.lower()
                    or class_name.lower() in controller_name.lower()
                ):
                    relationships["ui5_controllers_with_python"].append(
                        {"controller": controller, "python_class": py_class}
                    )
...
def find_cap_entities(
    engine: EnhancedAngleQueryEngine, namespace: str = None
) -> List[Dict[str, Any]]:
...
def find_javascript_functions(
    engine: EnhancedAngleQueryEngine, is_async: bool = None
) -> List[Dict[str, Any]]:
...
def analyze_project_architecture(engine: EnhancedAngleQueryEngine) -> Dict[str, Any]:
    ...
    return {
        ...
        "javascript_components": {
            "functions": len(
                [f for f in engine.facts if f.get("predicate") == "javascript.Function"]
            ),
