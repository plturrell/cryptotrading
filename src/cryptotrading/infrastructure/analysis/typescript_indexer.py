...
class TypeScriptSCIPIndexer:
    ...
    def __init__(self):
        # TypeScript patterns
        self.interface_pattern = re.compile(r'(?:export\s+)?interface\s+(\w+)(?:\s*<([^>]+)>)?\s*(?:extends\s+([^{]+))?\s*{')
        self.class_pattern = re.compile(r'(?:export\s+)?(?:abstract\s+)?class\s+(\w+)(?:\s*<([^>]+)>)?\s*(?:extends\s+(\w+))?\s*(?:implements\s+([^{]+))?\s*{')
        self.function_pattern = re.compile(r'(?:export\s+)?(?:async\s+)?function\s+(\w+)(?:\s*<([^>]+)>)?\s*\([^)]*\)(?:\s*:\s*([^{]+))?\s*{')
        self.method_pattern = re.compile(r'(?:(public|private|protected|readonly)\s+)?(?:(static|async)\s+)?(\w+)(?:\s*<([^>]+)>)?\s*\([^)]*\)(?:\s*:\s*([^{]+))?\s*{')
        self.variable_pattern = re.compile(r'(?:export\s+)?(?:const|let|var)\s+(\w+)(?:\s*:\s*([^=]+))?\s*=')
        self.type_pattern = re.compile(r'(?:export\s+)?type\s+(\w+)(?:\s*<([^>]+)>)?\s*=')
        self.enum_pattern = re.compile(r'(?:export\s+)?enum\s+(\w+)\s*{')
        self.import_pattern = re.compile(r'import\s+(?:{([^}]+)}|(\w+)|\*\s+as\s+(\w+))\s+from\s+[\'"]([^\'"]+)[\'"]')
    ...
    def _extract_symbols(self, content: str, file_path: str) -> List[TypeScriptSymbol]:
        ...
        for match in self.interface_pattern.finditer(content):
            ...
            symbols.append(TypeScriptSymbol(
                name=match.group(1),
                kind="interface",
                line=line_num,
