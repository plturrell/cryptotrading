...
class JavaScriptUI5Indexer:
    ...
    ARROW_FUNCTION_PATTERN = re.compile(r'(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s+)?\(([^)]*)\)\s*=>', re.MULTILINE)
    ...
    REQUIRE_PATTERN = re.compile(r'(?:const|let|var)\s+(?:{([^}]+)}|(\w+))\s*=\s*require\s*\(\s*["\']([^"\']+)["\']\s*\)', re.MULTILINE)
    EXPORT_PATTERN = re.compile(r'export\s+(?:default\s+)?(?:class|function|const|let|var)\s+(\w+)', re.MULTILINE)
    
    # UI5 specific patterns
    UI5_CONTROLLER_PATTERN = re.compile(r'sap\.ui\.define\s*\(\s*\[([^\]]+)\]\s*,\s*function\s*\(([^)]*)\)', re.MULTILINE)
