...
class DataOrchestrator:
        ...
        with trace_context(f"pipeline_{pipeline_name}") as span:
            try:
                ...
                for dep in pipeline.dependencies:
                    if (
                        dep not in self.pipeline_stats
                        or self.pipeline_stats[dep].get("status") != "success"
                    ):
                ...
                self.pipeline_stats[pipeline_name] = {
                    ...
                    "runs_count": self.pipeline_stats.get(pipeline_name, {}).get("runs_count", 0)
                    + 1,
                }
    ...
    async def _run_pipeline(self, pipeline: PipelineTask):
        ...
        try:
            ...
            self.business_metrics.track_pipeline_execution(pipeline_name, True, duration * 1000)
            ...
            # Track business metrics
            self.business_metrics.track_pipeline_execution(pipeline_name, False, duration * 1000)
    ...
    async def _send_quality_alert(
        self, dataset: str, check: DataQualityCheck, result: Dict[str, Any]
    ):
