...
analytics_v2_ns = Namespace('analytics', description='Technical analysis and factor calculations only')

# Models for analytics
technical_analysis_model = analytics_v2_ns.model('TechnicalAnalysis', {
...
factor_analysis_model = analytics_v2_ns.model('FactorAnalysis', {
    'symbol': fields.String(description='Symbol analyzed'),
...
class TechnicalAnalysisV2(Resource):
    ...
    def get(self, symbol):
        ...
        requested_indicators = request.args.get('indicators', '').split(',') if request.args.get('indicators') else None
...
class FactorAnalysisV2(Resource):
    ...
    @analytics_v2_ns.param('factor_types', 'Comma-separated factor types (momentum, value, quality)')
    def get(self, symbol):
...
class CorrelationAnalysisV2(Resource):
    ...
    @analytics_v2_ns.expect(analytics_v2_ns.model('CorrelationRequest', {
        ...
        'correlation_type': fields.String(description='Type of correlation (price, returns, volatility)', default='returns')
    }))
    def post(self):
...
class MarketSummaryV2(Resource):
    ...
    @analytics_v2_ns.param('symbols', 'Comma-separated list of symbols', default='BTC,ETH,BNB,ADA,SOL')
    ...
    def get(self):
        ...
        try:
            ...
            market_summary = loop.run_until_complete(
                analytics_service.get_market_summary(symbols)
            )
...
class MCPAnalysisV2(Resource):
    ...
    @analytics_v2_ns.param('analysis_depth', 'Analysis depth (basic, detailed, comprehensive)', default='detailed')
    def get(self, symbol):
