...
prediction_v2_model = ml_v2_ns.model(
    "MLPredictionV2",
    {
...
class MLPredictionV2(Resource):
    ...
    @ml_v2_ns.param(
        "model_type", "Model type (lstm, transformer, xgboost, ensemble)", default="ensemble"
    )
    ...
    def get(self, symbol):
        ...
        try:
            result = loop.run_until_complete(ml_service.get_prediction(symbol, horizon, model_type))
    ...
    def get(self):
        ...
        try:
            if symbol:
                ...
                result = loop.run_until_complete(ml_service.get_model_performance(symbol, "24h"))
                return {"symbol": symbol.upper(), "performance": result, "api_version": "2.0"}
            else:
                ...
                "recent_completions": [
                    {
                        "model_type": "ensemble",
                        "symbol": "ADA",
                        "completed_at": "2024-08-18T16:45:00Z",
                        "performance": 0.73,
                    }
                ],
                "auto_retraining": {
...
class MLTrainingTriggerV2(Resource):
        ...
        ml_v2_ns.model(
            "TrainingRequestV2",
            {
                ...
                "notify_completion": fields.Boolean(
                    description="Send notification on completion", default=True
                ),
            },
        )
    )
    def post(self):
        ...
        try:
            result = loop.run_until_complete(ml_service.trigger_training(symbols, model_types))
...
class MLFeaturesV2(Resource):
    ...
    def get(self, symbol):
        ...
        try:
            result = loop.run_until_complete(ml_service.get_feature_importance(symbol, model_type))
