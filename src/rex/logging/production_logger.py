...
def setup_production_logging(config: LoggingConfig = None) -> StructuredLogger:
    ...
    if config.ELASTICSEARCH_HOST:
        try:
            ...
            print("Elasticsearch logging not available (python-elasticsearch-logger not installed)", file=sys.stderr)
        except Exception as e:
            print(f"Failed to setup Elasticsearch logging: {e}", file=sys.stderr)
    
    # Create main application logger
    app_logger = StructuredLogger('reks.a2a')
    app_logger.info("Production logging configured", 
...
def log_agent_operation(agent_id: str, operation: str):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            logger.info(f"Agent operation started: {operation}",
                       operation=operation,
                       agent_id=agent_id)
            ...
            try:
                ...
                logger.info(f"Agent operation completed: {operation}",
                           operation=operation,
                           agent_id=agent_id,
                           duration=duration,
                           success=True)
                ...
                logger.error(f"Agent operation failed: {operation}",
                           error=e,
                           operation=operation,
                           agent_id=agent_id,
                           duration=duration,
