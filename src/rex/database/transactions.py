...
class TransactionManager:
    ...
    def transaction(
        ...
        try:
            ...
            transaction_info['duration'] = (transaction_info['end_time'] - start_time).total_seconds()
            ...
            if thread_id in self.active_transactions:
                self.active_transactions[thread_id] = [
                    t for t in self.active_transactions[thread_id] 
    ...
    def get_transaction_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'rolled_back': len([t for t in self.transaction_log if t.get('status') == 'rolled_back']),
        ...
        if completed_transactions:
            stats['avg_duration'] = sum(t['duration'] for t in completed_transactions) / len(completed_transactions)
...
class DistributedTransactionManager:
    ...
    def __init__(self, db_clients: Dict[str, Any]):
        ...
        self.transaction_managers = {
            name: TransactionManager(client) 
    ...
    def distributed_transaction(self, participants: List[str] = None):
        ...
        try:
            ...
            for db_name in participants:
                ...
                # Start transaction
                session.begin()
                logger.info(f"Prepared transaction on {db_name} for distributed transaction {transaction_id}")
            ...
            for db_name, session in sessions.items():
                session.commit()
                logger.info(f"Committed transaction on {db_name} for distributed transaction {transaction_id}")
...
def transactional(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            return tx_manager.execute_in_transaction(
