sap.ui.define([
    ...
    "../utils/Constants"
], function (Controller, Fragment, MessageToast, JSONModel, Constants) {
    "use strict";
    ...
    return Controller.extend("com.rex.trading.controller.LanguageSwitcher", {
        ...
        onInit: function () {
            ...
            const _oModel = new JSONModel(oLanguages);
        ...
        onLanguageChange: function (oEvent) {
            ...
            // Show confirmation
            const sMessage = sSelectedLanguage === "ru" ?
        ...
        _getCurrentLanguage: function () {
            const sStoredLang = localStorage.getItem("app-language");
            if (sStoredLang) {
                return sStoredLang;
            }
        ...
        _setLanguage: function (sLanguage) {
            ...
            // Update model
            const _oModel = this.getView().getModel("languages");
        ...
        _loadI18nModel: function () {
            const sCurrentLang = this._getCurrentLanguage();
            let sI18nPath = "i18n/i18n";
        ...
        translatePage: async function() {
            const sTargetLang = this._getCurrentLanguage();
            if (sTargetLang === "en") return; // No translation needed

            // Get all text elements
            const aElements = document.querySelectorAll("[data-translate=\"true\"]");

            for (const element of aElements) {
                const sOriginalText = element.getAttribute("data-original-text") || element.textContent;
                element.setAttribute("data-original-text", sOriginalText);

                const sTranslated = await this.getAITranslation(sOriginalText, sTargetLang);
