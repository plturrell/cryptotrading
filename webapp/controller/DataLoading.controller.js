sap.ui.define([
    ...
    "sap/m/MessageBox",
    "../utils/Constants"
], function (Controller, JSONModel, MessageToast, MessageBox, Constants) {
    "use strict";
    ...
    return Controller.extend("com.rex.cryptotrading.controller.DataLoading", {
        ...
        onInit: function () {
            ...
            const _oModel = new JSONModel({
                ...
                yahooStartDate: new Date(Date.now() - Constants.TIME.DAYS_30_MS), // 30 days ago
                yahooEndDate: new Date(),
                fredStartDate: new Date(Date.now() - Constants.TIME.DAYS_365_MS), // 1 year ago
        ...
        onLoadYahooData: function() {
            const _oModel = this.getView().getModel("dataLoading");
            const aSymbols = this.byId("yahooSymbols").getSelectedKeys();
            const oDateRange = this.byId("yahooDateRange");
            const sInterval = this.byId("yahooInterval").getSelectedKey();
        ...
        onLoadFREDData: function() {
            const _oModel = this.getView().getModel("dataLoading");
            const aSeries = this.byId("fredSeries").getSelectedKeys();
            const oDateRange = this.byId("fredDateRange");
        ...
        onLoadGeckoData: function() {
            const aNetworks = this.byId("geckoNetworks").getSelectedKeys();
            const iPoolCount = this.byId("poolCount").getValue();
            const bIncludeVolume = this.byId("includeVolume").getSelected();
            const bIncludeLiquidity = this.byId("includeLiquidity").getSelected();
        ...
        _loadAllData: function() {
            const aYahooSymbols = this.byId("yahooSymbols").getSelectedKeys();
            const aFredSeries = this.byId("fredSeries").getSelectedKeys();
            const aNetworks = this.byId("geckoNetworks").getSelectedKeys();
            const oYahooDateRange = this.byId("yahooDateRange");
        ...
        onCancelJob: function(oEvent) {
            const oContext = oEvent.getSource().getBindingContext("dataLoading");
            ...
            MessageBox.confirm("Cancel all active jobs?", {
                onClose: function(oAction) {
                    if (oAction === MessageBox.Action.OK) {
                        ...
                        const aJobs = this.getView().getModel("dataLoading").getProperty("/activeJobs");
