sap.ui.define([
    ...
    return BaseObject.extend("com.rex.cryptotrading.model.DataManager", {
         ...
         * @param {Object} oData - Data to se
         * @param {boolean} bMerge - Whether to merge with existing data
         */
        setData: function(oData, bMerge) {
            const oOldData = this._oModel.getData();

            if (bMerge) {
                const oMergedData = Object.assign({}, oOldData, oData);
        ...
        setProperty: function(sPath, vValue) {
            const vOldValue = this._oModel.getProperty(sPath);

            if (vOldValue !== vValue) {
                this._oModel.setProperty(sPath, vValue);

                // Fire property change even
        ...
        updateObject: function(sPath, oUpdates) {
            const oCurrentObject = this.getProperty(sPath) || {};
            const oUpdatedObject = Object.assign({}, oCurrentObject, oUpdates);
        ...
        addToArray: function(sArrayPath, vItem, iIndex) {
            const aArray = this.getProperty(sArrayPath) || [];
            const aNewArray = aArray.slice();
        ...
        _setupChangeDetection: function() {
            // Attach to model's checkUpdate even
