...
class HealthChecker:
    ...
    async def _check_auth_system(self) -> Dict[str, Any]:
        ...
        try:
            from .auth import MCPAuthenticator, AuthCredentials
    ...
    async def get_health_status(self, include_details: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            response = {
                ...
                "timestamp": time.time(),
                "checks_passed": sum(1 for r in results.values() if r.status == HealthStatus.HEALTHY),
                "total_checks": len(results)
            }
            
            if include_details:
                response["checks"] = {
                    name: check.to_dict() for name, check in results.items()
                }
